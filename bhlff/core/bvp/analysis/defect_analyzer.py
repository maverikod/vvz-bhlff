...
class TopologicalDefectAnalyzer:
    ...
    def analyze_block_defects(self, phase_block: np.ndarray, block_offset: int = 0) -> List[Dict[str, Any]]:
        """
    ...
    def analyze_defects(self, defects: List[Dict[str, Any]], charge_locations: List[Tuple[int, ...]]) -> Dict[str, Any]:
        """
        ...
        total_positive = sum(defect["strength"] for defect in defects if defect["strength"] > 0)
        total_negative = sum(defect["strength"] for defect in defects if defect["strength"] < 0)
    ...
    def _analyze_defect_properties(self, phase: np.ndarray, location: Tuple[int, ...]) -> Dict[str, Any]:
    ...
    def _extract_defect_region(self, phase: np.ndarray, location: Tuple[int, ...]) -> np.ndarray:
    ...
    def _analyze_defect_interactions(self, defects: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        ...
        for i, defect1 in enumerate(defects):
            for j, defect2 in enumerate(defects):
                if i != j:
                    ...
                    interaction_strength = (strength1 * strength2) / (distance ** 2 + 1e-10)
    ...
    def _analyze_defect_distribution(self, defects: List[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    def _compute_distribution_uniformity(self, locations: List[Tuple[int, ...]]) -> float:
