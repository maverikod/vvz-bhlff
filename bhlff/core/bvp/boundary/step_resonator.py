...
class FrequencyDependentResonator:
    ...
    def compute_coefficients(self, frequencies: np.ndarray) -> Tuple[np.ndarray, np.ndarray]:
        """
...
class CascadeResonatorFilter:
    ...
    def apply_cascade_filter(self, 
                           field: np.ndarray, 
                           frequencies: np.ndarray,
                           axes: Iterable[int] = (0, 1, 2)) -> np.ndarray:
        """
...
def create_frequency_dependent_resonator(field_shape: Tuple[int, ...], 
                                       frequency_axis: int = -1,
                                       parameters: Optional[Dict[str, Any]] = None) -> FrequencyDependentResonator:
    """
...
def create_cascade_filter(stages: int = 3, 
                         parameters: Optional[Dict[str, Any]] = None) -> CascadeResonatorFilter:
    """
