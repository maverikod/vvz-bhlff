...
class BVPBlockProcessor:
    ...
    def solve_envelope_blocked(self, source: np.ndarray, 
        ...
        for iteration in range(max_iterations):
            ...
            for block_data, block_info in self.block_processor.iterate_blocks():
                ...
                block_solution = self._solve_block_bvp(block_data, source_block, block_info)
    ...
    def _extract_source_block(self, source: np.ndarray, block_info: BlockInfo) -> np.ndarray:
    ...
    def _solve_block_bvp(self, current_block: np.ndarray, source_block: np.ndarray, 
        ...
        susceptibility_block = self._compute_block_susceptibility(current_block, block_info)
    ...
    def _compute_block_stiffness(self, block_data: np.ndarray, block_info: BlockInfo) -> np.ndarray:
        """Compute stiffness matrix for block."""
    ...
    def _compute_block_susceptibility(self, block_data: np.ndarray, block_info: BlockInfo) -> np.ndarray:
        ...
        chi_double_prime_0 = self.config.get("envelope_equation", {}).get("chi_double_prime_0", 0.1)
    ...
    def _check_convergence(self, old_solution: np.ndarray, new_solution: np.ndarray, 
    ...
    def _extract_envelope_block(self, envelope: np.ndarray, block_info: BlockInfo) -> np.ndarray:
    ...
    def _detect_block_quenches(self, envelope_block: np.ndarray, block_info: BlockInfo) -> List[Dict[str, Any]]:
    ...
    def _compute_block_impedance(self, envelope_block: np.ndarray, block_info: BlockInfo) -> np.ndarray:
