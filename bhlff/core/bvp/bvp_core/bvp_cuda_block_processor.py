...
class BVPCUDABlockProcessor(CUDABlockProcessor):
    ...
    def solve_envelope_cuda_blocked(
        ...
        for iteration in range(max_iterations):
            ...
            for block_data, block_info in self.iterate_blocks_cuda():
                ...
                block_solution = self._solve_block_bvp_cuda(
    ...
    def _solve_envelope_cpu_fallback(
    ...
    def _extract_source_block_cuda(
    ...
    def _solve_block_bvp_cuda(
        self, current_block: cp.ndarray, source_block: cp.ndarray, block_info
    ) -> cp.ndarray:
        """
        ...
        susceptibility_block = self._compute_block_susceptibility_cuda(
    ...
    def _compute_block_stiffness_cuda(
        ...
        stiffness_matrix = self.helpers.compute_full_stiffness_matrix_cuda(
            block_data, block_info, stiffness
        )

        return stiffness_matrix.reshape(block_data.shape + block_data.shape)

    def _compute_block_susceptibility_cuda(
        self, block_data: cp.ndarray, block_info
    ) -> cp.ndarray:
    ...
    def _check_convergence_cuda(
        ...
        relative_error = cp.linalg.norm(new_solution - old_solution) / cp.linalg.norm(
            old_solution
        )
    ...
    def detect_quenches_cuda_blocked(self, envelope: np.ndarray) -> Dict[str, Any]:
        ...
        for block_data, block_info in self.iterate_blocks_cuda():
            ...
            block_quenches = self._detect_block_quenches_cuda(
    ...
    def _extract_envelope_block_cuda(
    ...
    def _detect_block_quenches_cuda(
    ...
    def compute_impedance_cuda_blocked(self, envelope: np.ndarray) -> np.ndarray:
        ...
        for block_data, block_info in self.iterate_blocks_cuda():
            ...
            block_impedance = self._compute_block_impedance_cuda(
                envelope_block, block_info
            )
    ...
    def _compute_block_impedance_cuda(
        self, envelope_block: cp.ndarray, block_info
    ) -> cp.ndarray:
