...
class BVPCudaBlockProcessorHelpers:
    ...
    def compute_full_stiffness_matrix_cuda(self, block_data: cp.ndarray, block_info, stiffness: cp.ndarray) -> cp.ndarray:
        """
        ...
        for i in range(block_size):
            for j in range(block_size):
                ...
                if i == j:
                    ...
                    spatial_distance = self._compute_spatial_distance_cuda(i, j, block_shape)
                    coupling_strength = self._compute_7d_bvp_coupling_cuda(spatial_distance, stiffness)
                    stiffness_matrix[i, j] = coupling_strength
        
        # Apply 7D BVP boundary conditions on GPU
        boundary_conditions = self._apply_7d_bvp_boundary_conditions_cuda(block_info, block_shape)
        stiffness_matrix += boundary_conditions
        
        return stiffness_matrix
    
    def compute_full_susceptibility_matrix_cuda(self, block_data: cp.ndarray, block_info, susceptibility: cp.ndarray) -> cp.ndarray:
        """
    ...
    def _compute_7d_bvp_coupling_cuda(self, spatial_distance: float, stiffness: cp.ndarray) -> complex:
        """
    ...
    def _apply_7d_bvp_boundary_conditions_cuda(self, block_info, block_shape) -> cp.ndarray:
        """
    ...
    def _compute_7d_bvp_amplitude_coupling_cuda(self, field_i: complex, field_j: complex, susceptibility: cp.ndarray) -> complex:
        """
    ...
    def _apply_7d_bvp_phase_coupling_cuda(self, block_data: cp.ndarray, block_info) -> cp.ndarray:
        """
        ...
        for i in range(block_size):
            for j in range(block_size):
                if i != j:
                    ...
                    phase_diff = cp.angle(block_data.flat[i]) - cp.angle(block_data.flat[j])
