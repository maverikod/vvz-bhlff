...
class BVPCore:
    ...
    def solve_envelope(self, source: np.ndarray) -> np.ndarray:
        ...
        if source.shape != self.domain_7d.shape:
            raise ValueError(f"Source shape {source.shape} incompatible with domain shape {self.domain_7d.shape}")
    ...
    def _create_default_domain(self) -> Domain7D:
        ...
        return Domain7D(
            ...
            dimensions=7,
            phase_periods=domain_config.get("phase_periods", [2*np.pi, 2*np.pi, 2*np.pi]),
