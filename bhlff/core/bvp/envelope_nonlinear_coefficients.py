...
class EnvelopeNonlinearCoefficients:
    ...
    def __init__(self, constants: BVPConstants):
        ...
        try:
            self.nonlinear_threshold = constants.get_envelope_parameter("nonlinear_threshold")
        except KeyError:
    ...
    @memory_protected_class_method(memory_threshold=0.8, shape_param='envelope', dtype_param='envelope')
    ...
    def _compute_local_adaptive_scale(self, amplitude: np.ndarray) -> np.ndarray:
        ...
        amplitude_gradient = np.sqrt(sum(np.gradient(amplitude, axis=i)**2 for i in range(amplitude.ndim)))
        
        # Compute phase coherence (if complex field)
        if np.iscomplexobj(amplitude):
            phase = np.angle(amplitude)
            phase_gradient = np.sqrt(sum(np.gradient(phase, axis=i)**2 for i in range(phase.ndim)))
