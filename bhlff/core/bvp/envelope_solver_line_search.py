...
class EnvelopeSolverLineSearch:
    ...
    def __init__(self, constants: Optional[BVPConstants] = None) -> None:
        ...
        self.max_iterations = int(self.constants.get_numerical_parameter("line_search_max_iter"))
    ...
    def perform_line_search(
        ...
        for _ in range(self.max_iterations):
            ...
            if new_norm <= current_norm + self.gamma * alpha * np.real(np.sum(residual.conj() * delta_envelope)):
                return alpha
    ...
    def perform_wolfe_line_search(
        ...
        c2 = self.constants.get_numerical_parameter("curvature_c2")  # Curvature parameter
    ...
    def perform_adaptive_line_search(
        ...
        for _ in range(self.max_iterations):
            ...
            if new_norm <= current_norm + gamma * alpha * np.real(np.sum(residual.conj() * delta_envelope)):
                return alpha
