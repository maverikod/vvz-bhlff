...
class NodesAnalyzer:
    ...
    def analyze_node_distribution(self, envelope: np.ndarray) -> Dict[str, Any]:
        ...
        mean_distance = np.mean(distances)
        std_distance = np.std(distances)
        spherical_symmetry = 1.0 - (std_distance / mean_distance) if mean_distance > 0 else 1.0
        ...
        if node_count > 1:
            ...
            clustering_coefficient = np.std(pairwise_distances) / np.mean(pairwise_distances) if pairwise_distances else 0.0
        else:
            clustering_coefficient = 0.0
