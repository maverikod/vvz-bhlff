...
class PowerLawCore:
    ...
    def _fit_power_law(self, region_data: Dict[str, np.ndarray]) -> Dict[str, float]:
        ...
        try:
            ...
            r_squared = self._compute_r_squared_full(valid_distances, valid_amplitudes, popt, power_law_func)
            chi_squared = self._compute_chi_squared_full(valid_distances, valid_amplitudes, popt, power_law_func)
    ...
    def _calculate_fitting_quality(
        ...
        try:
            ...
            physical_quality = self._compute_physical_constraints_quality(exponent, coefficient)
    ...
    def _compute_r_squared_full(self, distances: np.ndarray, amplitudes: np.ndarray, 
                               popt: np.ndarray, func) -> float:
        """
    ...
    def _compute_chi_squared_full(self, distances: np.ndarray, amplitudes: np.ndarray, 
                                  popt: np.ndarray, func) -> float:
        """
        ...
        try:
            ...
            chi_squared = np.sum(((amplitudes - predicted) / np.maximum(amplitudes, 1e-10)) ** 2)
    ...
    def _compute_physical_constraints_quality(self, exponent: float, coefficient: float) -> float:
        """
