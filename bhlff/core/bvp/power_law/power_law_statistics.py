...
class PowerLawStatistics:
    ...
    def _calculate_statistical_metrics(self, envelope: np.ndarray) -> Dict[str, float]:
        ...
        k_magnitude = np.sqrt(np.sum(np.meshgrid(*[k_values]*envelope.ndim), axis=0)**2)
        
        # Bin the power spectrum
        k_bins = np.logspace(np.log10(k_values[k_values > 0].min()), 
                            np.log10(k_values.max()), 50)
        ...
        if np.sum(valid_mask) < 3:
            return {"mean_exponent": 0.0, "std_exponent": 0.0, 
        ...
        slope, intercept, r_value, p_value, std_err = np.polyfit(log_k, log_power, 1, full=True)
    ...
    def _calculate_confidence_intervals(
        ...
        for _ in range(n_bootstrap):
            # Bootstrap sample
            bootstrap_indices = np.random.choice(envelope.size, envelope.size, replace=True)
            bootstrap_envelope = envelope.flatten()[bootstrap_indices].reshape(envelope.shape)
            ...
            k_magnitude = np.sqrt(np.sum(np.meshgrid(*[k_values]*bootstrap_envelope.ndim), axis=0)**2)
            k_bins = np.logspace(np.log10(k_values[k_values > 0].min()), 
                                np.log10(k_values.max()), 50)
            power_binned = np.zeros(len(k_bins)-1)
            ...
            if np.sum(valid_mask) >= 3:
                log_k = np.log(k_centers[valid_mask])
                log_power = np.log(power_binned[valid_mask])
                slope, intercept, r_value, _, _ = np.polyfit(log_k, log_power, 1, full=True)
        ...
        alpha = 0.05
        exponent_ci = [np.percentile(exponents, 100*alpha/2), 
                      np.percentile(exponents, 100*(1-alpha/2))]
        coefficient_ci = [np.percentile(coefficients, 100*alpha/2), 
                         np.percentile(coefficients, 100*(1-alpha/2))]
        quality_ci = [np.percentile(qualities, 100*alpha/2), 
