...
class AdvancedPowerLawFitting:
    ...
    def fit_power_law_advanced(self, region_data: Dict[str, np.ndarray], 
        ...
        try:
            ...
            if method == "curve_fit":
                fit_result = self.optimization_methods.fit_using_curve_fit(radial_profile)
            elif method == "minimize":
                fit_result = self.optimization_methods.fit_using_minimize(radial_profile)
            elif method == "custom":
                fit_result = self.optimization_methods.fit_using_custom_optimization(radial_profile)
            else:
                raise ValueError(f"Unknown fitting method: {method}")
            
            # Perform comprehensive quality analysis
            quality_analysis = self.quality_analyzer.perform_comprehensive_quality_analysis(
