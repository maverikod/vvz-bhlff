...
class PowerLawRegionAnalysis:
    ...
    def analyze_regions(self, envelope: np.ndarray) -> List[Dict[str, Any]]:
        ...
        self.logger.info(f"Region analysis completed: {len(region_analyses)} regions analyzed")
        return region_analyses

    def _find_dimension_tail_regions(self, envelope: np.ndarray, dimension: int) -> List[Dict[str, Any]]:
