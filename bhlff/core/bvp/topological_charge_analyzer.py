...
class TopologicalChargeAnalyzer:
    ...
    def compute_topological_charge(self, field: np.ndarray) -> Dict[str, Any]:
        ...
        for block_idx, block_slice in enumerate(blocks):
            ...
            try:
                ...
                for defect in block_defects:
                    global_defect = tuple(
                        defect[i] + block_slice[i].start for i in range(len(defect))
                    )
                ...
                for defect_location in global_defects:
                    charge = self._compute_defect_charge_vectorized(
                        phase, defect_location
                    )
        ...
        charge_stability = self._compute_charge_stability(
            all_charges, all_charge_locations
        )

        # Analyze defects
        defect_analysis = self._analyze_defects(
            phase, all_charge_locations, all_charges
        )
    ...
    def _compute_defect_charge(
        self, phase: np.ndarray, defect_location: Tuple[int, ...]
    ) -> float:
        """
    ...
    def _compute_defect_charge_cuda(
        self, phase: np.ndarray, defect_location: Tuple[int, ...]
    ) -> float:
        """
    ...
    def _compute_defect_charge_cpu(
        self, phase: np.ndarray, defect_location: Tuple[int, ...]
    ) -> float:
        """
    ...
    def _generate_overlapping_blocks(
        self, field_shape: Tuple[int, ...], block_size: Tuple[int, ...]
    ) -> List[Tuple[slice, ...]]:
        """
                        ...
                        for m in range(0, field_shape[4], step_sizes[4]):
                            for n in range(0, field_shape[5], step_sizes[5]):
                                for o in range(0, field_shape[6], step_sizes[6]):
                                    ...
                                    block_slice = (
                                        slice(
                                            i, min(i + block_size[0], field_shape[0])
                                        ),
                                        slice(
                                            j, min(j + block_size[1], field_shape[1])
                                        ),
                                        slice(
                                            k, min(k + block_size[2], field_shape[2])
                                        ),
                                        slice(
                                            l, min(l + block_size[3], field_shape[3])
                                        ),
                                        slice(
                                            m, min(m + block_size[4], field_shape[4])
                                        ),
                                        slice(
                                            n, min(n + block_size[5], field_shape[5])
                                        ),
                                        slice(
                                            o, min(o + block_size[6], field_shape[6])
                                        ),
                                    )
    ...
    def _find_defects_vectorized(
        self, phase_block: np.ndarray
    ) -> List[Tuple[int, ...]]:
        """
    ...
    def _find_defects_cuda_vectorized(
        self, phase_block: np.ndarray
    ) -> List[Tuple[int, ...]]:
        """
    ...
    def _find_defects_cpu_vectorized(
        self, phase_block: np.ndarray
    ) -> List[Tuple[int, ...]]:
        """
    ...
    def _extract_defects_vectorized(
        self, high_grad_mask: np.ndarray
    ) -> List[Tuple[int, ...]]:
        """
    ...
    def _compute_defect_charge_vectorized(
        self, phase: np.ndarray, defect_location: Tuple[int, ...]
    ) -> float:
        """
    ...
    def _compute_charge_stability(
        self, charges: List[float], locations: List[Tuple[int, ...]]
    ) -> float:
        """
        ...
        if len(locations) > 1:
            ...
            for i in range(len(locations)):
                for j in range(i + 1, len(locations)):
                    dist = np.sqrt(
                        sum((a - b) ** 2 for a, b in zip(locations[i], locations[j]))
                    )
            ...
            if distances:
                ...
                spatial_stability = min(
                    1.0, avg_distance / 10.0
    ...
    def _analyze_defects(
        self,
        phase: np.ndarray,
        charge_locations: List[Tuple[int, ...]],
        charges: List[float],
    ) -> Dict[str, Any]:
        """
            ...
            for i in range(len(charge_locations)):
                for j in range(i + 1, len(charge_locations)):
                    ...
                    dist = np.sqrt(
                        sum(
                            (a - b) ** 2
                            for a, b in zip(charge_locations[i], charge_locations[j])
                        )
                    )
                    interaction = (
                        charges[i] * charges[j] / (dist + 1e-6)
