...
class TopologicalDefectAnalyzer:
    ...
    @memory_protected_class_method(memory_threshold=0.8, shape_param='phase', dtype_param='phase')
    ...
    def _find_topological_defects_cuda(self, phase: np.ndarray) -> List[Tuple[int, ...]]:
        """
    ...
    def analyze_defect_types(self, phase: np.ndarray, defect_locations: List[Tuple[int, ...]]) -> List[str]:
        """
    ...
    def analyze_defect_interactions(self, defect_locations: List[Tuple[int, ...]], 
                                  defect_charges: List[float]) -> Dict[str, Any]:
        """
        ...
        for i in range(len(defect_locations)):
            for j in range(i + 1, len(defect_locations)):
                ...
                dist = np.sqrt(sum((a - b)**2 for a, b in zip(defect_locations[i], defect_locations[j])))
    ...
    def _extract_neighborhood(self, field: np.ndarray, center: Tuple[int, ...], radius: int) -> np.ndarray:
        """
