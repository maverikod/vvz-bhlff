...
class BlockProcessor:
    ...
    def __init__(self, domain: Domain, block_size: int = 8, overlap: int = 2):
        ...
        self.logger.info(f"Block processor initialized: {self.total_blocks} blocks, "
    ...
    def _compute_block_configuration(self) -> None:
        ...
        for dim_size in self.domain_shape:
            ...
            n_blocks = max(1, (dim_size - self.overlap) // (self.block_size - self.overlap))
        ...
        self.block_memory_usage = np.prod([self.block_size] * self.n_dims) * 8 * 1e-9  # GB
        self.total_memory_usage = self.block_memory_usage * self.total_blocks
        
        self.logger.info(f"Block configuration: {self.blocks_per_dim} blocks per dimension")
    ...
    def iterate_blocks(self) -> Iterator[Tuple[np.ndarray, BlockInfo]]:
        ...
        for block_indices in self._generate_block_indices():
            ...
            block_info = BlockInfo(
                ...
                shape=tuple(end - start for start, end in zip(start_indices, end_indices)),
    ...
    def _generate_block_indices(self) -> Iterator[Tuple[int, ...]]:
        ...
        for block_indices in product(*[range(n_blocks) for n_blocks in self.blocks_per_dim]):
            yield block_indices
    
    def _compute_block_boundaries(self, block_indices: Tuple[int, ...]) -> Tuple[Tuple[int, ...], Tuple[int, ...]]:
        """Compute block boundaries for given block indices."""
        start_indices = []
        end_indices = []
        
        for i, (block_idx, dim_size) in enumerate(zip(block_indices, self.domain_shape)):
            # Compute start index
            if block_idx == 0:
                start_idx = 0
            else:
    ...
    def _extract_block_data(self, start_indices: Tuple[int, ...], end_indices: Tuple[int, ...]) -> np.ndarray:
    ...
    def process_block(self, block_data: np.ndarray, block_info: BlockInfo, 
    ...
    def _process_block_fft(self, block_data: np.ndarray, block_info: BlockInfo) -> np.ndarray:
    ...
    def _process_block_convolution(self, block_data: np.ndarray, block_info: BlockInfo) -> np.ndarray:
    ...
    def _process_block_gradient(self, block_data: np.ndarray, block_info: BlockInfo) -> np.ndarray:
    ...
    def merge_blocks(self, processed_blocks: List[Tuple[np.ndarray, BlockInfo]]) -> np.ndarray:
        """
    ...
    def _create_weight_mask(self, block_info: BlockInfo) -> np.ndarray:
        ...
        for dim in range(self.n_dims):
            if block_info.start_indices[dim] > 0:
                ...
                weight_mask[tuple(slice(0, overlap_size) if i == dim else slice(None) 
                                for i in range(self.n_dims))] *= 0.5
            
            if block_info.end_indices[dim] < self.domain_shape[dim]:
                # Overlap at end
                overlap_size = min(self.overlap, block_shape[dim])
                weight_mask[tuple(slice(-overlap_size, None) if i == dim else slice(None) 
                                for i in range(self.n_dims))] *= 0.5
    ...
    def optimize_block_size(self, available_memory_gb: float) -> int:
        ...
        self.logger.info(f"Optimized block size: {optimized_size} (available memory: {available_memory_gb} GB)")
