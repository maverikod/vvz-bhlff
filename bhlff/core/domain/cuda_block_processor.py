...
class CUDABlockProcessor(BlockProcessor):
    ...
    def _initialize_cuda(self) -> None:
        ...
        try:
            ...
            self.logger.info(f"CUDA device {self.device_id}: {self.free_memory / 1e9:.1f} GB free memory")
    ...
    def _extract_block_data_cuda(self, start_indices: Tuple[int, ...], 
                               end_indices: Tuple[int, ...]) -> cp.ndarray:
    ...
    def process_block_cuda(self, block_data: cp.ndarray, block_info: BlockInfo, 
                          operation: str = "fft") -> cp.ndarray:
        """
    ...
    def _process_block_fft_cuda(self, block_data: cp.ndarray, block_info: BlockInfo) -> cp.ndarray:
    ...
    def _process_block_convolution_cuda(self, block_data: cp.ndarray, block_info: BlockInfo) -> cp.ndarray:
    ...
    def _process_block_gradient_cuda(self, block_data: cp.ndarray, block_info: BlockInfo) -> cp.ndarray:
    ...
    def _process_block_bvp_cuda(self, block_data: cp.ndarray, block_info: BlockInfo) -> cp.ndarray:
    ...
    def merge_blocks_cuda(self, processed_blocks: List[Tuple[cp.ndarray, BlockInfo]]) -> cp.ndarray:
        ...
        if not self.cuda_available:
            # Fallback to CPU processing
            cpu_blocks = [(cp.asnumpy(block_data), block_info) 
    ...
    def optimize_block_size_cuda(self, available_memory_gb: float = None) -> int:
        ...
        self.logger.info(f"CUDA optimized block size: {optimized_size} "
