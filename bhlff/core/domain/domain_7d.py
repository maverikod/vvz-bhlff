...
class Domain7D:
    ...
    def __init__(self, spatial_config: SpatialConfig, phase_config: PhaseConfig, 
    ...
    def _setup_phase_coordinates(self) -> None:
        ...
        self.phi_1 = np.linspace(0, self.phase_config.phi_1_max, self.phase_config.N_phi_1, endpoint=False)
        self.phi_2 = np.linspace(0, self.phase_config.phi_2_max, self.phase_config.N_phi_2, endpoint=False)
        self.phi_3 = np.linspace(0, self.phase_config.phi_3_max, self.phase_config.N_phi_3, endpoint=False)
    ...
    def get_spatial_shape(self) -> Tuple[int, int, int]:
        ...
        """
        return (self.spatial_config.N_x, self.spatial_config.N_y, self.spatial_config.N_z)
    ...
    def get_phase_shape(self) -> Tuple[int, int, int]:
        ...
        """
        return (self.phase_config.N_phi_1, self.phase_config.N_phi_2, self.phase_config.N_phi_3)
    ...
    def get_full_7d_shape(self) -> Tuple[int, int, int, int, int, int]:
        ...
        """
        return (self.spatial_config.N_x, self.spatial_config.N_y, self.spatial_config.N_z,
                self.phase_config.N_phi_1, self.phase_config.N_phi_2, self.phase_config.N_phi_3)
    ...
    def compute_7d_volume_element(self) -> np.ndarray:
        ...
        volume_element = (self.dx * self.dy * self.dz * 
                         self.dphi_1 * self.dphi_2 * self.dphi_3 * self.dt)
