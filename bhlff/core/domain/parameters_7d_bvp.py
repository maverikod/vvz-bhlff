...
class Parameters7DBVP:
    ...
    def __post_init__(self):
        ...
        self.logger.info(f"Parameters7DBVP initialized with kappa_0={self.kappa_0}, chi_prime={self.chi_prime}")
    ...
    def _validate_parameters(self) -> None:
        ...
        if self.chi_double_prime_0 < 0:
            raise ValueError(f"chi_double_prime_0 must be non-negative, got {self.chi_double_prime_0}")
        if self.chi_double_prime_2 < 0:
            raise ValueError(f"chi_double_prime_2 must be non-negative, got {self.chi_double_prime_2}")
        if self.k0 <= 0:
        ...
        if self.lambda_param < 0:
            raise ValueError(f"lambda_param must be non-negative, got {self.lambda_param}")
        if self.nu <= 0:
    ...
    def compute_susceptibility(self, amplitude: np.ndarray) -> np.ndarray:
        ...
        """
        chi_double_prime = self.chi_double_prime_0 + self.chi_double_prime_2 * amplitude**2
