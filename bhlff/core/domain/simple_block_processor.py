...
class SimpleBlockProcessor:
    ...
    def __init__(self, domain: Domain, config: SimpleConfig = None):
        ...
        self.logger.info(f"Simple block processor initialized: "
    ...
    def _merge_block_result(self, result: np.ndarray, block_result: np.ndarray, 
        ...
        if block_result.shape != result[slices].shape:
            self.logger.warning(f"Shape mismatch: block_result {block_result.shape} vs result slice {result[slices].shape}")
