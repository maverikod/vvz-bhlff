...
class VectorizedBlockOperations:
    ...
    def _merge_blocks_cuda_vectorized(
        ...
        for block, block_info in zip(blocks, block_indices):
            ...
            weight_mask = self._create_weight_mask_cuda(block_info, overlap)
    ...
    def _merge_blocks_cpu_vectorized(
        ...
        for block, block_info in zip(blocks, block_indices):
            ...
            # Create weight mask for overlap handling
            weight_mask = self._create_weight_mask_cpu(block_info, overlap)
    ...
    def _create_weight_mask_cuda(
        self, block_info: BlockInfo, overlap: int
    ) -> "cp.ndarray":
        """
        ...
        for dim in range(len(self.domain_shape)):
            if block_info.start_indices[dim] > 0 and overlap > 0:
                ...
                weight_mask[
                    tuple(
                        ...
                        for i in range(len(self.domain_shape))
                    )
                ] *= 0.5
            
            if (
                block_info.end_indices[dim] < self.domain_shape[dim]
                and overlap > 0
            ):
                # Overlap at end
                overlap_size = min(overlap, block_shape[dim])
