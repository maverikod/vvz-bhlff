...
class VectorizedBlockProcessor(BlockProcessor):
    ...
    def __init__(self, domain: Domain, block_size: int = 8, overlap: int = 2, 
        ...
        if self.use_cuda:
            self.logger.info("Vectorized block processor initialized with CUDA acceleration")
        else:
            self.logger.info("Vectorized block processor initialized with CPU vectorization")
    
    def process_blocks_vectorized(self, operation: str = "fft", 
        ...
        self.logger.info(f"Processing {self.total_blocks} blocks vectorized with batch size {batch_size}")
        
        # Collect all blocks
        all_blocks = list(self.iterate_blocks())
        
        if self.use_cuda:
            return self._process_blocks_cuda_vectorized(all_blocks, operation, batch_size)
        else:
            return self._process_blocks_cpu_vectorized(all_blocks, operation, batch_size)
    
    def _process_blocks_cuda_vectorized(self, all_blocks: List[Tuple[np.ndarray, BlockInfo]], 
                                       operation: str, batch_size: int) -> np.ndarray:
        ...
        for i in range(0, len(gpu_blocks), batch_size):
            ...
            batch_processed = self._process_batch_cuda_vectorized(batch_blocks, operation)
    ...
    def _process_blocks_cpu_vectorized(self, all_blocks: List[Tuple[np.ndarray, BlockInfo]], 
                                      operation: str, batch_size: int) -> np.ndarray:
        ...
        for i in range(0, len(all_blocks), batch_size):
            ...
            batch_processed = self._process_batch_cpu_vectorized(batch_blocks, operation)
            processed_blocks.extend(batch_processed)
    ...
    def _process_batch_cuda_vectorized(self, batch_blocks: List[Tuple[np.ndarray, BlockInfo]], 
        ...
        processed_blocks = []
        for i, (processed_block, block_info) in enumerate(zip(processed_stacked, block_info_list)):
            processed_blocks.append((processed_block, block_info))
        
        return processed_blocks
    
    def _process_batch_cpu_vectorized(self, batch_blocks: List[Tuple[np.ndarray, BlockInfo]], 
                                    operation: str) -> List[Tuple[np.ndarray, BlockInfo]]:
    ...
    def merge_blocks_cuda(self, processed_blocks: List[Tuple['cp.ndarray', BlockInfo]]) -> 'cp.ndarray':
        ...
        result = cp.divide(result, weight_map, out=cp.zeros_like(result), where=weight_map != 0)
        ...
        return {
            **base_info,
            ...
            "vectorized_operations": [
                "fft", "convolution", "gradient", "bvp_solve"
            ]
        }
