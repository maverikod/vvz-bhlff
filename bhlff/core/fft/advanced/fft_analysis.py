...
class FFTAnalysis:
    ...
    def _analyze_solution(self, solution: np.ndarray, source: np.ndarray) -> Dict[str, Any]:
        ...
        return {
            ...
            'relative_residual': residual_norm / solution_norm if solution_norm > 0 else 0.0,
            'convergence_quality': 'good' if residual_norm < 1e-6 else 'fair' if residual_norm < 1e-3 else 'poor'
        }
