...
class FFTPlan7D:
    ...
    def __init__(self, domain_shape: Tuple[int, ...], precision: str = "float64"):
        ...
        self.logger.info(f"FFTPlan7D initialized: domain={domain_shape}, precision={precision}")
    ...
    def execute_fft(self, field: np.ndarray, direction: str = 'forward') -> np.ndarray:
        ...
        if field.shape != self.domain_shape:
            raise ValueError(f"Field shape {field.shape} incompatible with domain shape {self.domain_shape}")
    ...
    def execute_block_fft(self, field: np.ndarray, block_size: Tuple[int, ...], 
        ...
        num_blocks = tuple((n + bs - 1) // bs for n, bs in zip(self.domain_shape, block_size))
        ...
        for block_idx in np.ndindex(num_blocks):
            ...
            block_slice = tuple(
                slice(block_idx[i] * block_size[i], 
    ...
    def get_performance_stats(self) -> Dict[str, Any]:
        ...
        """
        total_calls = (self.performance_stats['forward_fft_calls'] + 
                      self.performance_stats['inverse_fft_calls'])
        
        if total_calls > 0:
            avg_forward_time = (self.performance_stats['total_forward_time'] / 
                              max(1, self.performance_stats['forward_fft_calls']))
            avg_inverse_time = (self.performance_stats['total_inverse_time'] / 
                              max(1, self.performance_stats['inverse_fft_calls']))
        else:
        ...
        return {
            ...
            'total_forward_time_ms': self.performance_stats['total_forward_time'] * 1000,
            'total_inverse_time_ms': self.performance_stats['total_inverse_time'] * 1000,
    ...
    def _create_fft_plan(self, direction: str, optimize: bool = False) -> Dict[str, Any]:
        """
    ...
    def _create_block_fft_plan(self, direction: str, optimize: bool = False) -> Dict[str, Any]:
        """
