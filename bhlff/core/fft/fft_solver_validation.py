...
class FFTSolverValidation:
    ...
    def validate_solution(self, solution: np.ndarray, source: np.ndarray,
        ...
        if solution.shape != self.domain.shape:
            raise ValueError(f"Solution shape {solution.shape} incompatible with domain {self.domain.shape}")
        
        if source.shape != self.domain.shape:
            raise ValueError(f"Source shape {source.shape} incompatible with domain {self.domain.shape}")
        ...
        if is_valid:
            self.logger.info(f"Solution validation passed: relative residual = {relative_residual:.2e}")
        else:
            self.logger.warning(f"Solution validation failed: relative residual = {relative_residual:.2e} > {tolerance:.2e}")
    ...
    def check_energy_conservation(self, field: np.ndarray, 
        ...
        if expected_energy is not None:
            ...
            relative_error = energy_error / abs(expected_energy) if expected_energy != 0 else energy_error
            result.update({
            ...
            if result['is_conserved']:
                self.logger.info(f"Energy conservation passed: relative error = {relative_error:.2e}")
            else:
                self.logger.warning(f"Energy conservation failed: relative error = {relative_error:.2e} > {tolerance:.2e}")
        ...
        if boundary_type == 'periodic':
            ...
            for i in range(3):
                ...
                if boundary_error > 1e-12:
                    ...
                    result['errors'].append(f"Periodicity violation in dimension {i}: error = {boundary_error:.2e}")
            ...
            for i in range(3):
                ...
                if boundary_error > 1e-12:
                    result['is_satisfied'] = False
                    result['errors'].append(f"Periodicity violation in phase dimension {i}: error = {boundary_error:.2e}")
        
        elif boundary_type == 'dirichlet':
            # Check that field is zero at boundaries
            boundary_error = np.linalg.norm(field[0, :, :, :, :, :, :]) + np.linalg.norm(field[-1, :, :, :, :, :, :])
            if boundary_error > 1e-12:
                result['is_satisfied'] = False
                result['errors'].append(f"Dirichlet boundary condition violation: error = {boundary_error:.2e}")
        
        if result['is_satisfied']:
            self.logger.info(f"Boundary conditions satisfied for {boundary_type} boundaries")
        else:
            self.logger.warning(f"Boundary conditions violated for {boundary_type} boundaries: {result['errors']}")
