...
class FractionalLaplacian:
    ...
    def handle_special_cases(self, k_magnitude: np.ndarray) -> np.ndarray:
        ...
        if max_k > 0 and 2 * self.beta * np.log(max_k) > 700:  # exp(700) ≈ float64 max
            self.logger.warning(
                f"Potential overflow in |k|^(2β) computation for β={self.beta}"
            )
    ...
    def get_operator_info(self) -> Dict[str, Any]:
        ...
        """
        max_k = np.max(
            np.sqrt(sum(K**2 for K in np.meshgrid(*self._wave_vectors, indexing="ij")))
        )
        ...
        return {
            ...
            "interaction_range": (
                "local"
                if self.beta < 0.5
                else "intermediate" if self.beta < 1.5 else "long_range"
            ),
