...
class MemoryManager7D:
    ...
    def __init__(self, domain_shape: Tuple[int, ...], max_memory_gb: float = 8.0):
        ...
        self.logger.info(f"MemoryManager7D initialized: domain={domain_shape}, "
    ...
    def optimize_memory(self) -> None:
        ...
        if status['memory_utilization'] > 0.8:  # 80% threshold
            ...
            blocks_to_compress = list(self.active_blocks.keys())[:-5]  # Keep last 5 blocks
            ...
            self.logger.info(f"Memory optimization complete: {len(blocks_to_compress)} blocks compressed")
    ...
    def _calculate_optimal_block_size(self) -> Tuple[int, ...]:
        ...
        target_elements = min(total_elements, elements_per_gb // 4)  # Use 1/4 of available memory
        ...
        if len(self.domain_shape) == 7:
            ...
            scale_factor = (target_elements / total_elements) ** (1/len(self.domain_shape))
            block_size = tuple([max(1, int(n * scale_factor)) for n in self.domain_shape])
    ...
    def _create_empty_block(self, block_id: Tuple[int, ...]) -> np.ndarray:
        ...
        block_dims = []
        for i, (block_idx, domain_size, block_size) in enumerate(zip(block_id, self.domain_shape, self.block_size)):
    ...
    def _compress_block(self, block_id: Tuple[int, ...], block_data: np.ndarray) -> None:
        """
    ...
    def _setup_memory_monitoring(self) -> None:
        ...
        """
        self.logger.info(f"Memory monitoring setup: max_memory={self.max_memory_bytes/1024**3:.1f}GB")
        
        # Log initial memory status
        status = self.get_memory_status()
        self.logger.info(f"Initial memory status: {status['memory_usage_gb']:.2f}GB used, "
