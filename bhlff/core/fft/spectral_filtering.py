...
class SpectralFiltering:
    ...
    def apply_low_pass_filter(self, field: np.ndarray, cutoff_frequency: float,
        ...
        transfer_function = 1.0 / (1.0 + (self._k_magnitude / cutoff_frequency)**(2 * order))
    ...
    def apply_high_pass_filter(self, field: np.ndarray, cutoff_frequency: float,
                             order: int = 2) -> np.ndarray:
        """
    ...
    def apply_band_pass_filter(self, field: np.ndarray, center_frequency: float,
                             bandwidth: float, order: int = 2) -> np.ndarray:
        """
    ...
    def apply_noise_reduction(self, field: np.ndarray, noise_level: float = 0.1,
