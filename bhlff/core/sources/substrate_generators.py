...
class TopologicalSubstrateGenerator:
    ...
    def compose_multiscale_substrate_blocked(self, base_substrate: np.ndarray, layer_config: Dict[str, Any]) -> np.ndarray:
        """
        ...
        xp = cp if self.use_cuda else np
        arr = cp.asarray(base_substrate) if (self.use_cuda and not isinstance(base_substrate, cp.ndarray)) else base_substrate
    ...
    def _optimal_block_size_3d(self, array7d) -> int:
        ...
        try:
            ...
            phi_factor = self.domain.N_phi * self.domain.N_phi * self.domain.N_phi * self.domain.N_t
