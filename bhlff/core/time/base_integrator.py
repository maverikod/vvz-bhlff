...
class BaseTimeIntegrator(ABC):
    ...
    def _validate_parameters(self) -> None:
        ...
        if self.parameters.nu <= 0:
            raise ValueError(f"Diffusion coefficient ν must be positive, got {self.parameters.nu}")
        
        if not (0 < self.parameters.beta < 2):
            raise ValueError(f"Fractional order β must be in (0,2), got {self.parameters.beta}")
        
        if self.parameters.lambda_param < 0:
            raise ValueError(f"Damping parameter λ must be non-negative, got {self.parameters.lambda_param}")
