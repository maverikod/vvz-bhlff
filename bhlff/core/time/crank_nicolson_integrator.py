...
class CrankNicolsonIntegrator(BaseTimeIntegrator):
    ...
    def integrate(self, initial_field: np.ndarray, source_field: np.ndarray, 
        ...
        self.logger.info(f"Crank-Nicolson integration completed over {len(time_steps)} time steps")
        return result
    
    def step(self, current_field: np.ndarray, current_source: np.ndarray, 
             next_source: np.ndarray, dt: float) -> np.ndarray:
        """
        ...
        current_source_spectral = self._spectral_ops.forward_fft(current_source, 'ortho')
        ...
        current_contribution = current_spectral - (dt/2) * self._spectral_coeffs * current_spectral
        
        # Source contribution
        source_contribution = (dt/2) * (current_source_spectral + next_source_spectral)
    ...
    def step_implicit(self, current_field: np.ndarray, source_field: np.ndarray, 
                     dt: float) -> np.ndarray:
        """
    ...
    def __repr__(self) -> str:
        ...
        return (f"CrankNicolsonIntegrator("
