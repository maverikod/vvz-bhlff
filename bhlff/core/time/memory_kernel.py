...
class MemoryKernel:
    ...
    def _setup_memory_system(self) -> None:
        ...
        for i in range(self.num_memory_vars):
            ...
            # Set relaxation time
            tau = default_taus[i] if i < len(default_taus) else default_taus[-1] * (i + 1)
            self.relaxation_times.append(tau)
            
            # Set coupling strength
            gamma = default_gammas[i] if i < len(default_gammas) else default_gammas[-1] / (i + 1)
            self.coupling_strengths.append(gamma)
        
        self._initialized = True
        self.logger.info(f"Memory kernel initialized with {self.num_memory_vars} variables")
    ...
    def apply(self, field: np.ndarray, time: float) -> np.ndarray:
        ...
        for i, (memory_var, gamma) in enumerate(zip(self.memory_variables, self.coupling_strengths)):
    ...
    def evolve(self, field: np.ndarray, dt: float) -> None:
        ...
        for i, (memory_var, tau) in enumerate(zip(self.memory_variables, self.relaxation_times)):
    ...
    def set_relaxation_times(self, taus: List[float]) -> None:
        ...
        if len(taus) != self.num_memory_vars:
            raise ValueError(f"Expected {self.num_memory_vars} relaxation times, got {len(taus)}")
    ...
    def set_coupling_strengths(self, gammas: List[float]) -> None:
        ...
        if len(gammas) != self.num_memory_vars:
            raise ValueError(f"Expected {self.num_memory_vars} coupling strengths, got {len(gammas)}")
