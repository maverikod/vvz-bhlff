...
class QuenchDetector:
    ...
    def __init__(self, domain: Domain, 
        ...
        self.logger.info(f"Quench detector initialized with thresholds: "
    ...
    def detect_quench(self, field: np.ndarray, time: float) -> bool:
        ...
        if self._previous_energy is not None:
            rate_of_change = abs(current_energy - self._previous_energy) / (time - self._last_time)
        ...
        if quench_detected:
            ...
            self.logger.warning(f"Quench detected at t={time:.3f}: {', '.join(quench_reasons)}")
    ...
    def set_thresholds(self, energy_threshold: Optional[float] = None,
                      rate_threshold: Optional[float] = None,
                      magnitude_threshold: Optional[float] = None) -> None:
        """
        ...
        if energy_threshold is not None:
            if energy_threshold <= 0:
                raise ValueError(f"Energy threshold must be positive, got {energy_threshold}")
            self.energy_threshold = energy_threshold
        ...
        if magnitude_threshold is not None:
            if magnitude_threshold <= 0:
                raise ValueError(f"Magnitude threshold must be positive, got {magnitude_threshold}")
            self.magnitude_threshold = magnitude_threshold
        
        self.logger.info(f"Thresholds updated: energy={self.energy_threshold}, "
