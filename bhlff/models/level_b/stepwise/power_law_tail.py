...
class PowerLawTailAnalyzer:
    ...
    def analyze(
        ...
        log_range = float(np.log10(r_tail.max() / max(r_tail.min(), self.eps)))

        slope, intercept, r_value, _, _ = stats.linregress(log_r, log_A)
        r_squared = float(r_value**2)
        theoretical_slope = float(2.0 * beta - 3.0)
        rel_err = abs(slope - theoretical_slope) / max(abs(theoretical_slope), self.eps)

        passed = (r_squared >= 0.95) and (rel_err <= 0.2) and (log_range >= min_decades)
