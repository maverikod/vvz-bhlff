...
class ZoneAnalysis:
    ...
    def identify_zone_boundaries(self, envelope: np.ndarray) -> List[Dict[str, Any]]:
        ...
        if np.any(core_mask):
            core_boundary = {
                ...
                "boundary_location": np.where(core_mask)[0][0] if len(np.where(core_mask)[0]) > 0 else 0,
        ...
        if np.any(tail_mask):
            tail_boundary = {
                ...
                "boundary_location": np.where(tail_mask)[0][-1] if len(np.where(tail_mask)[0]) > 0 else amplitude.size - 1,
    ...
    def analyze_zone_properties(self, envelope: np.ndarray) -> Dict[str, Dict[str, float]]:
        ...
        core_mask = amplitude > core_threshold
        core_properties = {
            "mean_amplitude": np.mean(amplitude[core_mask]) if np.any(core_mask) else 0.0,
        ...
        transition_properties = {
            "mean_amplitude": np.mean(amplitude[transition_mask]) if np.any(transition_mask) else 0.0,
            "variance": np.var(amplitude[transition_mask]) if np.any(transition_mask) else 0.0,
        ...
        tail_mask = amplitude < tail_threshold
        tail_properties = {
            "mean_amplitude": np.mean(amplitude[tail_mask]) if np.any(tail_mask) else 0.0,
        ...
        if amplitude.ndim >= 3:
            ...
            if len(transition_coords[0]) > 0:
                ...
                transition_region = {
                    ...
                    "region_location": (transition_coords[0][0], transition_coords[1][0], transition_coords[2][0]),
