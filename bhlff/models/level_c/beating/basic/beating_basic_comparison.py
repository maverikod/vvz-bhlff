...
class BeatingBasicComparison:
    ...
    def _compare_interference_patterns(self, patterns1: List[Dict[str, Any]], patterns2: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        if not patterns1 or not patterns2:
            return {'similarity': 0.0, 'difference': 1.0, 'pattern_count_diff': abs(len(patterns1) - len(patterns2))}
        ...
        if strengths1 and strengths2:
            ...
            strength_diff = abs(mean_strength1 - mean_strength2) / max(mean_strength1, mean_strength2) if max(mean_strength1, mean_strength2) > 0 else 0
    ...
    def _compare_mode_coupling(self, coupling1: Dict[str, Any], coupling2: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # Overall similarity
        overall_similarity = (strength_similarity + efficiency_similarity + type_similarity) / 3
    ...
    def _compute_overall_comparison(self, comparison_results: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if similarities:
            ...
            overall_difference = np.mean(differences) if differences else 1.0 - overall_similarity
        else:
        ...
        return {
            ...
            'consistency_score': 1.0 - np.std(similarities) if len(similarities) > 1 else 1.0
        }
