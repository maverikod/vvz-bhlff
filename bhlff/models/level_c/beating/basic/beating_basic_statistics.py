...
class BeatingBasicStatistics:
    ...
    def perform_statistical_analysis(self, envelope: np.ndarray, basic_results: Dict[str, Any]) -> Dict[str, Any]:
        """
    ...
    def _analyze_frequency_statistics(self, frequencies: list) -> Dict[str, Any]:
        ...
        return {
            ...
            'coefficient_of_variation': float(np.std(frequencies_array) / np.mean(frequencies_array)) if np.mean(frequencies_array) > 0 else 0.0
        }
    ...
    def _analyze_coupling_statistics(self, coupling: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if 'coupling_strength' in coupling:
            ...
            coupling_stats['strength_confidence_interval'] = [max(0.0, strength - 0.1), min(1.0, strength + 0.1)]
        ...
        if 'coupling_efficiency' in coupling:
            ...
            coupling_stats['efficiency_confidence_interval'] = [max(0.0, efficiency - 0.05), min(1.0, efficiency + 0.05)]
    ...
    def _perform_hypothesis_testing(self, envelope: np.ndarray, basic_results: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if 'beating_strength' in basic_results:
            ...
            hypothesis_tests['significant_beating'] = {
                ...
                'conclusion': 'Reject null hypothesis' if significant_beating else 'Fail to reject null hypothesis'
            }
        
        # Test for mode coupling
        if 'mode_coupling' in basic_results:
            coupling_strength = basic_results['mode_coupling'].get('coupling_strength', 0.0)
            ...
            hypothesis_tests['significant_coupling'] = {
                ...
                'conclusion': 'Reject null hypothesis' if significant_coupling else 'Fail to reject null hypothesis'
            }
    ...
    def calculate_confidence_intervals(self, data: np.ndarray, confidence_level: float = 0.95) -> Dict[str, list]:
