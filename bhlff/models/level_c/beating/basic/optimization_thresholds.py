...
class BeatingThresholdOptimizer:
    ...
    def optimize_thresholds(self, envelope: np.ndarray, results: Dict[str, Any]) -> Dict[str, Any]:
        """
        ...
        detection_accuracy = self._calculate_detection_accuracy(envelope, optimization_result.x)
        false_positive_rate = self._calculate_false_positive_rate(envelope, optimization_result.x)
    ...
    def _create_threshold_objective_function(self, envelope: np.ndarray, results: Dict[str, Any]):
        ...
        def threshold_objective(x):
            ...
            objective_value = self._calculate_threshold_objective_value(envelope, x, results)
    ...
    def _calculate_detection_accuracy(self, envelope: np.ndarray, thresholds: np.ndarray) -> float:
        """
        Calculate detection accuracy.
        ...
        phase_coherence_detected = np.std(envelope_flat) / np.mean(envelope_flat) > phase_coherence_threshold
    ...
    def _calculate_false_positive_rate(self, envelope: np.ndarray, thresholds: np.ndarray) -> float:
        """
        Calculate false positive rate.
        ...
        false_positive_rate = 0.1 * (1.0 / (1.0 + interference_threshold * coupling_threshold * phase_coherence_threshold))
