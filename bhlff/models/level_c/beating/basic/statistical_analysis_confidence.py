...
class StatisticalConfidenceAnalyzer:
    ...
    def analyze_confidence_intervals(self, envelope: np.ndarray) -> Dict[str, Any]:
        ...
        overall_confidence = self._calculate_overall_confidence(pattern_confidence, correlation_analysis)
    ...
    def _calculate_pattern_confidence(self, envelope: np.ndarray) -> float:
        ...
        pattern_regularity = 1.0 / (1.0 + signal_std / signal_mean) if signal_mean > 0 else 0
    ...
    def _calculate_overall_confidence(
        ...
        amplitude_phase_corr = correlation_analysis.get("amplitude_phase_correlation", 0.0)
        spatial_temporal_corr = correlation_analysis.get("spatial_temporal_correlation", 0.0)
        mode_corr = correlation_analysis.get("mode_correlation", 0.0)
        
        # Calculate overall confidence
        correlation_confidence = np.mean([amplitude_phase_corr, spatial_temporal_corr, mode_corr])
