...
class BeatingMLPredictionFeatures:
        ...
        for i in range(1, envelope.shape[0] - 1):
            for j in range(1, envelope.shape[1] - 1):
                for k in range(1, envelope.shape[2] - 1):
                    laplacian[i, j, k] = (
                        envelope[i+1, j, k] - 2*envelope[i, j, k] + envelope[i-1, j, k] +
                        envelope[i, j+1, k] - 2*envelope[i, j, k] + envelope[i, j-1, k] +
                        envelope[i, j, k+1] - 2*envelope[i, j, k] + envelope[i, j, k-1]
                    )
    ...
    def _calculate_spatial_correlation(self, envelope: np.ndarray) -> float:
        ...
        correlation = np.corrcoef(envelope[0, :, :].flatten(), envelope[-1, :, :].flatten())[0, 1]
