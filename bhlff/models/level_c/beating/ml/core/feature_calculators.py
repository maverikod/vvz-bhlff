...
class FeatureCalculator:
    ...
    def calculate_frequency_coupling_strength(self, envelope: np.ndarray) -> float:
        ...
        mode1_power = np.real(mode1_power) if np.isscalar(mode1_power) else np.real(mode1_power[0])
            ...
            for j in range(power_spectrum.shape[1]):
                if power_spectrum.ndim > 2:
                    for k in range(power_spectrum.shape[2]):
                        ...
                        if (0 <= sym_i < power_spectrum.shape[0] and
                            0 <= sym_j < power_spectrum.shape[1] and
                            0 <= sym_k < power_spectrum.shape[2]):
                            symmetry += abs(power_spectrum[i, j, k] - 
                    ...
                    if (0 <= sym_i < power_spectrum.shape[0] and 
                       0 <= sym_j < power_spectrum.shape[1]):
                        symmetry += abs(power_spectrum[i, j] - 
                                      power_spectrum[sym_i, sym_j])
