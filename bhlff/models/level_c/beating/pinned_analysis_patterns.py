...
class PinnedPatternAnalyzer:
    ...
    def _analyze_modified_spatial_patterns(
        ...
        pattern_size = np.mean([np.sum(field > 0.5 * np.max(field)) for field in time_evolution])
        pattern_regularity = np.std([np.sum(field > 0.5 * np.max(field)) for field in time_evolution])
        
        # Apply pinning effects
        modified_pattern_size = pattern_size * (1.0 + pinning_strength * 0.2)
        modified_pattern_regularity = pattern_regularity * (1.0 - pinning_strength * 0.1)
