...
class BeatingValidationFrequencies:
    ...
    def validate_beating_frequencies_physical(self, frequencies: List[float]) -> Dict[str, Any]:
        ...
        for i, freq in enumerate(frequencies):
            ...
            if not self._is_within_theoretical_bounds(freq):
                theoretical_errors.append(f"Frequency {freq} outside theoretical bounds at index {i}")
        ...
        validation_result['physical_validation'] = {
            ...
            'physical_frequency_count': len([f for f in frequencies if self._is_physically_valid_frequency(f)])
        }
        ...
        validation_result['theoretical_validation'] = {
            ...
            'theoretical_frequency_count': len([f for f in frequencies if self._is_within_theoretical_bounds(f)])
        }
        ...
        if frequencies:
            validation_result['frequency_metrics'] = {
                ...
                'physical_frequency_ratio': validation_result['physical_validation']['physical_frequency_count'] / len(frequencies),
                'theoretical_frequency_ratio': validation_result['theoretical_validation']['theoretical_frequency_count'] / len(frequencies)
            }
        ...
        self.logger.info(f"Physical beating frequency validation completed: {'PASSED' if validation_result['frequencies_valid'] else 'FAILED'}")
        return validation_result
    ...
    def _is_physically_valid_frequency(self, frequency: float) -> bool:
        ...
        """
        return (self.min_physical_frequency <= frequency <= self.max_physical_frequency and
                not np.isnan(frequency) and not np.isinf(frequency))
    ...
    def _is_within_theoretical_bounds(self, frequency: float) -> bool:
        ...
        for bound_type, (min_freq, max_freq) in self.theoretical_frequency_bounds.items():
        ...
        for i, freq1 in enumerate(frequencies):
            for j, freq2 in enumerate(frequencies[i+1:], i+1):
                ...
                for harmonic in [2, 3, 4, 5]:
                    if abs(ratio - harmonic) < tolerance or abs(inverse_ratio - harmonic) < tolerance:
    ...
    def validate_beating_frequencies(self, frequencies: List[float]) -> Dict[str, Any]:
        ...
        self.logger.info(f"Basic beating frequency validation completed: {'PASSED' if validation_result['frequencies_valid'] else 'FAILED'}")
        return validation_result
