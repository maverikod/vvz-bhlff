...
class AdmittanceAnalysisCore:
    ...
    def analyze_admittance_spectrum(
        ...
        for i, omega in enumerate(frequencies):
            admittances[i] = self._compute_admittance_at_frequency(domain, boundary, omega)
    ...
    def _compute_admittance_at_frequency(
        ...
        solution_field = self._solve_bvp_with_boundary(domain, boundary, source_field, omega)
    ...
    def _create_source_field(self, domain: Dict[str, Any], omega: float) -> np.ndarray:
        ...
        source_field = np.exp(1j * omega * X / L) * np.exp(-(X - L/2)**2 / (L/4)**2)
        
        return source_field
    
    def _solve_bvp_with_boundary(
        self, domain: Dict[str, Any], boundary: BoundaryGeometry, source_field: np.ndarray, omega: float
    ) -> np.ndarray:
        """
        Solve BVP with boundary conditions.
        ...
        source_with_boundary = self._apply_boundary_conditions(source_field, boundary_mask)
    ...
    def _apply_boundary_conditions(self, source_field: np.ndarray, boundary_mask: np.ndarray) -> np.ndarray:
        """
        Apply boundary conditions.
    ...
    def _create_boundary_mask(self, domain: Dict[str, Any], boundary: BoundaryGeometry) -> np.ndarray:
        """
    ...
    def _compute_current(self, solution_field: np.ndarray, domain: Dict[str, Any], boundary: BoundaryGeometry) -> complex:
        """
        Compute current.
