...
class RadialConcentrationAnalyzer:
    ...
    def analyze_field_concentration(
        ...
        near_boundary_concentration = self._analyze_near_boundary_concentration(domain, boundary, field)
        
        # Analyze far-boundary concentration
        far_boundary_concentration = self._analyze_far_boundary_concentration(domain, boundary, field)
        
        # Analyze overall concentration pattern
        overall_concentration = self._analyze_overall_concentration_pattern(domain, boundary, field)
    ...
    def _analyze_near_boundary_concentration(
        ...
        concentration_metrics = {
            ...
            "concentration_ratio": np.mean(np.abs(near_boundary_field)) / np.mean(np.abs(field)),
        }
    ...
    def _analyze_far_boundary_concentration(
        ...
        concentration_metrics = {
            ...
            "concentration_ratio": np.mean(np.abs(far_boundary_field)) / np.mean(np.abs(field)),
        }
    ...
    def _create_near_boundary_mask(self, domain: Dict[str, Any], boundary_thickness: float) -> np.ndarray:
        """
        Create near-boundary mask.
        ...
        # Create near-boundary mask
        near_boundary_mask = (
            (X <= boundary_thickness) | (X >= L - boundary_thickness) |
            (Y <= boundary_thickness) | (Y >= L - boundary_thickness) |
            (Z <= boundary_thickness) | (Z >= L - boundary_thickness)
        )
        
        return near_boundary_mask
    
    def _create_far_boundary_mask(self, domain: Dict[str, Any], boundary_thickness: float) -> np.ndarray:
        """
        Create far-boundary mask.
