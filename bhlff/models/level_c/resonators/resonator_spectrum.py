...
class ResonatorSpectrumAnalyzer:
    ...
    def analyze_resonance_quality(self, envelope: np.ndarray) -> Dict[str, Any]:
        ...
        for peak in resonance_peaks:
            q_factor = self._calculate_quality_factor(peak, spectrum_analysis['resonance_spectrum'])
            quality_factors.append(q_factor)
        
        # Calculate resonance stability
        stability_metrics = self._calculate_resonance_stability(envelope, resonance_peaks)
    ...
    def _calculate_resonance_characteristics(self, power_spectrum: np.ndarray) -> np.ndarray:
        ...
        for i in range(1, power_spectrum.shape[0] - 1):
            for j in range(1, power_spectrum.shape[1] - 1):
                if (power_spectrum[i, j] > power_spectrum[i-1, j] and
                    power_spectrum[i, j] > power_spectrum[i+1, j] and
                    power_spectrum[i, j] > power_spectrum[i, j-1] and
                    power_spectrum[i, j] > power_spectrum[i, j+1]):
    ...
    def _calculate_quality_factor(self, peak: Dict[str, Any], spectrum: np.ndarray) -> float:
    ...
    def _calculate_resonance_stability(self, envelope: np.ndarray, peaks: List[Dict[str, Any]]) -> Dict[str, float]:
        ...
        amplitude_variation = np.std(amplitudes) / np.mean(amplitudes) if np.mean(amplitudes) > 0 else 0.0
