...
class DefectDynamics:
    ...
    def simulate_defect_motion(
        ...
        Physical Meaning:
            Computes the trajectory of a topological defect under
            the influence of effective forces, gyroscopic effects,
            and dissipation.

        Mathematical Foundation:
            Integrates the Thiele equation: ẋ = -∇U_eff + G × ẋ + D ẋ
            using a numerical scheme to obtain the defect trajectory.

        Args:
            initial_position: Starting position of defect
            time_steps: Number of time steps
            field: Background field configuration

        Returns:
            Dictionary containing position, velocity, and forces
        """
        # Initialize trajectory arrays
        positions = np.zeros((time_steps, 3))
        velocities = np.zeros((time_steps, 3))
        forces = np.zeros((time_steps, 3))
        ...
        for step in range(1, time_steps):
            # Find current defect position
            current_position = self._find_defect_position(field)
            positions[step] = current_position

            # Compute forces
            effective_force = self._compute_defect_force(current_position, field)
    ...
    def _get_defect_mass(self) -> float:
        ...
        Physical Meaning:
            Returns the effective mass of the topological defect,
            which determines its inertia and response to forces.

        Returns:
            Defect mass
        """
        return self.defect_mass
