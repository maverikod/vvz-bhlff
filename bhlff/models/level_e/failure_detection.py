...
class FailureDetector:
    ...
    def _check_passivity_violation(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'max_violation': max([v['violation_magnitude'] for v in violations]) if violations else 0.0
        }
    ...
    def _check_singular_mode(self) -> Dict[str, Any]:
        ...
        for mode_id, mode_info in mode_data.items():
            ...
            if abs(lambda_val) < 1e-10 and abs(source_val) > 1e-10:
                singular_modes.append({
                    ...
                    'lambda': lambda_val,
                    'source': source_val,
                    'singularity_strength': abs(source_val) / abs(lambda_val) if lambda_val != 0 else float('inf')
                })
        ...
        return {
            ...
            'max_singularity': max([m['singularity_strength'] for m in singular_modes]) if singular_modes else 0.0
        }
    ...
    def _check_energy_conservation(self) -> Dict[str, Any]:
        ...
        for time, energy in energy_data.items():
            if time > 0:
                ...
                relative_change = energy_change / abs(energy_data[0]) if energy_data[0] != 0 else 0
        ...
        return {
            ...
            'violations': violations,
            'count': len(violations),
            'max_violation': max([v['relative_change'] for v in violations]) if violations else 0.0
        }
    ...
    def _check_topological_charge(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'violations': violations,
            'count': len(violations),
            'max_deviation': max([v['deviation'] for v in violations]) if violations else 0.0
        }
    ...
    def _assess_overall_failures(self, failures: Dict[str, Any]) -> Dict[str, Any]:
        ...
        detected_failures = [name for name, result in failures.items() 
    ...
    def analyze_failure_boundaries(self, parameter_ranges: Dict[str, Tuple[float, float]]) -> Dict[str, Any]:
        """
        ...
        for param_name, (min_val, max_val) in parameter_ranges.items():
            ...
            for value in test_values:
                ...
                has_failures = any(result.get('detected', False) 
