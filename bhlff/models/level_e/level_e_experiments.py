...
class LevelEExperiments:
    ...
    def _setup_analyzers(self) -> None:
        ...
        self.phase_mapper = PhaseMapper(
            self.config.get('phase_mapping_config', {})
        )
    ...
    def _run_sensitivity_analysis(self) -> Dict[str, Any]:
        ...
        try:
            ...
            sensitivity_results = self.sensitivity_analyzer.analyze_parameter_sensitivity(n_samples)
            
            # Analyze mass-complexity correlation
            mass_complexity_results = self.sensitivity_analyzer.analyze_mass_complexity_correlation(
                sensitivity_results['samples'],
                sensitivity_results['outputs']
            )
    ...
    def _run_robustness_testing(self) -> Dict[str, Any]:
        ...
        try:
            ...
            noise_levels = robustness_params.get('noise_levels', [0.0, 0.05, 0.1, 0.15, 0.2])
            ...
            param_results = self.robustness_tester.test_parameter_uncertainty(parameter_uncertainty)
            
            # Run geometry perturbation tests
            geometry_results = self.robustness_tester.test_geometry_perturbations(geometry_perturbations)
    ...
    def _run_discretization_analysis(self) -> Dict[str, Any]:
        ...
        try:
            ...
            domain_sizes = discretization_params.get('domain_sizes', [10.0, 15.0, 20.0, 25.0, 30.0])
            time_steps = discretization_params.get('time_steps', [0.001, 0.005, 0.01, 0.02, 0.05])
            
            # Run grid convergence analysis
            grid_results = self.discretization_analyzer.analyze_grid_convergence(grid_sizes)
            
            # Run domain size analysis
            domain_results = self.discretization_analyzer.analyze_domain_size_effects(domain_sizes)
            
            # Run time step analysis
            time_results = self.discretization_analyzer.analyze_time_step_stability(time_steps)
    ...
    def _run_failure_detection(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if parameter_ranges:
                boundary_results = self.failure_detector.analyze_failure_boundaries(parameter_ranges)
    ...
    def _run_phase_mapping(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if resonance_data:
                resonance_classification = self.phase_mapper.classify_resonances(resonance_data)
                phase_mapping_results['resonance_classification'] = resonance_classification
            
            # Combine results
            results = {
                'phase_mapping': phase_mapping_results,
                'status': 'completed'
            }
    ...
    def _assess_overall_results(self, results: Dict[str, Any]) -> Dict[str, Any]:
        ...
        success_rate = len(completed_experiments) / total_experiments if total_experiments > 0 else 0
    ...
    def _generate_recommendations(self, results: Dict[str, Any]) -> List[str]:
        ...
        if 'E1_sensitivity' in results and results['E1_sensitivity'].get('status') == 'completed':
            sensitivity_results = results['E1_sensitivity'].get('sensitivity_analysis', {})
            stability_metrics = sensitivity_results.get('stability_metrics', {})
            
            if not stability_metrics.get('is_converged', False):
                recommendations.append("Sensitivity analysis did not converge - consider increasing sample size")
            
            if stability_metrics.get('dominance_ratio', 1) > 10:
                recommendations.append("High parameter dominance detected - consider parameter scaling")
        
        # Check robustness results
        if 'E2_robustness' in results and results['E2_robustness'].get('status') == 'completed':
            robustness_results = results['E2_robustness']
            
            for noise_level, noise_results in robustness_results.get('noise_robustness', {}).items():
                if noise_results.get('degradation', {}).get('degradation', 0) > 0.5:
                    recommendations.append(f"High degradation at noise level {noise_level} - consider noise reduction")
        
        # Check discretization results
        if 'E3_discretization' in results and results['E3_discretization'].get('status') == 'completed':
            ...
            if convergence_analysis.get('overall_convergence', {}).get('overall_rate', 0) < 1.0:
                recommendations.append("Poor convergence detected - consider grid refinement")
        
        # Check failure detection results
        if 'E4_failures' in results and results['E4_failures'].get('status') == 'completed':
            ...
            if overall_assessment.get('status') == 'critical':
                recommendations.append("Critical failures detected - review system parameters")
        
        # Check performance results
        if 'E6_performance' in results and results['E6_performance'].get('status') == 'completed':
            performance_results = results['E6_performance'].get('performance_analysis', {})
            ...
            if overall_scaling.get('overall_assessment') == 'poor':
                recommendations.append("Poor scaling performance - consider algorithm optimization")
