...
class BaryonSoliton(SolitonModel):
    ...
    def _setup_fr_constraints(self) -> None:
        ...
        self.fr_rotation_axis = self.fr_rotation_axis / np.linalg.norm(self.fr_rotation_axis)
    ...
    def apply_fr_constraints(self, field: np.ndarray) -> np.ndarray:
        ...
        constrained_field = (1 - self.fr_constraint_strength) * field + \
                          self.fr_constraint_strength * rotated_field
        ...
        for i in range(field.shape[0]):
            for j in range(field.shape[1]):
                for k in range(field.shape[2]):
                    ...
                    if (0 <= rotated_pos[0] < field.shape[0] and
                        0 <= rotated_pos[1] < field.shape[1] and
                        0 <= rotated_pos[2] < field.shape[2]):
                        rotated_field[i, j, k] = self._interpolate_field(field, rotated_pos)
