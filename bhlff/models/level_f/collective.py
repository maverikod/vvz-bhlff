...
class CollectiveExcitations(AbstractModel):
    ...
    def compute_susceptibility(self, frequencies: np.ndarray) -> np.ndarray:
        ...
        for i, (omega_n, A_n) in enumerate(zip(mode_frequencies, mode_amplitudes)):
            ...
            susceptibility += A_n / (omega_n**2 - frequencies**2 - 1j * gamma * frequencies)
    ...
    def _apply_excitation(self, external_field: np.ndarray, 
    ...
    def _compute_external_force(self, external_field: np.ndarray,
                               excitation_amplitude: float) -> np.ndarray:
        """
    ...
    def _find_spectral_peaks(self, spectrum: np.ndarray, 
        ...
        for i in range(1, len(spectrum) - 1):
            if (spectrum[i] > spectrum[i-1] and 
                ...
                spectrum[i] > self.peak_threshold):
    ...
    def _compute_quality_factors(self, peaks: Dict[str, Any],
                               damping_analysis: Dict[str, Any]) -> np.ndarray:
        """
    ...
    def _fit_dispersion_relation(self, k_values: np.ndarray,
                               frequencies: np.ndarray) -> Dict[str, Any]:
        """
        ...
        r_squared = 1 - np.sum((frequencies - omega_fit)**2) / np.sum((frequencies - np.mean(frequencies))**2)
