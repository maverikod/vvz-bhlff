...
class CollectiveModesSpectrumAnalyzer:
    ...
    def _calculate_spectral_density(self, frequencies: np.ndarray) -> float:
        ...
        if len(frequencies) > 0:
            frequency_range = np.max(frequencies) - np.min(frequencies)
            spectral_density = len(frequencies) / frequency_range if frequency_range > 0 else 0.0
        else:
    ...
    def _calculate_spectral_skewness(self, frequencies: np.ndarray) -> float:
        ...
        if len(frequencies) > 0:
            ...
            skewness = np.mean(((frequencies - mean_freq) / std_freq) ** 3) if std_freq > 0 else 0.0
        else:
            skewness = 0.0
    ...
    def _calculate_spectral_kurtosis(self, frequencies: np.ndarray) -> float:
        ...
        if len(frequencies) > 0:
            mean_freq = np.mean(frequencies)
            std_freq = np.std(frequencies)
            kurtosis = np.mean(((frequencies - mean_freq) / std_freq) ** 4) if std_freq > 0 else 0.0
        else:
            kurtosis = 0.0
    ...
    def _find_spectral_gaps(self, frequencies: np.ndarray) -> List[Dict[str, Any]]:
        ...
        if len(frequencies) > 1:
            ...
            gap_threshold = np.mean(frequency_differences) + 2 * np.std(frequency_differences)
    ...
    def _find_spectral_clusters(self, frequencies: np.ndarray) -> List[Dict[str, Any]]:
        ...
        if len(frequencies) > 1:
            ...
            cluster_threshold = np.mean(frequency_differences) - np.std(frequency_differences)
    ...
    def _analyze_mode_spacing(self, frequencies: np.ndarray) -> Dict[str, Any]:
        ...
        if len(frequencies) > 1:
            ...
            spacing_analysis = {
                ...
                "spacing_regularity": 1.0 / (1.0 + np.std(spacing) / np.mean(spacing)) if np.mean(spacing) > 0 else 0.0,
            }
        else:
            spacing_analysis = {
                "mean_spacing": 0.0,
