...
class PotentialLandscapeAnalyzer:
        ...
        for di in [-1, 0, 1]:
            for dj in [-1, 0, 1]:
                for dk in [-1, 0, 1]:
                    ...
                    if (0 <= ni < potential.shape[0] and 
                        0 <= nj < potential.shape[1] and 
                        0 <= nk < potential.shape[2]):
    ...
    def _analyze_potential_barriers(self, potential: np.ndarray) -> Dict[str, Any]:
        ...
        barrier_statistics = {
            ...
            "average_barrier_height": np.mean([barrier["height"] for barrier in barriers]) if barriers else 0.0,
            "max_barrier_height": max([barrier["height"] for barrier in barriers]) if barriers else 0.0,
        }
    ...
    def _is_potential_barrier(self, potential: np.ndarray, i: int, j: int, k: int) -> bool:
        """
    ...
    def _analyze_potential_wells(self, potential: np.ndarray) -> Dict[str, Any]:
        ...
        well_statistics = {
            "num_wells": len(wells),
            "average_well_depth": np.mean([well["depth"] for well in wells]) if wells else 0.0,
