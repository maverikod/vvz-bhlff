...
class NonlinearEffects(AbstractModel):
    ...
    def add_nonlinear_interactions(self, 
        ...
        """
        # Update parameters
        self.nonlinear_strength = nonlinear_params.get('strength', self.nonlinear_strength)
    ...
    def _setup_cubic_nonlinearity(self) -> None:
        ...
        self.nonlinear_derivative = lambda psi: 3 * self.nonlinear_strength * np.abs(psi) * np.sign(psi)
    ...
    def _setup_quartic_nonlinearity(self) -> None:
        ...
        self.nonlinear_derivative = lambda psi: 4 * self.nonlinear_strength * np.abs(psi)**2 * np.sign(psi)
    ...
    def _compute_nonlinear_corrections(self, linear_modes: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for amp in linear_amplitudes:
            ...
            correction = self.nonlinear_strength * np.mean(amp)**(self.nonlinear_order - 1)
    ...
    def _analyze_nonlinear_stability(self) -> Dict[str, Any]:
        ...
        nonlinear_stable = (system_stability['is_stable'] and 
                          self.nonlinear_strength < 2.0)
    ...
    def _analyze_soliton_properties(self, solitons: List[Dict[str, Any]]) -> Dict[str, Any]:
        """
