...
class SolitonModeAnalyzer(SolitonAnalysisBase):
    ...
    def compute_full_mode_analysis(self, amp1: float, width1: float, pos1: float, amp2: float, width2: float, pos2: float, amp3: float, width3: float, pos3: float) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            for i in range(N):
                for j in range(N):
                    ...
                    interaction_12 = self.interaction_strength * amp1 * amp2 * self._step_resonator_interaction(abs(x[i] - x[j]), width1 + width2)
                    interaction_13 = self.interaction_strength * amp1 * amp3 * self._step_resonator_interaction(abs(x[i] - x[j]), width1 + width3)
                    interaction_23 = self.interaction_strength * amp2 * amp3 * self._step_resonator_interaction(abs(x[i] - x[j]), width2 + width3)
                    
                    # Three-body interaction
                    three_body = self.three_body_strength * amp1 * amp2 * amp3 * self._step_resonator_interaction(abs(x[i] - x[j]), width1 + width2 + width3)
        ...
        try:
            ...
            for i in range(N):
                for j in range(N):
                    if i == j:
                        # Diagonal term
                        kinetic_matrix[i, j] = self.mu * (np.abs(x[i]) ** (2 * self.beta))
                    else:
                        # Off-diagonal terms (simplified)
                        kinetic_matrix[i, j] = -self.mu * (np.abs(x[i] - x[j]) ** (2 * self.beta)) / (2 * N)
    ...
    def _compute_mode_participation_ratios(self, eigenvectors: np.ndarray, profile1: np.ndarray, profile2: np.ndarray, profile3: np.ndarray) -> Dict[str, float]:
        """Compute mode participation ratios."""
        try:
            participation_ratios = {}
