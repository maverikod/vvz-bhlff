...
Multi-soliton analysis and optimization.

This module implements multi-soliton solution finding and analysis
using full 7D BVP theory with soliton-soliton interactions.

Physical Meaning:
    Finds and analyzes multi-soliton solutions including two and three
...
class MultiSolitonSolver(SolitonAnalysisBase):
    """
    Multi-soliton solution finder and analyzer.
        ...
        try:
            ...
            def two_soliton_equations_7d(params):
                ...
                def two_soliton_ode(x, y):
                    ...
                    return self._compute_7d_two_soliton_ode(x, y, amp1, width1, pos1, amp2, width2, pos2)
                ...
                try:
                    ...
                    if sol.success:
                        ...
                        energy = self._compute_two_soliton_energy(sol.y, amp1, width1, pos1, amp2, width2, pos2)
        ...
        try:
            ...
            def three_soliton_equations_7d(params):
                ...
                def three_soliton_ode(x, y):
                    ...
                    return self._compute_7d_three_soliton_ode(x, y, amp1, width1, pos1, amp2, width2, pos2, amp3, width3, pos3)
                ...
                try:
                    ...
                    if sol.success:
                        ...
                        energy = self._compute_three_soliton_energy(
    ...
    def _compute_7d_two_soliton_ode(self, x: np.ndarray, y: np.ndarray, amp1: float, width1: float, pos1: float, amp2: float, width2: float, pos2: float) -> np.ndarray:
        ...
        try:
            ...
            interaction_potential = self.interaction_strength * field * (source1 * source2)
            
            # ODE system with interactions
            dydx = np.array([
                field_deriv,  # dy[0]/dx = y[1]
                -fractional_laplacian - self.lambda_param * field - interaction_potential + total_source
            ])
    ...
    def _compute_7d_three_soliton_ode(self, x: np.ndarray, y: np.ndarray, amp1: float, width1: float, pos1: float, amp2: float, width2: float, pos2: float, amp3: float, width3: float, pos3: float) -> np.ndarray:
        ...
        try:
            ...
            three_body = self.three_body_strength * field * (source1 * source2 * source3)
            ...
            dydx = np.array([
                field_deriv,  # dy[0]/dx = y[1]
                -fractional_laplacian - self.lambda_param * field - total_interaction + total_source
            ])
    ...
    def _compute_two_soliton_energy(self, solution: np.ndarray, amp1: float, width1: float, pos1: float, amp2: float, width2: float, pos2: float) -> float:
        ...
        try:
            ...
            interaction_energy = self.interaction_strength * amp1 * amp2 * np.exp(-distance / (width1 + width2))
    ...
    def _compute_three_soliton_energy(self, solution: np.ndarray, amp1: float, width1: float, pos1: float, amp2: float, width2: float, pos2: float, amp3: float, width3: float, pos3: float) -> float:
        ...
        try:
            ...
            interaction_12 = self.interaction_strength * amp1 * amp2 * np.exp(-distance_12 / (width1 + width2))
            interaction_13 = self.interaction_strength * amp1 * amp3 * np.exp(-distance_13 / (width1 + width3))
            interaction_23 = self.interaction_strength * amp2 * amp3 * np.exp(-distance_23 / (width2 + width3))
            
            # Three-body interaction energy
            three_body_energy = self.three_body_strength * amp1 * amp2 * amp3 * np.exp(-(distance_12 + distance_13 + distance_23) / (width1 + width2 + width3))
