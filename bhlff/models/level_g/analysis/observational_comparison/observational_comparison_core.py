...
class ObservationalComparisonCore:
    ...
    def __init__(self, evolution_results: Dict[str, Any], observational_data: Dict[str, Any] = None, analysis_parameters: Dict[str, Any] = None):
        ...
        self._data_loader = ObservationalDataLoader(observational_data, analysis_parameters)
        self._observable_extractor = ObservableExtractor(evolution_results, analysis_parameters)
    ...
    def compare_with_observations(self) -> Dict[str, Any]:
        ...
        comparison_results = self._statistical_comparison.perform_comparison(obs_data, model_observables)
        
        # Compute chi-squared
        chi_squared = self._statistical_comparison.compute_chi_squared(obs_data, model_observables)
        ...
        comparison = {
            "structure_formation_comparison": self._compare_structure_formation(obs_data, model_observables),
            "parameter_comparison": self._compare_parameters(obs_data, model_observables),
            "statistical_comparison": comparison_results,
            "goodness_of_fit": self._statistical_comparison.compute_goodness_of_fit(obs_data, model_observables),
    ...
    def _compare_structure_formation(self, obs_data: Dict[str, Any], model_observables: Dict[str, Any]) -> Dict[str, Any]:
        """
    ...
    def _compare_parameters(self, obs_data: Dict[str, Any], model_observables: Dict[str, Any]) -> Dict[str, Any]:
        """
    ...
    def _compare_correlation_functions(self, obs_corr: np.ndarray, model_corr: np.ndarray) -> Dict[str, Any]:
        """Compare correlation functions."""
    ...
    def _compare_power_spectra(self, obs_power: np.ndarray, model_power: np.ndarray) -> Dict[str, Any]:
        """Compare power spectra."""
    ...
    def _compare_structure_statistics(self, obs_stats: Dict[str, Any], model_stats: Dict[str, Any]) -> Dict[str, Any]:
        """Compare structure statistics."""
        comparison = {}
            ...
            if key in obs_stats and key in model_stats:
                ...
                if isinstance(obs_value, (int, float)) and isinstance(model_value, (int, float)):
                    comparison[key] = {
                        ...
                        "relative_difference": abs(model_value - obs_value) / abs(obs_value) if obs_value != 0 else 0.0
                    }
        
        return comparison

    def _compare_parameter(self, obs_value: float, model_value: float) -> Dict[str, Any]:
        ...
        return {
            ...
            "difference": abs(model_value - obs_value),
            "relative_difference": abs(model_value - obs_value) / abs(obs_value) if obs_value != 0 else 0.0,
            "agreement": "good" if abs(model_value - obs_value) / abs(obs_value) < 0.1 else "fair"
        }
