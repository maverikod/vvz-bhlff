...
class ObservationalComparisonStatistics7D:
    ...
    def compute_7d_correlation_function(self) -> np.ndarray:
        ...
        if phase_field.ndim == 3:
            ...
            correlation = np.correlate(density_contrast.flatten(), density_contrast.flatten(), mode='full')
    ...
    def compute_7d_power_spectrum(self) -> np.ndarray:
        ...
        if power_spectrum.ndim > 1:
            ...
            ky = np.fft.fftfreq(power_spectrum.shape[1]) if power_spectrum.ndim > 1 else np.array([0])
            kz = np.fft.fftfreq(power_spectrum.shape[2]) if power_spectrum.ndim > 2 else np.array([0])
    ...
    def compute_7d_structure_statistics(self) -> Dict[str, Any]:
        ...
        statistics = {
            ...
            'density_contrast_range': [np.min(density_contrast), np.max(density_contrast)]
        }
