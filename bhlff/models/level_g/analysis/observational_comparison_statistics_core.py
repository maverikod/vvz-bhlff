...
class ObservationalComparisonStatisticsCore:
    ...
    def compare_statistics(self) -> Dict[str, Any]:
        ...
        observational_correlation = self.observational_data.get("correlation_function", np.array([]))
        ...
        observational_power_spectrum = self.observational_data.get("power_spectrum", np.array([]))
        
        # Compute theoretical structure statistics
        theoretical_structure_stats = self._stats_7d.compute_7d_structure_statistics()
        observational_structure_stats = self.observational_data.get("structure_statistics", {})
        
        # Compute agreements
        correlation_tolerance = self.analysis_parameters.get("correlation_tolerance", 0.1)
        ...
        power_spectrum_tolerance = self.analysis_parameters.get("power_spectrum_tolerance", 0.1)
        power_spectrum_agreement = self._compare_power_spectra(
            theoretical_power_spectrum, observational_power_spectrum, power_spectrum_tolerance)
        ...
        structure_statistics_agreement = self._compare_structure_statistics(
            theoretical_structure_stats, observational_structure_stats, structure_tolerance)
    ...
    def compute_chi_squared(self, obs_data: Dict[str, Any], model_observables: Dict[str, Any]) -> float:
        ...
        for key in obs_data:
            if key in model_observables:
                ...
                elif isinstance(obs_value, np.ndarray) and isinstance(model_value, np.ndarray):
                    # Chi-squared for arrays
                    if len(obs_value) == len(model_value):
                        errors = self.analysis_parameters.get(f"{key}_errors", np.ones_like(obs_value))
    ...
    def _compare_correlation_functions(self, theoretical: np.ndarray, observational: np.ndarray, tolerance: float) -> bool:
    ...
    def _compare_power_spectra(self, theoretical: np.ndarray, observational: np.ndarray, tolerance: float) -> bool:
    ...
    def _compare_structure_statistics(self, theoretical: Dict[str, Any], observational: Dict[str, Any], tolerance: float) -> bool:
        """Compare structure statistics."""
        if not theoretical or not observational:
            return True
        for key in theoretical:
            if key in observational:
                if isinstance(theoretical[key], (int, float)) and isinstance(observational[key], (int, float)):
    ...
    def _compute_aic_from_7d_field(self, chi_squared: float, n_parameters: int) -> float:
        """Compute AIC from 7D phase field."""
        return chi_squared + 2 * n_parameters

    def _compute_bic_from_7d_field(self, chi_squared: float, n_parameters: int, n_data_points: int) -> float:
