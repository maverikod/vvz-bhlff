...
class GravitationalEffectsModel(ModelBase):
    ...
    def _setup_gravitational_parameters(self) -> None:
        ...
        self.coordinate_system = self.gravity_params.get(
    ...
    def _compute_curvature_tensor(self) -> np.ndarray:
        ...
        if self.metric is None:
            return np.zeros(
                (4, 4, 4, 4, self.resolution, self.resolution, self.resolution)
            )
    ...
    def _compute_ricci_tensor(self, curvature: np.ndarray) -> np.ndarray:
        ...
        ricci_tensor = np.zeros(
            (4, 4, self.resolution, self.resolution, self.resolution)
        )
    ...
    def _compute_curvature_invariants(
        ...
        # Compute curvature invariants
        invariants = {
            "kretschmann_scalar": np.zeros(
                (self.resolution, self.resolution, self.resolution)
            ),
            "euler_density": np.zeros(
                (self.resolution, self.resolution, self.resolution)
            ),
            "chern_simons_density": np.zeros(
                (self.resolution, self.resolution, self.resolution)
            ),
        }
    ...
    def _compute_polarization(self) -> Dict[str, np.ndarray]:
        ...
        polarization = {
            "plus_polarization": np.zeros(
                (self.resolution, self.resolution, self.resolution)
            ),
            "cross_polarization": np.zeros(
                (self.resolution, self.resolution, self.resolution)
            ),
        }
