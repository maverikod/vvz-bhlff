...
class ParticleInversion(ModelBase):
    ...
    def __init__(self, observables: Dict[str, Any], 
                 priors: Dict[str, Any],
    ...
    def _setup_inversion_parameters(self) -> None:
        ...
        self.regularization_strength = self.optimization_params.get('regularization_strength', 0.01)
    ...
    def invert_parameters(self) -> Dict[str, Any]:
        ...
        self.inversion_results = {
            ...
            'parameter_uncertainties': self._compute_parameter_uncertainties(optimized_params)
        }
        ...
        for param_name, prior_range in self.priors.items():
            if isinstance(prior_range, list) and len(prior_range) == 2:
                ...
                if prior_range.get('type') == 'normal':
                    ...
                    # Default to uniform
                    min_val, max_val = prior_range.get('min', 0.0), prior_range.get('max', 1.0)
                    initial_params[param_name] = np.random.uniform(min_val, max_val)
            else:
                # Default value
    ...
    def _optimize_parameters(self, initial_params: Dict[str, float]) -> Dict[str, float]:
        """
        ...
        for iteration in range(self.max_iterations):
            ...
            for param_name in current_params:
                if param_name in gradients:
                    current_params[param_name] -= self.learning_rate * gradients[param_name]
    ...
    def _compute_loss(self, params: Dict[str, float]) -> float:
        ...
        for metric_name, weight in self.loss_weights.items():
            if metric_name in self.observables and metric_name in model_predictions:
                ...
                distance = self._compute_distance_metric(obs_value, mod_value, metric_name)
    ...
    def _compute_distance_metric(self, obs_value: float, mod_value: float, 
            ...
            if param_name in self.priors:
                ...
                if isinstance(prior_range, list) and len(prior_range) == 2:
                    ...
                    if param_value < min_val or param_value > max_val:
                        regularization += (param_value - min_val)**2 + (param_value - max_val)**2
    ...
    def _compute_parameter_uncertainties(self, params: Dict[str, float]) -> Dict[str, float]:
        """
...
class ParticleValidation(ModelBase):
    ...
    def __init__(self, inversion_results: Dict[str, Any],
    ...
    def _setup_validation_parameters(self) -> None:
        ...
        self.chi_squared_threshold = self.validation_criteria.get('chi_squared_threshold', 0.05)
        self.confidence_level = self.validation_criteria.get('confidence_level', 0.95)
        self.parameter_tolerance = self.validation_criteria.get('parameter_tolerance', 0.01)
        
        # Physical constraints
        self.energy_balance_tolerance = self.validation_criteria.get('energy_balance_tolerance', 0.03)
        self.passivity_threshold = self.validation_criteria.get('passivity_threshold', 0.0)
