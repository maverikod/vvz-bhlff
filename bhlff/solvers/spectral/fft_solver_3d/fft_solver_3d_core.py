...
3D FFT solver core facade for BVP envelope equations.

This module provides the main facade class for 3D FFT solving of BVP
envelope equations, coordinating all components.
...
class FFTSolver3D:
    """
    3D FFT solver facade for BVP envelope equations.
    ...
    Attributes:
        ...
        spectral_ops (SpectralOperations3D): 3D spectral operations.
        _spectral_coeffs (np.ndarray): Pre-computed spectral coefficients.
        boundary_handler (BoundaryHandler): Boundary conditions handler.
        bvp_integration (BVPIntegration): BVP integration handler.
    """
    ...
    def __init__(
        ...
        self.spectral_ops = SpectralOperations3D(domain, self.fft_backend)
        self._spectral_coeffs: np.ndarray

        # Initialize component handlers
        self.boundary_handler = BoundaryHandler(domain)
        self.bvp_integration = BVPIntegration(domain, bvp_core, config)
    ...
    def get_spectral_operations(self) -> SpectralOperations3D:
        """
        ...
        Returns:
            SpectralOperations3D: Spectral operations handler.
        """
    ...
    def solve_bvp_envelope(self, source: np.ndarray) -> np.ndarray:
        ...
        """
        return self.bvp_integration.solve_bvp_envelope(source)
    ...
    def detect_quenches(self, envelope: np.ndarray) -> Dict[str, Any]:
        ...
        """
        return self.bvp_integration.detect_quenches(envelope)
    ...
    def compute_bvp_impedance(self, envelope: np.ndarray) -> Dict[str, Any]:
        ...
        """
        return self.bvp_integration.compute_bvp_impedance(envelope)
    ...
    def get_bvp_core(self) -> Optional[BVPCore]:
        ...
        """
        return self.bvp_integration.get_bvp_core()
    ...
    def set_bvp_core(self, bvp_core: BVPCore) -> None:
        ...
        """
        self.bvp_integration.set_bvp_core(bvp_core)
