...
class MemoryMonitor:
    ...
    def get_gpu_memory_usage(self) -> Optional[Dict[str, float]]:
        ...
        try:
            ...
            return {
                ...
                'used_mb': (cp.cuda.Device().mem_info[1] - cp.cuda.Device().mem_info[0]) / (1024 * 1024),
    ...
    def get_memory_stats(self) -> Dict[str, Any]:
        ...
        if cpu_stats['used_mb'] > self.cpu_critical_threshold:
            ...
            stats['warnings'].append(f"CRITICAL: CPU memory usage {cpu_stats['used_mb']:.1f}MB exceeds threshold {self.cpu_critical_threshold}MB")
        elif cpu_stats['used_mb'] > self.cpu_warning_threshold:
            stats['warnings'] = stats.get('warnings', [])
            stats['warnings'].append(f"WARNING: CPU memory usage {cpu_stats['used_mb']:.1f}MB exceeds threshold {self.cpu_warning_threshold}MB")
        
        if stats['gpu']:
            gpu_stats = stats['gpu']
            if gpu_stats['used_mb'] > self.gpu_critical_threshold:
                stats['warnings'] = stats.get('warnings', [])
                stats['warnings'].append(f"CRITICAL: GPU memory usage {gpu_stats['used_mb']:.1f}MB exceeds threshold {self.gpu_critical_threshold}MB")
            elif gpu_stats['used_mb'] > self.gpu_warning_threshold:
                stats['warnings'] = stats.get('warnings', [])
                stats['warnings'].append(f"WARNING: GPU memory usage {gpu_stats['used_mb']:.1f}MB exceeds threshold {self.gpu_warning_threshold}MB")
    ...
    def set_memory_thresholds(self, cpu_warning: float = None, cpu_critical: float = None,
                              gpu_warning: float = None, gpu_critical: float = None) -> None:
        """
        ...
        self.logger.info(f"Memory thresholds updated: CPU warning={self.cpu_warning_threshold}MB, "
    ...
    def get_memory_recommendations(self) -> List[str]:
        ...
        if cpu_usage:
            ...
            if max_cpu > self.cpu_critical_threshold:
                recommendations.append(f"CRITICAL: Peak CPU memory usage {max_cpu:.1f}MB exceeds critical threshold")
            elif avg_cpu > self.cpu_warning_threshold:
                recommendations.append(f"WARNING: Average CPU memory usage {avg_cpu:.1f}MB is high")
            
            if max_cpu - min(cpu_usage) > 500:  # Large memory fluctuations
                recommendations.append("Consider implementing memory pooling to reduce fluctuations")
        
        # Analyze GPU memory trends
        gpu_usage = [entry['gpu']['used_mb'] for entry in self.memory_history if entry['gpu']]
        if gpu_usage:
            avg_gpu = sum(gpu_usage) / len(gpu_usage)
            max_gpu = max(gpu_usage)
            
            if max_gpu > self.gpu_critical_threshold:
                recommendations.append(f"CRITICAL: Peak GPU memory usage {max_gpu:.1f}MB exceeds critical threshold")
            elif avg_gpu > self.gpu_warning_threshold:
                recommendations.append(f"WARNING: Average GPU memory usage {avg_gpu:.1f}MB is high")
            
            if max_gpu - min(gpu_usage) > 200:  # Large GPU memory fluctuations
                recommendations.append("Consider using GPU memory pooling to reduce fluctuations")
...
@contextmanager
def memory_monitor_context(monitor: Optional[MemoryMonitor] = None, 
