...
class CodeMapper:
    ...
    def __init__(
        self, root_dir: str, output_file: str = "code_map.md", output_dir: str = None
    ):
        """
    ...
    def scan_directory(
        self, include_pattern: str = "*.py", exclude_pattern: str = None
    ) -> None:
        """
    ...
    def generate_report(self) -> str:
        ...
        # Статистика
        total_classes = sum(
            len(file_info.get("classes", [])) for file_info in self.code_map.values()
        )
        total_functions = sum(
            len(file_info.get("functions", [])) for file_info in self.code_map.values()
        )
        total_methods = sum(
            ...
            if file_info.get("classes"):
                ...
                for class_info in file_info["classes"]:
                    ...
                    if class_info.get("inheritance"):
                        report.append(
                            f"  - Наследование: {', '.join(class_info['inheritance'])}"
                        )
                    if class_info.get("docstring"):
                        report.append(
                            f"  - Описание: {class_info['docstring'][:100]}..."
                        )
                    report.append("")
                    ...
                    if class_info.get("methods"):
                        ...
                        for method in class_info["methods"]:
                            ...
                            report.append(
                                f"  - {private_mark}{abstract_mark}`{signature}`"
                            )
                            if docstring:
            ...
            if file_info.get("imports"):
                ...
                for imp in file_info["imports"][:5]:  # Показываем только первые 5
                    if imp["type"] == "import":
                        ...
                        main_imports.extend(
                            [f"{imp['module']}.{name}" for name in imp["names"]]
                        )
    ...
    def generate_method_index(self) -> str:
        ...
        index.append("")
        index.append(
            "Этот индекс содержит все методы проекта с указанием файлов и строк, где они определены."
        )
        ...
        for class_name in sorted(methods_by_class.keys()):
            ...
            for method_name in sorted(methods_by_class[class_name].keys()):
                ...
                # Заголовок метода
                private_mark = (
                    "🔒 " if any(m.get("is_private") for m in method_info) else ""
                )
                abstract_mark = (
                    "🔸 " if any(m.get("is_abstract") for m in method_info) else ""
                )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Анализатор кода для создания карты проекта"
    )
    parser.add_argument(
        "--root-dir",
        default=".",
        help="Корневая директория для анализа (по умолчанию: текущая)",
    )
    parser.add_argument(
        "--output",
        default="code_map.md",
        help="Файл для записи результатов (по умолчанию: code_map.md)",
    )
    parser.add_argument(
        "--index-output",
        default="method_index.md",
        help="Файл для записи индекса методов (по умолчанию: method_index.md)",
    )
    parser.add_argument(
        "--output-dir",
        help="Директория для записи выходных файлов (по умолчанию: текущая директория)",
    )
    parser.add_argument(
        "--include-pattern",
        default="*.py",
        help="Паттерн для включения файлов (по умолчанию: *.py)",
    )
    parser.add_argument("--exclude-pattern", help="Паттерн для исключения файлов")
    parser.add_argument(
        "--generate-index", action="store_true", help="Создать индекс методов"
    )
    parser.add_argument(
        "--index-only",
        action="store_true",
        help="Создать только индекс методов (без карты кода)",
    )
