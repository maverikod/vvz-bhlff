...
class CodeMapperCore:
        ...
        try:
            ...
            for node in ast.walk(tree):
                if isinstance(node, ast.ClassDef):
                    ...
                    function_info = self.extract_function_info(node, file_path, import_lines)
                    file_info["functions"].append(function_info)
                    self.code_map["functions"][f"{file_path}::{node.name}"] = function_info
            ...
            if import_lines:
                ...
                if max_import_line > 20:  # Consider imports after line 20 as "in middle"
                    self.issues["imports_in_middle"].append({
                        "file": file_path,
                        "line": max_import_line
                    })
    ...
    def _is_abstract_method(self, node: ast.FunctionDef) -> bool:
        ...
        return any(isinstance(decorator, ast.Name) and decorator.id == "abstractmethod" 
