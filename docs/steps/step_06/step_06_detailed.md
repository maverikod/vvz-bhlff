...
##### 1.1. Наблюдательные аналоги (уточнение %%7d-51%%)
- **Фотосфера (R≈1.0 R☉)** — внешняя динамическая «корка»: толщина ≈500 км, порог `Π_c≈1.16`, `η≪1`. В моделях Step 06 необходимо проверять, что численные оболочки воспроизводят высокое отражение без требования «твёрдости».
- **BCZ/тахоклина (R≈0.713 R☉, w=0.02–0.05 R☉)** — внутренняя переходная зона. Наши резонаторы должны удерживать `Π_eff≥2.2` при `η≈0.15`, что задаёт требования к добротности и сглаженности профиля.
- **NSSL** трактуется как толстый динамический слой (`η≈0.5`, `Π_c≈3.7`) — в нём ожидается плавное рифление, а не ступенька. Все Level C сценарии должны явно логировать `Π_eff` и сравнивать его с этими порогами.
- Для отчётности используем ту же таблицу параметров (`χ²`, `β`, `Π_c`, `η`) что и в разделе A1.3 Level A, чтобы обеспечить непрерывность требований между уровнями.
...
#### 4. 7D Ньютоновская форма и тензорный отклик (уточнение %%7d-53%%)
- **Разделение ускорений:** `a_i = a_i^G + a_i^D`, где `a_i^G = Ω_i × v_i` (геометрический вклад без работы), `a_i^D` — драйв-индуцированная перестройка.
- **Нелокальная реакция среды:** `F_i^{med} = Σ_j T_{ij} a_j^D`, `T_{ij} = S_{ij} + A_{ij}`. `S` отвечает за мощность перетока `P_phase = Σ v_i·S_{ij} a_j^D`, `A` даёт чисто гироскопические поправки.
- **Моменты:** для спинов используем `τ_i^{med} = Σ_j C_{ij} a_j^D` с аналогичным разложением на `C^S/C^A`. Это связывает ориентацию резонаторов с траекториями и обязует Step 06 пересобирать `T/C` при каждом обновлении `Q_i`.
- **Требования к реализации:**
  1. Внутри расчётов Level C необходимо хранить `S` и `A` явно и логировать `P_phase`, чтобы различать режимы «орбиты/джеты» (`P_phase≈0`) и «перетоков».
  2. Матрица `T` должна собираться по полосам (`κ_b, ε_b, γ_b, φ_b`) с учётом анизотропных проекторов `A_j^{(b)}`, `B_i^{(b)}`, как указано в 7d-53.
  3. Для анизотропных резонаторов обязательно учитывать спин-орбитальные поправки `δF_i^{spin}` (матрицы `G_{ij}`), иначе мы потеряем эффект, подтверждённый теорией.
  4. Любые упрощения вида «твёрдая масса m_i» должны быть явно помечены как аппроксимации; точная схема использует самодействие `T_{ii}` вместо отдельной массы.
