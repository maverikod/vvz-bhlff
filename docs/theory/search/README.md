...
1. **`--db-dir /mnt/data`** — скрипт находит в каталоге все `*.sqlite` (кроме манифеста `*.chain.sqlite`) и ищет по ним.
2. **`--db-path-glob "/mnt/data/ALL_theory_blocks.chain.part*.sqlite"`** — явно указать все части цепочки по подстановке.
3. **`--db-path /mnt/data/ALL_theory_blocks.chain.part002.sqlite`** — передать любой один файл цепочки: скрипт автоматически подхватит все `chain.part*.sqlite` в том же каталоге и будет искать по всем.
...
```bash
# Достаточно указать любой один файл цепочки — скрипт найдёт все part*.sqlite
# в том же каталоге и будет искать по всем
python3 /mnt/data/theta_search_tool/docs/theory/search/search_theory_index.py \
    --mode sqlite_search \
