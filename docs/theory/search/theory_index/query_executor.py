...
def execute_query(
    ...
    if query_node.op == "OR":
        if not query_node.left and not query_node.right:
            return []
        left_results = execute_query(query_node.left, term_results) if query_node.left else []
        right_results = execute_query(query_node.right, term_results) if query_node.right else []
        # Union
