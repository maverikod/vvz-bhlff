...
from bhlff.core.domain.domain_7d import (
    Domain7D,
    SpatialConfig,
    PhaseConfig,
    TemporalConfig,
...
def create_3d_domain(config: dict) -> Domain:
    ...
    return Domain(L=spatial["L_x"], N=spatial["N_x"], dimensions=7)
...
def run_bvp_7d_example():
    ...
    config_path = os.path.join(
        os.path.dirname(__file__), "..", "configs", "bvp_7d_config.json"
    )
    ...
    try:
        ...
        print(
            f"  Envelope magnitude range: [{np.min(np.abs(envelope_7d)):.2e}, {np.max(np.abs(envelope_7d)):.2e}]"
        )
    ...
    try:
        ...
        print(
            f"  Satisfied postulates: {postulate_results['satisfaction_count']}/{postulate_results['total_postulates']}"
        )
