...
from bhlff.tests.unit.test_level_b.test_fundamental_properties import (
...
def run_level_b_analysis_example():
    ...
    print(
        f"BVP parameters: μ={bvp_params.mu}, β={bvp_params.beta}, λ={bvp_params.lambda_param}"
    )
    ...
    print(
        f"Solution amplitude range: [{np.min(np.abs(solution)):.3e}, {np.max(np.abs(solution)):.3e}]"
    )
    ...
    print(
        f"     Slope: {power_law_result['slope']:.3f} (theoretical: {power_law_result['theoretical_slope']:.3f})"
    )
    ...
    print(
        f"     Core fraction: {zone_result['zone_stats']['core']['volume_fraction']:.3f}"
    )
    ...
    power_law_analyzer.visualize_power_law_analysis(
        power_law_result, f"{output_dir}/power_law_analysis.png"
    )

    node_analyzer.visualize_node_analysis(
        node_result, f"{output_dir}/node_analysis.png"
    )

    zone_analyzer.visualize_zone_analysis(
        zone_result, f"{output_dir}/zone_analysis.png"
    )

    # 3D visualization
    visualizer.create_3d_visualization(
        solution, center, f"{output_dir}/3d_field_visualization.png"
    )
    ...
    visualizer.create_comprehensive_report(
        test_results, f"{output_dir}/comprehensive_report"
    )
...
def run_parameter_variation_example():
    ...
    for beta in beta_values:
        try:
            ...
            print(
                f"{beta:.1f}\t{result['slope']:.3f}\t{result['theoretical_slope']:.3f}\t"
...
if __name__ == "__main__":
    ...
    print(
        "Check the 'level_b_analysis_output' directory for all results and visualizations."
    )
