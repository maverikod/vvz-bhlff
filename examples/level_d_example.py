...
def run_mode_superposition_analysis(
    models: LevelDModels, field: np.ndarray
) -> Dict[str, Any]:
    """
...
def run_field_projection_analysis(
    models: LevelDModels, field: np.ndarray
) -> Dict[str, Any]:
    """
...
def visualize_results(results: Dict[str, Any], output_dir: str = "output"):
    ...
    if "projections" in results:
        ...
        plt.tight_layout()
        plt.savefig(
            os.path.join(output_dir, "field_projections.png"),
            dpi=300,
            bbox_inches="tight",
        )
        plt.close()
...
def save_results(results: Dict[str, Any], output_dir: str = "output"):
    ...
    if "superposition" in results:
        with open(
            os.path.join(output_dir, "mode_superposition_analysis.json"), "w"
        ) as f:
...
def main():
    ...
    if "superposition" in results:
        ...
        print(
            f"  Frame stability: {'PASSED' if superposition.get('passed', False) else 'FAILED'}"
        )
    ...
    if "projections" in results:
        ...
        print(
            f"  EM projection norm: {np.linalg.norm(projections['em_projection']):.3f}"
        )
        print(
            f"  Strong projection norm: {np.linalg.norm(projections['strong_projection']):.3f}"
        )
        print(
            f"  Weak projection norm: {np.linalg.norm(projections['weak_projection']):.3f}"
        )
