...
def example_cosmological_evolution():
    ...
    print(
        f"Structure growth rate: {structure_analysis.get('structure_growth_rate', 0):.2e}"
    )

    # Compute cosmological parameters
    cosmological_params = cosmology.compute_cosmological_parameters()
    print(
        f"Current Hubble parameter: {cosmological_params.get('current_hubble_parameter', 0):.2f}"
    )
...
def example_large_scale_structure():
    ...
    resolution = 64
    initial_fluctuations = np.random.normal(
        0, 0.1, (resolution, resolution, resolution)
    )
    ...
    evolution_params = {
        ...
        "cosmology": {"G": 6.67430e-11, "rho_m": 2.7e-27, "H0": 70.0},
    }
...
def example_particle_inversion():
    ...
    inversion = ParticleInversion(
        electron_observables, electron_priors, loss_weights, optimization_params
    )
    ...
    return {"inversion": inversion_results, "validation": validation_results}
