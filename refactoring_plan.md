...
- **1 экспоненциальный интегратор** → BVP envelope интегратор
...
**Проблема**: Классическое экспоненциальное решение:
```python
# НЕПРАВИЛЬНО (классический паттерн):
â(k,t) = â₀(k)e^(-(ν|k|^(2β)+λ)t) + ŝ₀(k)/(ν|k|^(2β)+λ+iω)(1-e^(-(ν|k|^(2β)+λ+iω)t))
```

**Решение**: BVP envelope уравнение:
```python
# ПРАВИЛЬНО (BVP теория):
