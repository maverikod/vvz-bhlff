...
#### **Приоритет 1: Разделение больших файлов (БЕЗ потери функциональности)**
1. **Разделить файлы превышающие 400 строк**:
   - `beating_ml_optimization.py` (429 строк) → 3 модуля
   - `solver_core_basic.py` (429 строк) → 3 модуля  
   - `beating_core_basic.py` (427 строк) → 3 модуля
   - **НЕ УДАЛЯТЬ** функциональность, только разделить

2. **Сохранить FFT как основу для спектрального анализа**:
   - **НЕ ТРОГАТЬ** 41 файл в `core/fft/` для спектрального анализа
   - FFT нужен для: Y(ω), {ω_n, Q_n}, R(ω), T(ω) - сравнение с реальными спектрами
   - **НО**: FFT экспоненты ≠ BVP экспоненциальные решения

3. **🚨 КРИТИЧЕСКИ ИСПРАВИТЬ экспоненциальную интеграцию**:
   - `exponential_integrator.py` - **ЗАМЕНИТЬ** на BVP envelope интегратор
   - `fft_twiddle_computer.py` - **ОСТАВИТЬ** (это математика FFT)
   - `spectral_filtering.py` - **ОСТАВИТЬ** (это математика FFT)
   - **ПРИЧИНА**: Экспоненциальные решения противоречат BVP-теории

#### **Приоритет 2: Исправление недописанного кода**
1. **Заменить `pass` на полную реализацию**:
   - В неабстрактных методах
...
#### **Приоритет 3: Интеграция с BVP-теорией**
1. **Добавить недостающую BVP функциональность**:
   - В файлы с частичной реализацией (35 файлов - 49%)
   - БЕЗ удаления существующего кода
   - **Фокус**: `fft_solver_7d_basic.py` - упрощенный solve_envelope()

2. **Добавить U(1)³ фазовую структуру**:
   - В файлы где она отсутствует (26 файлов - 37%)
   - БЕЗ удаления существующего кода
   - **Фокус**: FFT решатели, модели уровней B и C

3. **Добавить квенч-детекцию**:
   - В файлы где она отсутствует (36 файлов - 51%)
   - БЕЗ удаления существующего кода
   - **Фокус**: Базовые FFT решатели, стандартные интеграторы

4. **Заменить экспоненциальные интеграторы на BVP envelope методы**:
   - `exponential_integrator.py` → BVP envelope интегратор
   - Использовать прямые методы решения на частотах
   - FEM в частотной области для сложных случаев

### ИСПРАВЛЕННЫЕ ожидаемые результаты:

**До рефакторинга**: 311 файлов
**После рефакторинга**: ~320 файлов (увеличение на 3%)

**Причины увеличения**:
- Разделение больших файлов на модули
- Добавление недостающей BVP функциональности
- **НЕ УДАЛЕНИЕ** существующего кода

**Сохранение критически важных компонентов**:
- **41 файл FFT** - основа для спектрального анализа (Y(ω), {ω_n, Q_n})
- **FFT математика** - twiddle factors, нормализация (критически важны)
- **Все существующие решатели** - каждый имеет свою роль
- **Вся валидация и оптимизация** - необходимы для качества
- **ПРАВИЛЬНАЯ архитектура** - разделение классов по функциональности

**Критические исправления**:
- **Экспоненциальные интеграторы** → BVP envelope методы
- **Классические спектральные решения** → BVP envelope equation
- **Частичная BVP интеграция** → полная BVP интеграция
- **НЕ ТРОГАТЬ** правильное разделение классов

## ИСПРАВЛЕННЫЕ ожидаемые результаты

После выполнения рефакторинга:
- **Сохранение всей функциональности**: без потерь
- **Соблюдение лимита 400 строк**: все файлы ≤ 400 строк
- **Полная реализация**: замена `pass` и `NotImplemented`
- **Дополнение BVP-функциональности**: без удаления существующего
- **Покрытие тестами**: >90% для всех файлов
- **FFT для спектрального анализа**: сохранение всех 41 файла для Y(ω), {ω_n, Q_n}
- **BVP envelope методы**: замена экспоненциальных интеграторов
- **Полная BVP интеграция**: 35 файлов с частичной → полной реализацией
- **U(1)³ фазовая структура**: добавление в 26 файлов
- **Квенч-детекция**: добавление в 36 файлов
