...
def demo_performance_scaling():
    ...
    for size in data_sizes:
        ...
        print(
            f"{size:<8} {fitting_time:<10.4f} {points_per_second:<12.0f} {quality:<10.6f}"
        )
...
def demo_7d_phase_field_characteristics():
    ...
    test_data = {"r": r, "values": energy_density}

    print(f"7D Phase Field Data:")
    print(f"  Points: {len(r)}")
    print(f"  Base field range: [{np.min(base_field):.3f}, {np.max(base_field):.3f}]")
    print(
        f"  Phase modulation range: [{np.min(phase_modulation):.3f}, {np.max(phase_modulation):.3f}]"
    )
    print(
        f"  Topological charge range: [{np.min(topological_charge):.3f}, {np.max(topological_charge):.3f}]"
    )
    print(
        f"  Energy density range: [{np.min(energy_density):.3f}, {np.max(energy_density):.3f}]"
    )
...
def main():
    ...
    try:
        # Demonstrate vectorized power law fitting
        result, fitting_time, quality_time, decay_time = (
            demo_power_law_fitting_vectorized()
        )
