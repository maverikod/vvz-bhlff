...
def test_error_estimation_physics(integrator):
    ...
    field_4th = np.random.randn(8, 8, 8, 4, 4, 4, 8) + 1j * np.random.randn(8, 8, 8, 4, 4, 4, 8)
    field_5th = field_4th + 1e-6 * (np.random.randn(8, 8, 8, 4, 4, 4, 8) + 1j * np.random.randn(8, 8, 8, 4, 4, 4, 8))
    dt = 0.01
    ...
    assert "max_spatial_error" in error_components, "Should include spatial error analysis"
    assert "mean_spectral_error" in error_components, "Should include spectral error analysis"
    assert "high_frequency_error" in error_components, "Should include high-frequency error analysis"
...
def test_stability_analysis_physics(integrator):
    ...
    field_5th = field_4th + 0.01 * (np.random.randn(8, 8, 8, 4, 4, 4, 8) + 1j * np.random.randn(8, 8, 8, 4, 4, 4, 8))
    dt = 0.01
    ...
    assert "stability_indicator" in stability_analysis, "Should include stability indicator"
    assert "high_frequency_error" in stability_analysis, "Should include high-frequency error"
    assert "low_frequency_error" in stability_analysis, "Should include low-frequency error"
    
    # Stability indicator should be reasonable
    assert 0 <= stability_analysis["stability_indicator"] <= 10, "Stability indicator should be in reasonable range"
...
def test_integration_accuracy_physics(integrator):
    ...
    try:
        ...
        # Physics checks
        assert field_next.shape == initial_field.shape, "Result should have correct shape"
        ...
        assert final_magnitude < initial_magnitude, "Field should decay for exponential decay"
...
def test_runge_kutta_physics():
    ...
    field_next, error_estimate = rk_methods.embedded_rk_step(field, source, dt, test_rhs)
    ...
    assert field_magnitude_final < field_magnitude_initial, "Field should decay for exponential decay"
