...
sys.path.append(
    "/home/vasilyvz/Desktop/Инерция/7d/progs/bhlff/bhlff/models/level_a/validation"
)
...
def test_convergence_analysis_physics():
    ...
    detailed_analysis = convergence_analyzer._perform_full_convergence_analysis(
        envelope, source
    )

    # Physics checks
    assert (
        "finite_envelope" in detailed_analysis
    ), "Should include finite envelope check"
    ...
    assert (
        "iterative_analysis" in detailed_analysis
    ), "Should include iterative analysis"
    ...
    assert (
        "residual_converged" in residual_analysis
    ), "Should include convergence status"

    # Check iterative analysis
    iterative_analysis = detailed_analysis["iterative_analysis"]
    assert (
        "oscillation_measure" in iterative_analysis
    ), "Should include oscillation measure"
    ...
    assert (
        "spectral_stability" in spectral_analysis
    ), "Should include spectral stability"
    assert (
        "high_freq_content" in spectral_analysis
    ), "Should include high-frequency content"
...
def test_energy_analysis_physics():
    ...
    assert (
        "interaction_energy" in detailed_analysis
    ), "Should include interaction energy"
    assert "total_energy" in detailed_analysis, "Should include total energy"
    assert (
        "energy_conservation" in detailed_analysis
    ), "Should include energy conservation"
    assert "energy_balance" in detailed_analysis, "Should include energy balance"
    assert (
        "energy_distribution" in detailed_analysis
    ), "Should include energy distribution"
    ...
    assert (
        "energy_conserved" in energy_conservation
    ), "Should include energy conserved status"
    ...
    assert (
        "max_energy_fraction" in energy_distribution
    ), "Should include max energy fraction"
    assert (
        "energy_distributed" in energy_distribution
    ), "Should include energy distributed status"
    assert (
        "energy_concentration" in energy_distribution
    ), "Should include energy concentration"
    ...
    assert (
        detailed_analysis["kinetic_energy"] >= 0
    ), "Kinetic energy should be non-negative"
    assert (
        detailed_analysis["potential_energy"] >= 0
    ), "Potential energy should be non-negative"
    assert detailed_analysis["total_energy"] >= 0, "Total energy should be non-negative"
    assert (
        detailed_analysis["envelope_energy"] >= 0
    ), "Envelope energy should be non-negative"
    assert (
        detailed_analysis["source_energy"] >= 0
    ), "Source energy should be non-negative"
...
def test_challenging_fields():
    ...
    envelope = np.random.randn(4, 4, 4, 2, 2, 2, 4) + 1j * np.random.randn(
        4, 4, 4, 2, 2, 2, 4
    )
    source = np.random.randn(4, 4, 4, 2, 2, 2, 4) + 1j * np.random.randn(
        4, 4, 4, 2, 2, 2, 4
    )
...
def test_7d_specific_validation():
    ...
    interaction_energy = energy_analyzer._compute_interaction_energy_7d(
        envelope, source
    )
...
def test_validation_edge_cases():
    ...
    convergence_result = convergence_analyzer.check_convergence(
        zero_envelope, zero_source
    )
    energy_result = energy_analyzer.check_energy_conservation(
        zero_envelope, zero_source
    )
