...
def test_physical_validator():
    ...
    try:
        ...
        valid_result = {
            "field": np.random.randn(*domain_shape)
            + 1j * np.random.randn(*domain_shape),
        ...
        print(
            f"  - Physical constraints valid: {physical_result['physical_constraints_valid']}"
        )
        print(
            f"  - Constraint violations: {len(physical_result['constraint_violations'])}"
        )
        ...
        print(
            f"  - Theoretical bounds valid: {theoretical_result['theoretical_bounds_valid']}"
        )
...
def test_specific_validation_decorators():
    ...
    try:
        ...
        assert (
            "causality_validation" in test_causality_method()
        ), "Causality validation missing"
        assert (
            "structure_validation" in test_structure_method()
...
def test_validation_mixin():
    ...
    try:
        ...
        print(
            f"  - Physical constraints valid: {physical_result.get('physical_constraints_valid', False)}"
        )
        ...
        print(
            f"  - Theoretical bounds valid: {theoretical_result.get('theoretical_bounds_valid', False)}"
        )
        ...
        assert hasattr(
            test_instance, "validate_result_physical"
        ), "Physical validation method missing"
        assert hasattr(
            test_instance, "validate_result_theoretical"
        ), "Theoretical validation method missing"
        assert hasattr(
            test_instance, "validate_result_comprehensive"
        ), "Comprehensive validation method missing"
...
def test_invalid_results():
    ...
    try:
        ...
        assert not physical_result[
            "physical_constraints_valid"
        ], "Should detect invalid result"
        assert (
            len(physical_result["constraint_violations"]) > 0
...
def test_no_basic_validation():
    ...
    try:
        ...
        assert (
            len(basic_methods) == 0
        ), f"Found basic validation methods: {basic_methods}"

        print("✓ No basic validation methods found")

        # Check for comprehensive validation methods
        comprehensive_methods = [
            method for method in methods if "validate" in method.lower()
        ]
        assert (
            len(comprehensive_methods) > 0
        ), "No comprehensive validation methods found"
        ...
        theoretical_methods = [
            ...
            if "theoretical" in method.lower() or "physical" in method.lower()
        ]
