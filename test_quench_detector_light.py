...
def test_quench_detector_minimal():
    ...
    try:
        ...
        spatial_config = SpatialConfig(
            L_x=1.0, L_y=1.0, L_z=1.0, N_x=8, N_y=8, N_z=8  # Very small grid
        )
        phase_config = PhaseConfig(
            N_phi_1=4, N_phi_2=4, N_phi_3=4  # Very small phase grid
        )
        temporal_config = TemporalConfig(T_max=0.1, N_t=5, dt=0.02)  # Very short time
...
def test_quench_detector_step_by_step():
    ...
    steps = [
        ...
        (
            "Importing quench detector",
            lambda: __import__("bhlff.core.bvp.quench_detector"),
        ),
        ...
        (
            "Creating domain",
            lambda: Domain7D(spatial_config, phase_config, temporal_config),
        ),
        (
            "Creating envelope",
            lambda: np.random.random((4, 4, 4, 2, 2, 2, 3)).astype(np.float32),
        ),
