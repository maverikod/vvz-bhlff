...
def optimize_gpu_memory():
    ...
    try:
        ...
        if mem_info["total"] > 0:
            ...
            logger.info(
                f"Set GPU memory limit to {target_memory / 1024**3:.2f} GB (80% of total)"
            )
...
def create_optimized_domain():
    ...
    try:
        ...
        spatial_config = SpatialConfig(
            L_x=1.0,
            L_y=1.0,
            L_z=1.0,  # Smaller spatial domain
            N_x=32,
            N_y=32,
            N_z=32,  # Reduced grid size
        )
...
def create_test_envelope(domain_7d, use_gpu=False):
    ...
    try:
        ...
        if use_gpu and CUDA_AVAILABLE:
            logger.info(
                f"GPU envelope stats: min={cp.min(envelope):.3f}, max={cp.max(envelope):.3f}, mean={cp.mean(envelope):.3f}"
            )
        else:
            logger.info(
                f"CPU envelope stats: min={np.min(envelope):.3f}, max={np.max(envelope):.3f}, mean={np.mean(envelope):.3f}"
            )
...
def test_quench_detector_cpu(domain_7d, envelope):
    ...
    try:
        ...
        logger.info(
            f"Detector thresholds: amp={detector.amplitude_threshold}, det={detector.detuning_threshold}, grad={detector.gradient_threshold}"
        )
        ...
        if results["quench_locations"]:
            ...
            logger.info(
                f"  First quench strength: {results['quench_strengths'][0]:.3f}"
            )
        else:
...
def main():
    ...
    try:
        ...
        if CUDA_AVAILABLE:
            mem_info = get_gpu_memory_info()
            logger.info(
                f"GPU Memory: {mem_info['used']:.2f}GB used / {mem_info['total']:.2f}GB total"
            )
            optimize_gpu_memory()
        else:
        ...
        if CUDA_AVAILABLE:
            ...
            logger.info(
                f"CPU detected {results_cpu['total_quenches']} quenches in {time_cpu:.3f}s"
            )
            logger.info(
                f"GPU detected {results_gpu['total_quenches']} quenches in {time_gpu:.3f}s"
            )
        ...
        if CUDA_AVAILABLE:
            mem_info = get_gpu_memory_info()
            logger.info(
                f"Final GPU Memory: {mem_info['used']:.2f}GB used / {mem_info['total']:.2f}GB total"
            )
