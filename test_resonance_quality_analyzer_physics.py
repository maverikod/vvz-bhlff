...
def test_lorentzian_fitting_physics(analyzer):
    ...
    assert (
        0 <= fit_results["fitting_quality"] <= 1
    ), "Fitting quality should be between 0 and 1"
...
def test_quality_factor_calculation_physics(analyzer):
    ...
    quality_factors = analyzer.calculate_quality_factors(
        frequencies, magnitude, peak_indices
    )

    # Physics checks
    assert len(quality_factors) == len(
        peak_indices
    ), "Should return quality factor for each peak"
    ...
    if len(quality_factors) >= 2:
        ...
        assert (
            quality_factors[0] > quality_factors[1]
        ), "High-Q peak should have higher quality factor"
...
def test_fitting_quality_assessment_physics(analyzer):
    ...
    assert (
        fit_results["fitting_quality"] > 0.7
    ), "Fitting quality should be high for good fit"
    ...
    assert all(
        error >= 0 for error in param_errors
    ), "Parameter errors should be non-negative"
...
def test_fallback_methods_physics(analyzer):
    ...
    fallback_results = analyzer._fallback_lorentzian_estimation(
        frequencies, magnitude, peak_idx
    )
    ...
    assert (
        fallback_results["fitting_quality"] < 0.8
    ), "Fallback should have lower fitting quality"
...
def test_optimization_physics():
    ...
    magnitude = (
        A_true * gamma_true**2 / ((frequencies - f0_true) ** 2 + gamma_true**2)
        + offset_true
    )
    ...
    # Physics checks
    assert (
        fit_results["fitting_quality"] > 0.5
    ), "Optimization should achieve reasonable quality"
    ...
    assert (
        center_error < 0.2
    ), f"Center frequency recovery error too large: {center_error:.2e}"
    assert (
        amplitude_error < 0.5
    ), f"Amplitude recovery error too large: {amplitude_error:.2e}"
