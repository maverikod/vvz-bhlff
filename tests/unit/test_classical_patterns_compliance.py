                ...
                for pattern in exponential_patterns:
                    ...
                    if matches:
                        ...
                        for match in matches:
                            ...
                            if (
                                "#" in context[: context.find(match)]
                                or "Mathematical Foundation:" in context
                                or "Theoretical Background:" in context
                            ):
                                continue
                            if (
                                "classical comparison" not in context.lower()
                                and "for comparison" not in context.lower()
                                and "classical" not in context.lower()
                            ):
                                pytest.fail(
                                    f"Exponential decay found in {file_path}: {match}"
                                )
                    ...
                    if matches:
                        ...
                        for match in matches:
                            ...
                            if (
                                ...
                                pytest.fail(
                                    f"Spacetime curvature reference found in {file_path}: {match}"
                                )
            except FileNotFoundError:
                # File doesn't exist, skip
                pass
                    ...
                    if matches:
                        ...
                        for match in matches:
                            ...
                            if "abstract" not in context.lower():
                                pytest.fail(
                                    f"Placeholder found in {file_path}: {match}"
                                )
            except FileNotFoundError:
                # File doesn't exist, skip
                pass
    ...
    def test_file_size_compliance(self):
        ...
        for file_path in analysis_files:
            try:
                ...
                if len(lines) > 400:
                    pytest.fail(
                        f"File {file_path} exceeds 400 lines: {len(lines)} lines"
                    )
            except FileNotFoundError:
                # File doesn't exist, skip
                pass
            ...
            try:
                ...
                for element in required_elements:
                    if element not in content:
                        pytest.fail(
                            f"Missing required docstring element '{element}' in {file_path}"
                        )
            except FileNotFoundError:
                # File doesn't exist, skip
                pass
                ...
                if import_lines:
                    ...
                    for line_num in import_lines:
                        if line_num > 50:  # Allow some flexibility
                            pytest.fail(
                                f"Import found after line 50 in {file_path}: line {line_num}"
                            )
            except FileNotFoundError:
                # File doesn't exist, skip
                pass
