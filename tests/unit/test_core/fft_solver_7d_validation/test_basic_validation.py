...
class TestBasicValidation:
    ...
    def test_A06_solve_method_alias(self, solver, domain_7d):
        ...
        assert relative_error < 1e-12, f"solve() and solve_stationary() should be identical, error={relative_error}"
    ...
    def test_A07_get_spectral_coefficients(self, solver, domain_7d):
        ...
        assert isinstance(spectral_coeffs, np.ndarray), "Spectral coefficients should be numpy array"
        assert spectral_coeffs.shape == domain_7d.shape, f"Spectral coefficients shape {spectral_coeffs.shape} should match domain shape {domain_7d.shape}"
        
        # Check that coefficients are positive (for physical validity)
        assert np.all(spectral_coeffs >= 0), "Spectral coefficients should be non-negative"
        
        # Check that k=0 mode has correct value
        lambda_param = getattr(solver.parameters, 'lambda_param', 0.1)
        k0_value = spectral_coeffs[0, 0, 0, 0, 0, 0, 0]
        if lambda_param > 0:
            assert abs(k0_value - lambda_param) < 1e-12, f"k=0 mode should equal lambda_param={lambda_param}, got {k0_value}"
        else:
            assert k0_value == 1.0, f"k=0 mode should be 1.0 when lambda_param=0, got {k0_value}"
    ...
    def test_A08_spectral_coefficients_consistency(self, solver, domain_7d):
        ...
        if lambda_param > 0:
            assert abs(k0_coeff - lambda_param) < 1e-12, f"k=0 mode should equal lambda_param={lambda_param}, got {k0_coeff}"
        else:
            assert k0_coeff == 1.0, f"k=0 mode should be 1.0 when lambda_param=0, got {k0_coeff}"
        
        # Test that coefficients are non-negative and finite
        assert np.all(np.isfinite(spectral_coeffs)), "All spectral coefficients should be finite"
        assert np.all(spectral_coeffs >= 0), "All spectral coefficients should be non-negative"
        ...
        assert coeff_200 > coeff_100, f"Higher frequency mode should have larger coefficient: {coeff_200} > {coeff_100}"
        
        # Test that coefficients are symmetric for symmetric modes
        coeff_100 = spectral_coeffs[1, 0, 0, 0, 0, 0, 0]
        coeff_010 = spectral_coeffs[0, 1, 0, 0, 0, 0, 0]
        assert abs(coeff_100 - coeff_010) < 1e-12, f"Symmetric modes should have equal coefficients: {coeff_100} ≈ {coeff_010}"
