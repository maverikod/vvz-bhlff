...
class TestBasicValidation:
    ...
    def _create_plane_wave_source(self, domain: Domain, k_mode: Tuple[int, int, int], amplitude: float = 1.0) -> np.ndarray:
    ...
    def test_A01_plane_wave_stationary(self, solver, domain_7d):
        ...
        for k_mode in k_modes:
            ...
            assert relative_error < 1e-10, f"Plane wave test failed for k={k_mode}, error={relative_error}"
    ...
    def test_A02_analytical_constant_source(self, solver, domain_7d):
        ...
        # Check the value
        relative_error = np.abs(solution[0, 0, 0, 0, 0, 0, 0] - expected_solution[0, 0, 0, 0, 0, 0, 0]) / np.abs(expected_solution[0, 0, 0, 0, 0, 0, 0])
        assert relative_error < 1e-10, f"Constant source test failed, error={relative_error}"
    ...
    def test_A03_linearity_property(self, solver, domain_7d):
        ...
        # Check linearity
        relative_error = np.linalg.norm(combined_solution - expected_solution) / np.linalg.norm(expected_solution)
    ...
    def test_A04_energy_conservation(self, solver, domain_7d):
        ...
        assert max_solution < 10.0, f"Solution should not grow excessively, max={max_solution}"
    ...
    def test_A05_parameter_dependence(self, solver, domain_7d):
        ...
        for i in range(len(solutions)):
            for j in range(i+1, len(solutions)):
                relative_diff = np.linalg.norm(solutions[i] - solutions[j]) / np.linalg.norm(solutions[i])
                assert relative_diff > 1e-6, f"Solutions should differ for different parameters, diff={relative_diff}"
