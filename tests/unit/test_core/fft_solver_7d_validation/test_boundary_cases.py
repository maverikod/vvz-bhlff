...
class TestBoundaryCases:
    ...
    def test_C02_very_small_source(self, solver, domain_7d):
        ...
        ), "Solution should be finite for very small source"
        assert np.allclose(
            solution, source / solver.parameters.lambda_param, rtol=1e-10
        ), "Solution should be proportional to source"
    ...
    def test_C03_very_large_source(self, solver, domain_7d):
        ...
        assert np.all(
            np.isfinite(solution)
        ), "Solution should be finite for very large source"
        assert not np.any(
            np.isinf(solution)
        ), "Solution should not contain Inf for very large source"
    ...
    def test_C04_extreme_parameters(self, domain_7d):
        ...
        for case in extreme_cases:
            try:
                ...
                assert np.all(
                    np.isfinite(solution)
                ), f"Solution should be finite for extreme case {case}"
    ...
    def test_C05_singular_conditions(self, domain_7d):
        ...
        for case in singular_cases:
            ...
            assert np.all(
                np.isfinite(solution)
            ), f"Solution should be finite for singular case {case}"
    ...
    def test_C09_consistency_across_runs(self, solver, domain_7d):
        ...
        for i in range(1, len(solutions)):
            ...
            ) / np.linalg.norm(solutions[0])
            assert (
                relative_diff < 1e-15
            ), f"Solutions should be identical across runs, diff={relative_diff}"
