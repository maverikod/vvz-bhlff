...
class TestNumericalValidation:
    ...
    def _create_gaussian_source(self, domain: Domain, center: Tuple[float, float, float], width: float = 0.1) -> np.ndarray:
        ...
        # Create Gaussian
        cx, cy, cz = center
        source = np.exp(-((X - cx)**2 + (Y - cy)**2 + (Z - cz)**2) / (2 * width**2))
    ...
    def test_B01_convergence_test(self, domain_7d):
        ...
        for N in resolutions:
            ...
            source = self._create_gaussian_source(test_domain, (0.5, 0.5, 0.5), width=0.2)
        ...
        for i in range(len(solutions)-1):
            ...
            if sol2.shape[0] > sol1.shape[0]:
                ...
                sol2_downsampled = sol2_downsampled[:sol1.shape[0], :sol1.shape[1], :sol1.shape[2], :, :, :, :]
            else:
                sol2_downsampled = sol2
            
            # Solutions should be similar (converging)
            relative_diff = np.linalg.norm(sol1 - sol2_downsampled) / np.linalg.norm(sol1)
            assert relative_diff < 0.5, f"Solutions should converge, diff={relative_diff}"
    ...
    def test_B02_boundary_conditions(self, solver, domain_7d):
        ...
        assert max_solution < 100.0, f"Solution should not have extreme values, max={max_solution}"
    ...
    def test_B03_numerical_stability(self, solver, domain_7d):
        ...
        for case in test_cases:
            ...
            # Check stability
            assert np.all(np.isfinite(solution)), f"Solution should be finite for case {case}"
            assert not np.any(np.isnan(solution)), f"Solution should not contain NaN for case {case}"
            assert not np.any(np.isinf(solution)), f"Solution should not contain Inf for case {case}"
    ...
    def test_B04_precision_validation(self, domain_7d):
        ...
        for i, solution in enumerate(solutions):
            assert np.all(np.isfinite(solution)), f"Solution should be finite for precision {precisions[i]}"
            assert np.max(np.abs(solution)) < 100.0, f"Solution should be bounded for precision {precisions[i]}"
    ...
    def test_B05_spectral_accuracy(self, solver, domain_7d):
        ...
        kx, ky, kz = 2, 1, 0
        source = np.sin(2 * np.pi * kx * X / domain_7d.L) * np.sin(2 * np.pi * ky * Y / domain_7d.L)
        ...
        assert correlation > 0.9, f"Solution should correlate with source, correlation={correlation}"
