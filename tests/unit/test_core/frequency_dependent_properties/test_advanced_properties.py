...
class TestAdvancedProperties:
    ...
    def test_frequency_array_operations(self, freq_props):
        ...
        assert np.allclose(omega, expected_omega, rtol=1e-10), \
    ...
    def test_susceptibility_frequency_dependence(self, freq_props):
        ...
        assert np.std(chi_real) > 1e-10, "Real part of susceptibility should vary with frequency"
        assert np.std(chi_imag) > 1e-10, "Imaginary part of susceptibility should vary with frequency"
    ...
    def test_dispersion_relation_consistency(self, freq_props):
        ...
        for freq in frequencies:
            ...
            assert np.isclose(v_phase, expected_v_phase, rtol=1e-10), \
    ...
    def test_velocity_relationships(self, freq_props):
        ...
        for freq in frequencies:
            ...
            assert np.isfinite(v_phase), f"Phase velocity should be finite at frequency {freq}"
            assert np.isfinite(v_group), f"Group velocity should be finite at frequency {freq}"
    ...
    def test_refractive_index_properties(self, freq_props):
        ...
        for freq in frequencies:
            ...
            assert np.real(n) > 0, f"Real part of refractive index should be positive at frequency {freq}"
            
            # Imaginary part should be non-negative (for passive media)
            assert np.imag(n) >= 0, f"Imaginary part of refractive index should be non-negative at frequency {freq}"
            
            # Refractive index should be finite
            assert np.isfinite(n), f"Refractive index should be finite at frequency {freq}"
    ...
    def test_absorption_properties(self, freq_props):
        ...
        for freq in frequencies:
            ...
            assert alpha >= 0, f"Absorption coefficient should be non-negative at frequency {freq}"
            
            # Absorption should be finite
            assert np.isfinite(alpha), f"Absorption coefficient should be finite at frequency {freq}"
    ...
    def test_high_frequency_behavior(self, freq_props):
        ...
        for freq in high_frequencies:
            ...
            assert np.isfinite(chi), f"Susceptibility should be finite at high frequency {freq}"
            assert np.isfinite(k), f"Wave number should be finite at high frequency {freq}"
            assert np.isfinite(v_phase), f"Phase velocity should be finite at high frequency {freq}"
            assert np.isfinite(v_group), f"Group velocity should be finite at high frequency {freq}"
            assert np.isfinite(alpha), f"Absorption coefficient should be finite at high frequency {freq}"
            assert np.isfinite(n), f"Refractive index should be finite at high frequency {freq}"
    ...
    def test_low_frequency_behavior(self, freq_props):
        ...
        for freq in low_frequencies:
            ...
            assert np.isfinite(chi), f"Susceptibility should be finite at low frequency {freq}"
            assert np.isfinite(k), f"Wave number should be finite at low frequency {freq}"
            assert np.isfinite(v_phase), f"Phase velocity should be finite at low frequency {freq}"
            assert np.isfinite(v_group), f"Group velocity should be finite at low frequency {freq}"
            assert np.isfinite(alpha), f"Absorption coefficient should be finite at low frequency {freq}"
            assert np.isfinite(n), f"Refractive index should be finite at low frequency {freq}"
    ...
    def test_parameter_sensitivity(self, domain_7d):
        ...
        assert not np.isclose(chi1, chi2, rtol=1e-10), \
