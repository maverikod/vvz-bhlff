...
class TestBasicProperties:
    ...
    def test_frequency_domain_creation(self, freq_props, domain_7d):
        ...
        assert np.all(np.diff(freq_props.frequencies) > 0), "Frequencies should be increasing"
        assert np.all(np.diff(freq_props.omega) > 0), "Angular frequencies should be increasing"
    ...
    def test_susceptibility_calculation(self, freq_props):
        ...
        assert np.isfinite(chi_zero), "Susceptibility at zero frequency should be finite"
        
        # Test at non-zero frequency
        chi_nonzero = freq_props.compute_susceptibility(1.0)
        assert np.isfinite(chi_nonzero), "Susceptibility at non-zero frequency should be finite"
    ...
    def test_dispersion_relation(self, freq_props):
        ...
        for freq in frequencies:
            ...
            assert np.isfinite(k), f"Dispersion relation should be finite at frequency {freq}"
    ...
    def test_physical_constraints(self, freq_props):
        ...
        for freq in frequencies:
            ...
            assert np.isclose(np.real(chi), np.real(chi_neg), rtol=1e-10), \
                f"Real part of susceptibility should be even at frequency {freq}"
            
            # Imaginary part should be odd function of frequency
            assert np.isclose(np.imag(chi), -np.imag(chi_neg), rtol=1e-10), \
