...
class TestAdvancedCoefficients:
    ...
    def test_nonlinear_effects(self, nonlinear_coeffs):
        ...
        assert (
            nonlinear_coeffs.kappa_2 >= 0
        ), "Nonlinear stiffness should be non-negative"

        # The ratio kappa_2/kappa_0 should be reasonable
        ratio = nonlinear_coeffs.kappa_2 / nonlinear_coeffs.kappa_0
        assert (
            0 <= ratio <= 1
        ), "Nonlinear to linear stiffness ratio should be reasonable"
    ...
    def test_susceptibility_properties(self, nonlinear_coeffs):
        ...
        chi_complex = (
            nonlinear_coeffs.chi_prime + 1j * nonlinear_coeffs.chi_double_prime_0
        )

        # Real part should be positive
        assert (
            np.real(chi_complex) > 0
        ), "Real part of susceptibility should be positive"

        # Imaginary part should be non-negative
        assert (
            np.imag(chi_complex) >= 0
        ), "Imaginary part of susceptibility should be non-negative"

        # Magnitude should be finite
        assert np.isfinite(
            np.abs(chi_complex)
        ), "Susceptibility magnitude should be finite"
    ...
    def test_extreme_parameter_values(self, domain_7d):
        ...
        assert np.isfinite(
            nonlinear_coeffs_small.kappa_2
        ), "Very small kappa_2 should be finite"
        assert (
            nonlinear_coeffs_small.kappa_2 >= 0
        ), "Very small kappa_2 should be non-negative"
        ...
        # Should still be finite
        assert np.isfinite(
            nonlinear_coeffs_large.kappa_2
        ), "Very large kappa_2 should be finite"
        assert (
            nonlinear_coeffs_large.kappa_2 >= 0
        ), "Very large kappa_2 should be non-negative"
    ...
    def test_zero_nonlinear_coefficient(self, domain_7d):
        ...
        assert (
            nonlinear_coeffs_zero.kappa_2 == 0.0
        ), "Zero kappa_2 should be exactly zero"

        # Other coefficients should remain finite
        assert np.isfinite(
            nonlinear_coeffs_zero.kappa_0
        ), "kappa_0 should be finite with zero kappa_2"
        assert np.isfinite(
            nonlinear_coeffs_zero.chi_prime
        ), "chi_prime should be finite with zero kappa_2"
    ...
    def test_susceptibility_limits(self, domain_7d):
        ...
        # Should still be finite
        assert np.isfinite(
            nonlinear_coeffs_small.chi_double_prime_0
        ), "Very small chi_double_prime_0 should be finite"
        assert (
            nonlinear_coeffs_small.chi_double_prime_0 >= 0
        ), "Very small chi_double_prime_0 should be non-negative"
    ...
    def test_carrier_frequency_limits(self, domain_7d):
        ...
        for freq in frequencies:
            ...
            ), f"Carrier frequency should be finite for {freq}"
            assert (
                nonlinear_coeffs.carrier_frequency > 0
            ), f"Carrier frequency should be positive for {freq}"
