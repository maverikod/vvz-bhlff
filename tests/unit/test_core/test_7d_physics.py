...
class Test7DPhysics:
    ...
    def test_7d_envelope_equation_physics(self, envelope_solver, domain_7d):
        ...
        expected_shape = (domain_7d.N, domain_7d.N, domain_7d.N, 
                         domain_7d.N_phi, domain_7d.N_phi, domain_7d.N_phi, domain_7d.N_t)
        assert envelope.shape == expected_shape, f"Wrong envelope shape: {envelope.shape}"
    ...
    def test_quench_dynamics_physics(self, quench_detector, domain_7d):
        ...
        assert quench_gradient_correlation > 0.5, "Quenches don't correlate with gradients"
    ...
    def test_spectral_properties_physics(self, envelope_solver, domain_7d):
        ...
        assert -3 < power_law_exponent < -1, f"Power law exponent out of range: {power_law_exponent}"
    ...
    def _create_envelope_with_quenches(self, domain: Domain) -> np.ndarray:
        ...
        envelope[domain.N//4:3*domain.N//4, domain.N//4:3*domain.N//4, 
    ...
    def _validate_boundary_conditions(self, envelope: np.ndarray, domain: Domain) -> None:
        """Validate boundary conditions."""
        # Check spatial boundaries (should be periodic)
        assert np.allclose(envelope[0, :, :, :, :, :, :], 
                          envelope[-1, :, :, :, :, :, :], atol=1e-6), \
    ...
    def _compute_spatial_spectrum(self, envelope: np.ndarray, domain: Domain) -> np.ndarray:
        """Compute spatial spectrum."""
        # Take FFT in spatial dimensions
        spatial_fft = np.fft.fftn(envelope, axes=(0, 1, 2))
        return np.abs(spatial_fft)**2

    def _compute_phase_spectrum(self, envelope: np.ndarray, domain: Domain) -> np.ndarray:
