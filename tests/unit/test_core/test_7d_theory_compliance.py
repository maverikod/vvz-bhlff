...
class Test7DTheoryCompliance:
    ...
    def test_no_mass_terms_in_lagrangian(self):
        ...
        if file_path.exists():
            ...
            assert "k0**2 * np.abs(a) ** 2" not in content, \
                "Energy computer should not contain k₀²|a|² mass term"
            
            assert "no mass term" in content.lower(), \
                    ...
                    if pattern in content:
                        ...
                        for line in lines:
                            if pattern in line and not line.strip().startswith('#'):
                                assert "step resonator" in content.lower(), \
    ...
    def test_7d_space_time_structure(self):
        ...
        if file_path.exists():
            ...
            assert "spatial" in content and "phase" in content, \
    ...
    def test_quench_dynamics(self):
        ...
        if file_path.exists():
            ...
            assert "quench" in content.lower(), \
    ...
    def test_power_balance_postulate(self):
        ...
        file_path = Path("bhlff/core/bvp/postulates/power_balance/power_balance_postulate.py")
        if file_path.exists():
            content = file_path.read_text()
            
            # Check for power balance dynamics
            assert "power balance" in content.lower(), \
