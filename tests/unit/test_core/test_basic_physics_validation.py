...
class TestBasicPhysicsValidation:
    ...
    def test_domain_7d_structure_physics(self, domain_7d):
        ...
        assert domain_7d.shape == expected_shape, f"Wrong domain shape: {domain_7d.shape}"
    ...
    def test_gradient_physics(self, domain_7d):
        ...
        assert gradient.shape == expected_shape, f"Wrong gradient shape: {gradient.shape}"
    ...
    def test_laplacian_physics(self, domain_7d):
        ...
        assert laplacian.shape == expected_shape, f"Wrong Laplacian shape: {laplacian.shape}"
    ...
    def test_fft_energy_conservation_physics(self, domain_7d):
        ...
        spectral_energy = np.sum(np.abs(spectral_field)**2) / np.prod(test_field.shape)
    ...
    def _create_laplacian_test_field(self, domain: Domain) -> np.ndarray:
        ...
        X, Y, Z = np.meshgrid(x, y, z, indexing='ij')
        # Broadcast to all 7 dimensions
        field = (X**2 + Y**2 + Z**2)[:, :, :, np.newaxis, np.newaxis, np.newaxis, np.newaxis]
    ...
    def _create_phase_test_field(self, domain: Domain) -> np.ndarray:
        ...
        X, Y, Z = np.meshgrid(x, y, z, indexing='ij')
        # Broadcast to all 7 dimensions
        field = np.exp(1j * (X + Y + Z))[:, :, :, np.newaxis, np.newaxis, np.newaxis, np.newaxis]
