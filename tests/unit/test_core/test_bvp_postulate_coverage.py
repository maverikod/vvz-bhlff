...
class TestBVPPostulateCoverage:
    ...
    def test_quench_detector_numerical_stability(self):
        ...
        extreme_envelope = np.broadcast_to(extreme_envelope.reshape(-1, 1, 1, 1, 1, 1, 1), (8, 8, 8, 4, 4, 4, 8))
        
        quenches = detector.detect_quenches(extreme_envelope)
        assert isinstance(quenches, dict)
        assert np.isfinite(quenches['quench_locations']).all() if len(quenches['quench_locations']) > 0 else True
