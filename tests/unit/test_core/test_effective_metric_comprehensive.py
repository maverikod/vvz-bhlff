...
class TestEffectiveMetricComprehensive:
    ...
    def test_effective_metric_7d_structure(self):
        ...
        assert (
            "g_eff = np.zeros((7, 7))" in content
        ), "Effective metric should be 7x7 matrix for 7D space-time"

        # Check for proper indexing
        assert "g_eff[0, 0]" in content, "Time component should be g_eff[0, 0]"

        assert (
            "g_eff[i, i]" in content
        ), "Spatial components should be g_eff[i, i] for i=1,2,3"

        assert (
            "g_eff[alpha, alpha]" in content
    ...
    def test_effective_metric_time_component(self):
        ...
        assert (
            "g_eff[0, 0] = -1.0 / (c_phi**2" in content
        ), "Time component should be g₀₀ = -1/c_φ²"

        assert "c_phi" in content, "Phase velocity c_φ should be used in time component"

        assert (
            "envelope_correction" in content
    ...
    def test_effective_metric_phase_components(self):
        ...
        assert (
            "for alpha in range(4, 7)" in content
    ...
    def test_anisotropic_metric_components(self):
        ...
        assert (
            "A_xx" in content and "A_yy" in content and "A_zz" in content
        ), "Anisotropic metric should have A_xx, A_yy, A_zz components"

        assert (
            "envelope_invariants" in content
                        ...
                        for line in lines:
                            if term in line and not line.strip().startswith("#"):
                                ...
                                if (
                                    ...
                                    or "envelope dynamics" in content
                                    or "phase field" in content
                                ):
                                    continue
                                else:
                                    assert (
    ...
    def test_effective_metric_physical_meaning(self):
        ...
        assert (
            "no spacetime curvature" in content
            or "not from spacetime curvature" in content
        ), "Effective metric should not use spacetime curvature"

        assert (
            "envelope invariants" in content
