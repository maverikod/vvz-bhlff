...
class TestEnhancedBlockProcessor:
    ...
    def test_cpu_processing(self):
        """Test CPU processing."""
        field = np.random.random((8, 8, 8, 8, 8, 8, 8)) + 1j * np.random.random(
            (8, 8, 8, 8, 8, 8, 8)
        )
...
class TestBVPBlockProcessingSystem:
    ...
    def test_envelope_solution(self):
        ...
        # Create test source
        source = np.random.random(self.domain.shape) + 1j * np.random.random(
            self.domain.shape
        )

        # Solve envelope equation
        envelope = self.bvp_processor.solve_envelope_blocked(
            source, max_iterations=5, tolerance=1e-4  # Reduced for testing
        )
    ...
    def test_block_extraction(self):
        ...
        for (
            block_data,
            block_info,
        ) in self.bvp_processor.block_processor.base_processor.iterate_blocks():
            source_block = self.bvp_processor._extract_source_block(field, block_info)
            assert source_block.shape == block_data.shape

            envelope_block = self.bvp_processor._extract_envelope_block(
                field, block_info
            )
    ...
    def test_block_bvp_solution(self):
        ...
        # Create test data
        envelope_block = np.random.random(
            (4, 4, 4, 4, 4, 4, 4)
        ) + 1j * np.random.random((4, 4, 4, 4, 4, 4, 4))
        source_block = np.random.random((4, 4, 4, 4, 4, 4, 4)) + 1j * np.random.random(
            (4, 4, 4, 4, 4, 4, 4)
        )
        ...
        # Solve BVP for block
        solution = self.bvp_processor._solve_block_bvp(
...
class TestBlockProcessingIntegration:
    ...
    def test_memory_efficiency(self):
        ...
        source = np.random.random(large_domain.shape) + 1j * np.random.random(
            large_domain.shape
        )
    ...
    def test_adaptive_processing(self):
        ...
        for size in [16, 32, 64]:
            ...
            source = np.random.random(domain.shape) + 1j * np.random.random(
                domain.shape
            )
