...
class TestEnvelopeMetricIntegration:
    ...
    def test_envelope_curvature_metric_implementation(self):
        ...
        assert "def compute_envelope_curvature_metric" in content, \
        ...
        assert "np.gradient" in method_content or "phase_gradients" in method_content, \
            "compute_envelope_curvature_metric should compute phase field gradients"
        
        assert "g_eff" in method_content, \
    ...
    def test_anisotropic_metric_implementation(self):
        ...
        assert "def compute_anisotropic_metric" in content, \
        ...
        assert "A_xx" in method_content or "A_yy" in method_content or "A_zz" in method_content, \
    ...
    def test_integration_in_gravity_curvature_needed(self):
        ...
        has_envelope_metric_usage = "envelope_metric" in content or "effective_metric" in content
    ...
    def test_integration_in_gravity_einstein_needed(self):
        ...
        has_envelope_metric_usage = "envelope_metric" in content or "compute_envelope" in content
    ...
    def test_effective_metric_7d_structure(self):
        ...
        assert "(7, 7)" in content, \
            "Effective metric should be 7x7 for 7D BVP theory"
        
        # Check for proper component assignments
        assert "g_eff[0, 0]" in content, \
            "Time component g00 should be assigned"
        
        # Check for spatial components
        assert "range(1, 4)" in content or "g_eff[1, 1]" in content, \
            "Spatial components gij should be assigned"
        
        # Check for phase components
        assert "range(4, 7)" in content or "g_eff[4, 4]" in content, \
