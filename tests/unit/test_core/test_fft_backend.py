...
class TestFFTBackend:
    ...
    def test_fft_backend_inverse_transform(self, fft_backend):
        ...
        spectral_field = np.random.random(
            fft_backend.domain.shape
        ) + 1j * np.random.random(fft_backend.domain.shape)
    ...
    def test_fft_backend_numerical_stability(self, fft_backend):
        ...
        field = np.broadcast_to(
            field.reshape(-1, 1, 1, 1, 1, 1, 1), fft_backend.domain.shape
        )
