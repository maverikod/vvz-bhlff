...
class TestFFTPhysics:
    ...
    def test_fft_7d_structure_physics(self, domain_7d, spectral_ops):
        ...
        assert spectral_field.shape == domain_7d.shape, \
    ...
    def test_fft_numerical_stability_physics(self, domain_7d, spectral_ops):
        ...
        assert np.isfinite(spectral_field).all(), \
            "FFT not numerically stable for extreme values"
        
        # Inverse FFT
        reconstructed_field = spectral_ops.fft_backend.inverse_transform(spectral_field)
        
        # Should be stable
        assert np.isfinite(reconstructed_field).all(), \
            "Inverse FFT not numerically stable for extreme values"
    ...
    def test_fft_boundary_conditions_physics(self, domain_7d, spectral_ops):
        ...
        assert np.isfinite(spectral_field).all(), \
            "FFT does not handle boundary conditions correctly"
