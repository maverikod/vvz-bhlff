...
class TestMemoryKernel:
    ...
    def test_memory_evolution(self, memory_kernel, domain_7d):
        ...
        for i, (initial, current) in enumerate(
            zip(initial_memory, memory_kernel.memory_variables)
        ):
            assert not np.allclose(
                initial, current
            ), f"Memory variable {i} did not evolve"
    ...
    def test_relaxation_times(self, memory_kernel):
        ...
        for i in range(len(memory_kernel.relaxation_times) - 1):
            assert (
                memory_kernel.relaxation_times[i]
                >= memory_kernel.relaxation_times[i + 1]
    ...
    def test_coupling_strengths(self, memory_kernel):
        ...
        assert any(
            strength != 0 for strength in memory_kernel.coupling_strengths
