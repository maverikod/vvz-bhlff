...
class TestNoExponentialDamping:
    ...
    def test_no_exponential_memory_kernel(self):
        ...
        assert "_setup_exponential_kernel" not in content, "Found exponential kernel method"
        assert "np.exp(-r / length_scale)" not in content, "Found exponential decay formula"
        ...
        assert "no exponential attenuation" in content, "Missing comment about removed exponential decay"
    ...
    def test_no_exponential_cosmological_expansion(self):
        ...
        assert "No exponential decay" in content, "Missing comment about removed exponential decay"
    ...
    def test_no_exponential_damping_in_collective(self):
        ...
        assert "exponential decay" not in content.lower(), "Found exponential decay analysis"
    ...
    def test_no_exponential_damping_in_nonlinear(self):
        ...
        # Check that exponential damping is not used
        assert "gamma" not in content.lower() or "NO DAMPING" in content, "Found damping coefficient gamma"
        assert "np.exp(-gamma" not in content, "Found exponential damping with gamma"
    ...
    def test_no_exponential_damping_in_phase_mapping(self):
        ...
        # Check that exponential damping is not used
        assert "np.exp(-laplacian_operator * dt)" not in content, "Found exponential time evolution"
        assert "exponential decay" not in content.lower(), "Found exponential decay"
    ...
    def test_step_resonator_boundary_implementation(self):
        ...
        assert "semi-transparent" in content.lower(), "Missing semi-transparent description"
    ...
    def test_no_exponential_damping_in_sources(self):
        ...
        content_without_carrier = content.replace("carrier = np.exp(1j * carrier_phase)", "").replace("exp(iω₀t)", "")
        assert "exponential" not in content_without_carrier.lower() or "envelope_type" in content or "exponential envelope" in content, "Found exponential decay in sources"
    ...
    def test_memory_kernel_uses_resonator_model(self):
        ...
        content_without_comments = content.replace("no exponential decay", "").replace("exponential decay", "")
        assert "exponential decay" not in content_without_comments.lower(), "Found exponential decay description"
    ...
    def test_no_exponential_damping_in_defect_interactions(self):
        ...
        assert "np.exp(-r * self.screening_factor)" not in content or "diagnostic only" in content, "Found exponential screening in base regime"
        # Allow "exponential decay" in comments about removal, but not in actual code
        content_without_comments = content.replace("no exponential decay", "").replace("exponential decay", "")
        assert "exponential decay" not in content_without_comments.lower(), "Found exponential decay"
    ...
    def test_no_exponential_envelope_factor_in_bvp_integrator(self):
        ...
        assert "envelope_factor = np.exp(" not in content, "Found exponential envelope_factor"
        # Allow complex exponential for carrier modulation (np.exp(1j * ...))
        content_without_carrier = content.replace("carrier_modulation = np.exp(1j * carrier_frequency * t)", "")
        assert "np.exp(" not in content_without_carrier, "Found exponential function in envelope integrator"
        ...
        assert "No exponential attenuation" in content, "Missing comment about removed exponential attenuation"