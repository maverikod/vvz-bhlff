...
class TestNoMassTerms:
    ...
    def test_no_k0_mass_terms_in_energy_computer(self):
        ...
        ), "Found k₀²|a|² mass term in energy computer"
        assert (
            "k0**2 * np.abs(a)**2" not in content
        ), "Found k₀²|a|² mass term in energy computer"
        assert (
            "(mass term)" not in content
        ), "Found mass term comment in energy computer"
    ...
    def test_no_lambda_mass_terms_in_abstract_solver(self):
        ...
        assert "λ⟨a,a⟩" not in content.replace(
            "No mass term λ⟨a,a⟩", ""
        ), "Found classical λ⟨a,a⟩ mass term"
        assert (
            "lambda_param * np.sum(field**2)" not in content
        ), "Found classical mass term"
    ...
    def test_no_mass_attribute_in_particle(self):
        ...
        assert (
            "mass: float = 1.0" not in content
        ), "Found mass attribute in Particle class"
        assert "mass: float" not in content, "Found mass attribute in Particle class"
        assert (
            "No mass attribute" in content
        ), "Missing comment about removed mass attribute"
    ...
    def test_no_phase_mass_in_cosmology(self):
        ...
        assert (
            "phase_mass = " not in content
        ), "Found phase_mass assignment in cosmology"
    ...
    def test_no_phase_mass_in_evolution(self):
        ...
        assert (
            "phase_mass = " not in content
        ), "Found phase_mass assignment in evolution"
        assert "No phase_mass" in content, "Missing comment about removed phase_mass"
    ...
    def test_energy_functional_uses_derivatives(self):
        ...
        content_without_comments = content.replace(
            "No mass term k₀²|a|² - removed according to 7D BVP theory", ""
        ).replace("(no mass term)", "")
    ...
    def test_energy_computation_uses_gradients(self):
        ...
        assert "λ⟨a,a⟩" not in content.replace(
            "No mass term λ⟨a,a⟩", ""
        ), "Found classical mass term notation"
        ...
        for node in ast.walk(tree):
            if isinstance(node, ast.FunctionDef):
                if node.name == "compute_energy_density":
                    ...
                    assert (
                        "k0**2" not in function_source
                    ), "Found k₀² mass term in energy density"
                    assert (
                        "np.abs(a) ** 2" not in function_source
                    ), "Found |a|² mass term"
                    assert "mass term" not in function_source, "Found mass term comment"

                    # Check that derivative terms are present
                    assert (
                        "grad_spatial_sq" in function_source
