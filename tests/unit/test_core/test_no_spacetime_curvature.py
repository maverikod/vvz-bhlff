...
class TestNoSpacetimeCurvature:
                    ...
                    if term in content:
                        ...
                        for line in lines:
                            if term in line and not line.strip().startswith("#"):
                                ...
                                if (
                                    "phase space-time" in content
                                    or "M₇" in content
                                    or "VBP envelope" in content
                                ):
                                    continue
                                else:
                                    assert (
                                        False
                                    ), f"Found classical term '{term}' in {file_path}, should use phase space-time M₇"
    ...
    def test_7d_phase_space_time_structure(self):
        ...
        if file_path.exists():
            ...
            assert "7D" in content, "Should use 7D structure"

            # Check for phase space-time
            assert (
                "phase space-time" in content or "M₇" in content
    ...
    def test_no_riemann_tensor(self):
        ...
        riemann_terms = ["Riemann tensor", "Riemann curvature", "Riemannian geometry"]
                ...
                for term in einstein_terms:
                    if term in content:
                        ...
                        for line in lines:
                            if term in line and not line.strip().startswith("#"):
                                assert (
                                    "phase envelope balance" in content
    ...
    def test_no_ricci_tensor(self):
        ...
        ricci_terms = ["Ricci tensor", "Ricci curvature", "Ricci scalar"]
    ...
    def test_phase_space_time_coordinates(self):
        ...
        if file_path.exists():
            ...
            assert "phase" in content, "Domain should distinguish phase coordinates"
                ...
                for term in classical_gravity_terms:
                    if term in content:
                        ...
                        for line in lines:
                            if term in line and not line.strip().startswith("#"):
                                assert (
                                    "envelope" in content or "VBP" in content
