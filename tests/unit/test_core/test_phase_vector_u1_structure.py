    ...
    def test_topological_charge_computation(self, phase_vector):
        ...
        if phase_vector.domain.dimensions >= 1:
            ...
            if phase_vector.domain.dimensions == 7:
                # For 7D: ℝ³ₓ × 𝕋³_φ × ℝₜ
                x = np.linspace(-phase_vector.domain.L/2, phase_vector.domain.L/2, phase_vector.domain.N)
                # Apply phase winding only to first spatial dimension
                envelope = envelope * np.exp(1j * 2 * np.pi * x[:, None, None, None, None, None, None] / phase_vector.domain.L)
            else:
                x = np.linspace(-phase_vector.domain.L/2, phase_vector.domain.L/2, phase_vector.domain.N)
