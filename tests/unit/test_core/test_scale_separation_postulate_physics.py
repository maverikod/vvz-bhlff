...
class TestScaleSeparationPostulatePhysics:
    ...
    def test_scale_separation_postulate_physics(
        ...
        assert (
            scale_ratio > 1.0
        ), f"Scale separation insufficient: ratio = {scale_ratio}"
        ...
        ), f"Carrier wavelength too large: {carrier_wavelength}"

        # Physical validation 4: Envelope wavelength should be large
        envelope_wavelength = result["envelope_wavelength"]
        assert (
            envelope_wavelength > 1.0
        ), f"Envelope wavelength too small: {envelope_wavelength}"
