...
Fast tests for simple block processing system.

This module tests the simple block processing system for 7D BVP computations
...
from bhlff.core.domain.simple_block_processor import SimpleBlockProcessor, SimpleConfig, ProcessingMode
from bhlff.core.domain import Domain


class TestSimpleBlockProcessing:
    """Test simple block processor functionality."""
    ...
    def test_block_processing_operations(self):
        ...
        assert fft_time < 1.0  # Should be fast
        ...
        assert ifft_time < 1.0  # Should be fast
        ...
        assert bvp_time < 1.0  # Should be fast
    
    def test_memory_efficiency(self):
        """Test memory efficiency of block processing."""
        # Create very small field to avoid hanging
        field = np.random.random((2, 2, 2, 2, 2, 2, 2)) + 1j * np.random.random((2, 2, 2, 2, 2, 2, 2))
    ...
    def test_adaptive_processing(self):
        ...
        # Test with very small field sizes only to avoid hanging
        for size in [2]:  # Only smallest size
            domain = Domain(L=1.0, N=size, dimensions=7)
            config = SimpleConfig(
                block_size=2,  # Fixed small block size
                overlap_ratio=0.1,
                max_memory_usage=0.7,
                enable_adaptive_sizing=True,
                enable_memory_optimization=True,
    ...
    def test_performance_benchmark(self):
        ...
        assert result.shape == field.shape
        assert total_time < 3.0  # Should be fast for 3 iterations
