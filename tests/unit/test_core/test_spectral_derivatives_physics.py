...
class TestSpectralDerivativesPhysics:
    ...
    def test_spectral_derivatives_physics(self, domain_7d, spectral_derivs):
        ...
        assert np.isfinite(derivative).all(), \
            "Spectral derivative not finite"
        
        # Should preserve field shape
        assert derivative.shape == test_field.shape, \
            "Spectral derivative does not preserve field shape"

    def test_spectral_derivatives_energy_conservation_physics(self, domain_7d, spectral_derivs):
        """
        ...
        assert np.isfinite(derivative_energy), \
            "Derivative energy not finite"

    def test_spectral_derivatives_7d_structure_physics(self, domain_7d, spectral_derivs):
        """
        ...
        for axis in range(7):
            derivative = spectral_derivs.compute_derivative(test_field, axis=axis, order=1)
            
            # Physical validation: Should preserve 7D structure
            assert derivative.shape == domain_7d.shape, \
                f"Spectral derivative does not preserve 7D structure for axis {axis}"

    def test_spectral_derivatives_numerical_stability_physics(self, domain_7d, spectral_derivs):
        """
        ...
        assert np.isfinite(derivative).all(), \
            "Spectral derivative not numerically stable for extreme values"
    ...
    def test_spectral_derivatives_precision_physics(self, domain_7d, spectral_derivs):
        ...
        # Physical validation: Should be finite and reasonable
        assert np.isfinite(derivative).all(), \
            "Spectral derivative not finite for sinusoidal function"
        
        # Should be reasonable magnitude
        max_derivative = np.max(np.abs(derivative))
        assert max_derivative < 10.0, \
            f"Spectral derivative magnitude too large: {max_derivative}"

    def test_spectral_derivatives_boundary_conditions_physics(self, domain_7d, spectral_derivs):
        """
        ...
        assert np.isfinite(derivative).all(), \
            "Spectral derivative does not handle boundary conditions correctly"

    def test_spectral_derivatives_phase_structure_physics(self, domain_7d, spectral_derivs):
        """
        ...
        # Should be finite
        assert np.isfinite(derivative).all(), \
            "Spectral derivative not finite for complex phase structure"

    def test_spectral_derivatives_mixed_derivatives_physics(self, domain_7d, spectral_derivs):
        """
        ...
        # Physical validation: Should be finite and reasonable
        assert np.isfinite(mixed_derivative).all(), \
            "Mixed spectral derivative not finite"
        
        # Should preserve field shape
        assert mixed_derivative.shape == test_field.shape, \
            "Mixed spectral derivative does not preserve field shape"

    def test_spectral_derivatives_higher_order_physics(self, domain_7d, spectral_derivs):
        """
        ...
        for order in [1, 2, 3, 4]:
            derivative = spectral_derivs.compute_derivative(test_field, axis=0, order=order)
            
            # Physical validation: Should be finite and reasonable
            assert np.isfinite(derivative).all(), \
                f"Spectral derivative order {order} not finite"
