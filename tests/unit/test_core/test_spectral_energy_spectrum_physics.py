...
class TestSpectralEnergySpectrumPhysics:
    ...
    def test_spectral_energy_spectrum_physics(self, domain_7d, spectral_ops):
        ...
        assert np.isfinite(energy_spectrum).all(), "Spectral energy spectrum not finite"
        assert np.all(energy_spectrum >= 0), "Spectral energy spectrum not positive"

    def test_spectral_energy_spectrum_energy_conservation_physics(
        ...
        assert np.isfinite(total_energy), "Total spectral energy not finite"
    ...
    def test_spectral_energy_spectrum_7d_structure_physics(
        self, domain_7d, spectral_ops
    ):
        """
        ...
        assert (
            energy_spectrum.shape == domain_7d.shape
        ), "Spectral energy spectrum does not preserve 7D structure"

    def test_spectral_energy_spectrum_numerical_stability_physics(
        self, domain_7d, spectral_ops
    ):
        """
        ...
        assert np.isfinite(
            energy_spectrum
        ).all(), "Spectral energy spectrum not numerically stable for extreme values"
    ...
    def test_spectral_energy_spectrum_precision_physics(self, domain_7d, spectral_ops):
        ...
        assert np.isfinite(
            energy_spectrum
        ).all(), "Spectral energy spectrum not finite for sinusoidal function"
    ...
    def test_spectral_energy_spectrum_phase_structure_physics(
        self, domain_7d, spectral_ops
    ):
        """
        ...
        assert np.isfinite(
            energy_spectrum
        ).all(), "Spectral energy spectrum not finite for complex phase structure"

    def test_spectral_energy_spectrum_frequency_dependence_physics(
        self, domain_7d, spectral_ops
    ):
        """
        ...
        assert np.isfinite(
            energy_spectrum
        ).all(), "Spectral energy spectrum not finite for frequency test field"

        # Should have reasonable frequency content
        max_energy = np.max(energy_spectrum)
        assert max_energy > 0, "Spectral energy spectrum has no frequency content"

    def test_spectral_energy_spectrum_resolution_dependence_physics(
        self, domain_7d, spectral_ops
    ):
        """
        ...
        for N in resolutions:
            ...
            test_domain = Domain(L=1.0, N=N, dimensions=7, N_phi=4, N_t=8, T=1.0)
            ...
            # Compute spectral energy spectrum
            energy_spectrum = self._compute_energy_spectrum(
                test_field, test_spectral_ops
            )

            # Physical validation: Should be finite for all resolutions
            assert np.isfinite(
                energy_spectrum
            ).all(), f"Spectral energy spectrum not finite for resolution N={N}"

    def test_spectral_energy_spectrum_energy_distribution_physics(
        self, domain_7d, spectral_ops
    ):
        """
        ...
        assert total_energy > 0, "Spectral energy spectrum has no total energy"
        assert max_energy > 0, "Spectral energy spectrum has no maximum energy"
    ...
    def test_spectral_energy_spectrum_spectral_properties_physics(
        self, domain_7d, spectral_ops
    ):
        """
    ...
    def _compute_energy_spectrum(
        self, field: np.ndarray, spectral_ops: SpectralOperations
    ) -> np.ndarray:
        """Compute spectral energy spectrum."""
