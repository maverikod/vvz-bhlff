...
class TestSpectralOperations:
    ...
    def test_spectral_ops_numerical_stability(self, spectral_ops):
        ...
        field = np.broadcast_to(field.reshape(-1, 1, 1, 1, 1, 1, 1), spectral_ops.fft_backend.domain.shape)
    ...
    def test_spectral_ops_precision(self, spectral_ops):
        ...
        x = np.linspace(0, 2*np.pi, spectral_ops.fft_backend.domain.shape[0], endpoint=False)
        field = np.sin(x)
        field = np.broadcast_to(field.reshape(-1, 1, 1, 1, 1, 1, 1), spectral_ops.fft_backend.domain.shape)
