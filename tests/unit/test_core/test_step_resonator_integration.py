...
class TestStepResonatorIntegration:
    ...
    def test_cascade_filter_creation(self):
        ...
        filtered_field = cascade_filter.apply_cascade_filter(field, frequencies, axes=(0, 1))
    ...
    def test_resonator_integration_in_bvp_integrator(self):
        ...
        assert "FrequencyDependentResonator" in content, "Missing FrequencyDependentResonator import"
        assert "CascadeResonatorFilter" in content, "Missing CascadeResonatorFilter import"
        assert "_resonator" in content, "Missing resonator initialization"
        assert "compute_coefficients" in content, "Missing frequency-dependent coefficient computation"
        ...
        for i, line in enumerate(lines):
            if 'np.exp(' in line and 'freq_factor' not in line:
                ...
                if 'compute_coefficients' not in lines[max(0, i-5):i+5]:
                    assert False, f"Found unexpected np.exp usage in line {i+1}: {line.strip()}"
