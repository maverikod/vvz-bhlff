...
class TestA01PlaneWave:
    ...
    def test_anisotropy_check(self):
        ...
        assert anisotropy <= self.tolerance_anisotropy, \
    ...
    def test_solver_validation(self):
        ...
        print(f"Test A0.1.7: Solver validation - Residual: {residual_norm:.2e}, Error: {relative_error:.2e}")
    ...
    def test_solver_info(self):
        ...
        required_fields = ['domain_shape', 'parameters', 'spectral_coefficients_computed', 
                          'fft_plan_setup', 'solver_type']
        
        for field in required_fields:
            assert field in solver_info, \
                f"Solver info should include {field}"
        
        # Check values
        assert solver_info['domain_shape'] == self.domain.shape, \
            "Domain shape mismatch"
        
        assert solver_info['solver_type'] == 'basic', \
            "Solver type should be 'basic'"
        
        assert solver_info['spectral_coefficients_computed'], \
            "Spectral coefficients should be computed"
        
        assert solver_info['fft_plan_setup'], \
            "FFT plan should be setup"
