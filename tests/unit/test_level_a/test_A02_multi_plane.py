...
class TestA02MultiPlane:
    ...
    def create_multi_frequency_source(self, modes: List[List[int]], 
    ...
    def compute_analytical_solution(self, modes: List[List[int]], 
                                  amplitudes: List[complex]) -> np.ndarray:
        """
    ...
    def test_superposition_principle(self):
        ...
        amplitudes = [complex(np.random.randn(), np.random.randn()) for _ in range(self.n_modes)]
        ...
        # Check tolerance
        assert relative_error_L2 <= self.tolerance_L2, \
            f"L2 error {relative_error_L2:.2e} exceeds tolerance {self.tolerance_L2:.2e}"
        
        print(f"Test A0.2.1: Superposition principle - L2 error: {relative_error_L2:.2e}")
    ...
    def test_individual_mode_solutions(self):
        ...
        print(f"Test A0.2.2: Individual mode superposition - L2 error: {relative_error_L2:.2e}")
    ...
    def test_aliasing_detection(self):
        ...
        high_freq_modes = [[self.N//4, 0, 0], [0, self.N//4, 0], [0, 0, self.N//4]]
        ...
        source_high = self.create_multi_frequency_source(high_freq_modes, high_amplitudes)
        ...
        print(f"Test A0.2.3: Aliasing detection - Low energy: {low_energy:.2e}, High energy: {high_energy:.2e}")
    ...
    def test_spectral_analysis(self):
        ...
        peak_indices = np.unravel_index(np.argmax(np.abs(solution_fft)), solution_fft.shape)
        ...
        assert peak_indices == expected_peak, \
        ...
        print(f"Test A0.2.4: Spectral analysis - Peak amplitude error: {relative_error:.2e}")
    ...
    def test_energy_conservation(self):
        ...
        assert solution_energy < source_energy, \
        ...
        expected_ratio = 1.0 / (self.mu + self.lambda_param)  # Approximate for low frequencies
        ...
        print(f"Test A0.2.7: Energy conservation - Energy ratio: {energy_ratio:.2e}, Expected: {expected_ratio:.2e}")
