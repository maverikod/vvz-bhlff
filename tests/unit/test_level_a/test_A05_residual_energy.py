...
class TestA05ResidualEnergy:
    ...
    def create_multi_frequency_source(self, modes: list, amplitudes: list) -> np.ndarray:
        """
    ...
    def test_residual_computation(self):
        ...
        assert relative_residual <= self.tolerance_residual, \
            f"Relative residual {relative_residual:.2e} exceeds tolerance {self.tolerance_residual:.2e}"
        
        print(f"Test A0.5.1: Residual computation - Relative residual: {relative_residual:.2e}")
    ...
    def test_residual_orthogonality(self):
        ...
        assert normalized_orthogonality <= self.tolerance_orthogonality, \
            f"Orthogonality {normalized_orthogonality:.2e} exceeds tolerance {self.tolerance_orthogonality:.2e}"
        
        print(f"Test A0.5.2: Residual orthogonality - Normalized orthogonality: {normalized_orthogonality:.2e}")
    ...
    def test_energy_balance(self):
        ...
        print(f"Test A0.5.3: Energy balance - Energy ratio: {energy_ratio:.2e}, Expected: {expected_ratio:.2e}")
    ...
    def test_multi_frequency_residual(self):
        ...
        print(f"Test A0.5.4: Multi-frequency residual - Relative residual: {relative_residual:.2e}")
    ...
    def test_residual_spectral_analysis(self):
        ...
        assert relative_max_residual <= self.tolerance_residual, \
            f"Max residual amplitude {relative_max_residual:.2e} exceeds tolerance {self.tolerance_residual:.2e}"
        
        print(f"Test A0.5.5: Residual spectral analysis - Max relative residual: {relative_max_residual:.2e}")
    ...
    def test_solver_validation(self):
        ...
        assert 'residual_norm' in validation_results, \
            "Validation should include residual norm"
        
        assert 'relative_error' in validation_results, \
        ...
        # Check consistency
        residual_consistency = abs(residual_norm - manual_residual_norm) / manual_residual_norm
        error_consistency = abs(relative_error - manual_relative_error) / manual_relative_error
        
        assert residual_consistency <= 1e-6, \
            f"Residual norm consistency error {residual_consistency:.2e} exceeds tolerance"
        
        assert error_consistency <= 1e-6, \
            f"Relative error consistency error {error_consistency:.2e} exceeds tolerance"
        
        print(f"Test A0.5.7: Solver validation - Residual consistency: {residual_consistency:.2e}, Error consistency: {error_consistency:.2e}")
    ...
    def test_energy_conservation(self):
        ...
        relative_error = abs(actual_energy_ratio - expected_energy_ratio) / expected_energy_ratio
        
        assert relative_error <= 0.1, \
            f"Energy conservation error {relative_error:.2e} exceeds tolerance"
        
        print(f"Test A0.5.8: Energy conservation - Energy ratio: {actual_energy_ratio:.2e}, Expected: {expected_energy_ratio:.2e}")
