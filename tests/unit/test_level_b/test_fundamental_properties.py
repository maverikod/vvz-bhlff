...
class LevelBFundamentalPropertiesTests:
    ...
    def _create_test_solution(
        self, domain: Domain, center: List[float], parameters: Parameters
    ) -> np.ndarray:
        """
    ...
    def test_power_law_tail(self):
        ...
        domain = Domain(
            L=config["domain"]["L"], N=config["domain"]["N"], N_phi=4, N_t=8, T=1.0
        )
        ...
        solution = self._create_test_solution(
            domain, config["source"]["center"], parameters
        )
        ...
        # Check criteria
        passed = (
            analysis_result["r_squared"] >= config["analysis"]["r_squared_threshold"]
            and analysis_result["relative_error"]
            <= config["analysis"]["error_threshold"]
            and analysis_result["log_range"] >= config["analysis"]["min_decades"]
        )

        return {"passed": passed, "analysis_result": analysis_result, "config": config}
    ...
    def test_no_spherical_nodes(self) -> Dict[str, Any]:
        ...
        # Check criteria
        passed = (
            analysis_result["sign_changes"] <= config["analysis"]["max_sign_changes"]
            and not analysis_result["periodic_zeros"]
            and analysis_result["is_monotonic"]
        )
    ...
    def test_topological_charge(self) -> Dict[str, Any]:
        ...
        return {"passed": passed, "charge_result": charge_result, "config": config}
    ...
    def test_zone_separation(self) -> Dict[str, Any]:
        ...
        # Check criteria
        passed = (
            zone_result["r_core"] > 0
            and zone_result["r_tail"] > zone_result["r_core"]
            and zone_result["zone_stats"]["core"]["volume_fraction"] > 0
            and zone_result["zone_stats"]["tail"]["volume_fraction"] > 0
        )

        return {"passed": passed, "zone_result": zone_result, "config": config}
