...
class TestSimpleLevelB(unittest.TestCase):
    ...
    def _create_test_solution(self, beta: float) -> np.ndarray:
        ...
        distances = np.sqrt((X - self.center[0])**2 + (Y - self.center[1])**2 + (Z - self.center[2])**2)
    ...
    def test_power_law_analysis_basic(self):
        ...
        result = self.analyzer.analyze_power_law_tail(solution, 1.0, self.center, min_decades=0.5)
    ...
    def test_power_law_different_beta(self):
        ...
        for beta in beta_values:
            with self.subTest(beta=beta):
                ...
                # Run power law analysis with relaxed requirements
                result = self.analyzer.analyze_power_law_tail(solution, beta, self.center, min_decades=0.5)
                
                # Check that theoretical slope is correct
                expected_slope = 2 * beta - 3
                self.assertAlmostEqual(result['theoretical_slope'], expected_slope, places=10)
                
                print(f"β={beta}: slope={result['slope']:.3f}, theoretical={result['theoretical_slope']:.3f}")
    ...
    def test_radial_profile_computation(self):
        ...
        if len(increasing_r) > 1:
            ...
            self.assertGreater(len(decreasing_amp), 0, "Amplitude should generally decrease with radius")
