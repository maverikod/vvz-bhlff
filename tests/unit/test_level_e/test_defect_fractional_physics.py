...
class TestDefectFractionalGreenFunction:
    ...
    def test_fractional_green_normalization_beta_05(
        self, domain_3d, physics_params_beta_05
    ):
        ...
        for r in r_values:
            ...
            assert (
                green_gradient < 0
            ), f"Green function gradient should be negative at r={r}"
            ...
            if r > 1.0:  # Avoid small r where cutoff affects behavior
                ...
                assert (
                    1e-6 < ratio < 1e6
                ), f"Green function should scale as r^{expected_power} at r={r}, ratio={ratio}"

    def test_fractional_green_normalization_beta_1(
        self, domain_3d, physics_params_beta_1
    ):
        ...
        for r in r_values:
            ...
            if r > 1.0:  # Avoid small r where cutoff affects behavior
                ...
                assert (
                    1e-6 < ratio < 1e6
                ), f"Green function should scale as 1/r at r={r}, ratio={ratio}"

    def test_energy_monotonicity_under_approach(
        self, domain_3d, physics_params_beta_05
    ):
        ...
        for i in range(1, len(energies)):
            ...
            assert (
                delta_energy <= 0
            ), f"Energy should decrease under approach: ΔE={delta_energy} at separation {separations[i]}"

        # Check that energy becomes more negative (more bound) as defects approach
        assert (
            energies[-1] < energies[0]
        ), "Final energy should be more negative than initial energy"
    ...
    def test_no_mass_term_in_base_regime(self, domain_3d, physics_params_beta_05):
        ...
        assert (
            interactions.screening_factor == 0.0
        ), "Base regime should have no screening"
        ...
        ratios = [
            green_values[i] / green_values[i - 1] for i in range(1, len(green_values))
        ]
        ...
        for i, (ratio, r_ratio) in enumerate(zip(ratios, r_ratios)):
            expected_ratio = r_ratio**2
            assert (
                abs(ratio - expected_ratio) / expected_ratio < 0.2
            ), f"Decay should be power-law, not exponential at r={r_values[i+1]}"

    def test_defect_analyzer_fractional_interactions(
        self, domain_3d, physics_params_beta_05
    ):
        ...
        ), "Opposite charges should have attractive interaction"
    ...
    def test_fractional_green_normalization_consistency(self, domain_3d):
        ...
        for beta in beta_values:
            ...
            if beta < 1.5:
                assert (
                    green_gradient < 0
                ), f"Green function gradient should be negative for β={beta}"

            # Check that normalization is reasonable
            assert (
                1e-6 < green_value < 1e6
            ), f"Green function value should be reasonable for β={beta}"
    ...
    def test_annihilation_energy_fractional(self, domain_3d, physics_params_beta_05):
        ...
        ), "Closer defects should have higher annihilation energy"
