...
class TestDefectPhysics:
    ...
    def test_topological_charge_calculation(self, domain_7d, physics_params):
        ...
        assert (
            abs(calculated_charge - 1) < 0.1
        ), f"Charge should be 1, got {calculated_charge}"
    ...
    def test_defect_interaction_forces(self, domain_7d, physics_params):
        ...
        positions = [np.array([1.0, 2.0, 2.0]), np.array([3.0, 2.0, 2.0])]
        charges = [1, -1]
    ...
    def test_defect_annihilation_physics(self, domain_7d, physics_params):
        ...
        if result["annihilated"]:
            assert (
                result["annihilation_energy"] > 0
            ), "Annihilation should release energy"
            assert (
                result["energy_release_rate"] > 0
            ), "Energy release rate should be positive"
    ...
    def test_interaction_potential_setup(self, domain_7d, physics_params):
        ...
        ), "Should have interaction strength"
        assert hasattr(
            interactions, "interaction_range"
        ), "Should have interaction range"
        ...
        assert (
            interactions.interaction_strength > 0
        ), "Interaction strength should be positive"
        assert (
            interactions.interaction_range > 0
        ), "Interaction range should be positive"
    ...
    def test_green_function_physics(self, domain_7d, physics_params):
        ...
        for r in distances:
            ...
            assert np.isfinite(
                green_gradient
            ), f"Green gradient should be finite at r={r}"
    ...
    def test_defect_charge_conservation(self, domain_7d, physics_params):
        ...
        assert (
            abs(calculated_charge - charge) < 0.1
        ), f"Charge should be conserved: {charge} vs {calculated_charge}"
    ...
    def test_defect_energy_scaling(self, domain_7d, physics_params):
        ...
        for i in range(1, len(energies)):
            ...
            ), f"Energy should scale with strength: {ratio} vs {expected_ratio}"
