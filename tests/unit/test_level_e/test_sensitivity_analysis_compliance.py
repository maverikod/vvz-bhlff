...
class TestSensitivityAnalysisCompliance:
    ...
    def test_no_classical_mass_patterns(self):
        ...
        assert (
            "E_eff ~ ∫ [μ|∇a|² + |∇Θ|^(2β)] d³x d³φ dt" in method_content
    ...
    def test_7d_bvp_theory_compliance(self):
        ...
        assert (
            "In 7D BVP theory, mass is not a fundamental property" in content
        ), "Should explain 7D BVP mass concept"

        assert (
            "resistance to phase state rearrangement" in content
        ), "Should explain mass as resistance to phase changes"

        assert (
            "Field localization energy (μ|∇a|²)" in content
        ), "Should include localization energy in mass"

        assert (
            "Phase gradient energy (β-dependent terms)" in content
        ), "Should include phase gradient energy"

        assert (
            "Topological stability (winding numbers)" in content
    ...
    def test_sobol_indices_implementation(self):
        ...
        assert "interaction" in content, "Should compute interaction indices"
    ...
    def test_mass_complexity_correlation_analysis(self):
        ...
        assert (
            "correlation" in content
        ), "Should compute correlation between energy and complexity"
        assert "t_statistic" in content, "Should compute statistical significance"
        assert "p_value" in content, "Should compute p-value for significance test"
    ...
    def test_proper_7d_source_creation(self):
        ...
        assert "Gaussian" in method_content, "Source should use Gaussian localization"
