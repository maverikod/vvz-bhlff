...
def test_skyrme_gradient_descent_energy_nonincreasing():
    ...
    comp1 = (rng.standard_normal(domain.shape) + 1j * rng.standard_normal(domain.shape)).astype(np.complex128)
    comp2 = (rng.standard_normal(domain.shape) + 1j * rng.standard_normal(domain.shape)).astype(np.complex128)
    ...
    U21 = b
    U22 = np.conj(a)
    field = np.stack([np.stack([U11, U12], axis=-1), np.stack([U21, U22], axis=-1)], axis=-2).astype(np.complex128)
