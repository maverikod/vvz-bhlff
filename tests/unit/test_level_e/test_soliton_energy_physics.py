...
class TestSolitonEnergyPhysics:
    ...
    def test_kinetic_energy_physical_properties(
        self, domain_3d, physics_params, simple_u1_phase_field
    ):
        """
        ...
        kinetic_energy_static = soliton._energy_calculator.compute_kinetic_energy(
            static_field
        )

        # Static field should have zero kinetic energy
        assert (
            abs(kinetic_energy_static) < 1e-10
        ), f"Static field should have zero kinetic energy, got {kinetic_energy_static}"
        ...
        for i in range(N):
            for j in range(N):
                for k in range(N):
                    time_dependent_field[i, j, k] = simple_u1_phase_field[
                        i, j, k
                    ] * np.exp(1j * omega * t)

        kinetic_energy_dynamic = soliton._energy_calculator.compute_kinetic_energy(
            time_dependent_field
        )

        # Time-dependent field should have positive kinetic energy
        assert (
            kinetic_energy_dynamic > 0
        ), f"Time-dependent field should have positive kinetic energy, got {kinetic_energy_dynamic}"
    ...
    def test_skyrme_energy_physical_properties(
        ...
        assert (
            skyrme_energy >= 0
        ), f"Skyrme energy should be non-negative, got {skyrme_energy}"
        ...
        assert (
            skyrme_strong > skyrme_weak
        ), "Stronger coupling should give higher Skyrme energy"
    ...
    def test_energy_conservation_under_rotation(
        ...
        assert (
            abs(energy_original - energy_rotated) < 1e-6
        ), f"Energy should be conserved under rotation: {energy_original} vs {energy_rotated}"
    ...
    def test_energy_minimum_properties(self, domain_3d, physics_params):
        ...
        assert np.isfinite(
            energy_u1_phase
        ), "U(1)^3 phase field energy should be finite"
    ...
    def test_energy_gradient_properties(
        ...
        assert (
            gradient.shape == simple_u1_phase_field.shape
        ), "Gradient should have same shape as field"
    ...
    def test_energy_hessian_properties(
        ...
        assert (
            hessian_symmetry_error < 1e-6
        ), f"Hessian should be symmetric, max error: {hessian_symmetry_error}"
