...
class TestSolitonTopologyPhysics:
    ...
    def test_topological_charge_trivial_field(self, domain_3d, physics_params, trivial_field):
        """
    ...
    def test_topological_charge_conservation(self, domain_3d, physics_params, u1_phase_field_b1):
        ...
        for i in range(N):
            for j in range(N):
                for k in range(N):
                    ...
                    deformed_field[i, j, k] = deformed_field[i, j, k] * (1 + perturbation)
        ...
        assert charge_difference < 0.1, f"Topological charge should be conserved: {charge_original} vs {charge_deformed}"
    ...
    def test_topological_charge_precision(self, domain_3d, physics_params, u1_phase_field_b1):
        ...
        assert charge_std < 1e-6, f"Topological charge should be consistent, std: {charge_std}"
    ...
    def test_topological_charge_boundary_effects(self, physics_params):
        ...
        domain_periodic = Domain(L=4.0, N=64, dimensions=3, boundary_condition="periodic")
        domain_dirichlet = Domain(L=4.0, N=64, dimensions=3, boundary_condition="dirichlet")
        ...
        # Both charges should be finite
        assert np.isfinite(charge_periodic), "Charge should be finite for periodic boundary"
        assert np.isfinite(charge_dirichlet), "Charge should be finite for dirichlet boundary"
