...
class TestEnvelopeEffectiveMetric:
    ...
    def test_metric_units(self):
        ...
        assert all(isinstance(g[i, j], (int, float)) for i in range(7) for j in range(7))
    ...
    def test_metric_boundary_conditions(self):
        ...
        g_min = metric.compute_effective_metric_from_vbp_envelope({"chi_over_kappa": 0.001})
        g_max = metric.compute_effective_metric_from_vbp_envelope({"chi_over_kappa": 1000.0})
