...
class TestMLPredictionFullImplementation:
    ...
    def test_full_ml_frequency_prediction(self):
        ...
        with patch.object(self.prediction_core.feature_extractor, 'extract_frequency_features', return_value=features):
            with patch.object(self.prediction_core.feature_extractor, 'extract_7d_phase_features', return_value=np.array([0.5, 0.3, 0.4, 0.6, 0.7, 0.2])):
                with patch.object(self.prediction_core, '_load_trained_frequency_model', return_value=mock_model):
                    with patch.object(self.prediction_core, '_load_frequency_scaler', return_value=mock_scaler):
                        ...
                        self.prediction_core.feature_extractor.extract_frequency_features.assert_called_once_with(self.envelope)
                        # extract_7d_phase_features is called twice - once in ML path and once in analytical fallback
                        assert self.prediction_core.feature_extractor.extract_7d_phase_features.call_count >= 1
    ...
    def test_full_ml_coupling_prediction(self):
        ...
        # Mock model and scaler
        mock_model = Mock()
        mock_model.predict.return_value = np.array([[0.72, 0.64, 0.55, 0.36, 0.7, 0.945]])
        mock_scaler = Mock()
        mock_scaler.transform.return_value = np.array([[0.6, 0.8, 0.5, 0.4, 0.7, 0.9, 0.8, 0.3]])
        
        with patch.object(self.prediction_core.feature_extractor, 'extract_coupling_features', return_value=features):
            with patch.object(self.prediction_core.feature_extractor, 'extract_7d_phase_features', return_value=np.array([0.6, 0.8, 0.5, 0.4, 0.7, 0.9, 0.8, 0.3])):
                with patch.object(self.prediction_core, '_load_trained_coupling_model', return_value=mock_model):
                    with patch.object(self.prediction_core, '_load_coupling_scaler', return_value=mock_scaler):
                        # Test full ML prediction
                        result = self.prediction_core.predict_mode_coupling(self.envelope)
                        ...
                        # Verify 7D phase field features are used
                        self.prediction_core.feature_extractor.extract_coupling_features.assert_called_once_with(self.envelope)
                        # extract_7d_phase_features is called twice - once in ML path and once in analytical fallback
                        assert self.prediction_core.feature_extractor.extract_7d_phase_features.call_count >= 1
    ...
    def test_analytical_fallback_frequency_prediction(self):
        ...
        with patch.object(self.prediction_core.feature_extractor, 'extract_frequency_features', return_value=features):
            with patch.object(self.prediction_core.feature_extractor, 'extract_7d_phase_features', return_value=np.array([0.5, 0.3, 0.4, 0.6, 0.7, 0.2])):
                ...
                with patch.object(self.prediction_core, '_load_trained_frequency_model', return_value=None):
                    with patch.object(self.prediction_core, '_load_frequency_scaler', return_value=None):
    ...
    def test_analytical_fallback_coupling_prediction(self):
        ...
        with patch.object(self.prediction_core.feature_extractor, 'extract_coupling_features', return_value=features):
            with patch.object(self.prediction_core.feature_extractor, 'extract_7d_phase_features', return_value=np.array([0.6, 0.8, 0.5, 0.4, 0.7, 0.9, 0.8, 0.3])):
                # Mock no model available
                with patch.object(self.prediction_core, '_load_trained_coupling_model', return_value=None):
                    with patch.object(self.prediction_core, '_load_coupling_scaler', return_value=None):
                        # Test analytical fallback
                        result = self.prediction_core.predict_mode_coupling(self.envelope)
    ...
    def test_7d_phase_field_feature_extraction(self):
        ...
        freq_features = self.prediction_core.feature_extractor.extract_frequency_features(self.envelope)
        ...
        coup_features = self.prediction_core.feature_extractor.extract_coupling_features(self.envelope)
    ...
    def test_prediction_confidence_calculation(self):
        ...
        confidence = self.prediction_core._compute_prediction_confidence(features, mock_model)
    ...
    def test_feature_importance_extraction(self):
        ...
        expected_features = [
            ...
            "autocorrelation", "coupling_strength"
        ]
    ...
    def test_prediction_variance_calculation(self):
        ...
        variance = self.prediction_core._compute_prediction_variance(features, mock_model)
