...
class TestMLPredictionModels:
    ...
    def test_training_data_generation(self):
        ...
        X_freq, y_freq = self.data_generator.generate_frequency_training_data(
            n_samples=100
        )
        ...
        X_coup, y_coup = self.data_generator.generate_coupling_training_data(
            n_samples=100
        )
    ...
    def test_feature_extraction(self):
        ...
        # Extract features
        features = self.data_generator._extract_training_features(
            envelope, phase_params
        )
    ...
    def test_target_computation(self):
        ...
        target_frequencies = self.data_generator._compute_target_frequencies(
            phase_params
        )
    ...
    def test_prediction_accuracy(self):
        ...
        if self.trainer.model_manager.frequency_model is not None:
            X_freq_scaled = self.trainer.model_manager.frequency_scaler.transform(
                X_freq
            )
            y_freq_pred = self.trainer.model_manager.frequency_model.predict(
                X_freq_scaled
            )
        ...
        if self.trainer.model_manager.coupling_model is not None:
            ...
            y_coup_pred = self.trainer.model_manager.coupling_model.predict(
                X_coup_scaled
            )
