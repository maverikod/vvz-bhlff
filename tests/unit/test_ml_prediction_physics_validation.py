...
class TestMLPredictionPhysicsValidation:
                        ...
                        for m in range(block_shape[4]):
                            for n in range(block_shape[5]):
                                for o in range(block_shape[6]):
                                    ...
                                    envelope[i, j, k, l, m, n, o] = amplitude * np.exp(
                                        1j * phase
                                    )
    ...
    def test_prediction_accuracy_physics(self, test_envelope, test_parameters):
        ...
        low_freq_accuracy = optimizer._calculate_prediction_accuracy(
            test_parameters, low_freq_envelope
        )
    ...
    def test_prediction_precision_physics(self, test_envelope, test_parameters):
        ...
        noisy_precision = optimizer._calculate_prediction_precision(
            test_parameters, noisy_envelope
        )
    ...
    def test_phase_coherence_physics(self, test_envelope):
        ...
        random_phase_envelope = np.abs(test_envelope) * np.exp(
            1j * np.random.randn(*test_envelope.shape)
        )
        ...
        coherent_envelope = np.abs(test_envelope) * np.exp(
            1j * 0.1 * np.ones_like(test_envelope)
        )
        coherent_coherence = optimizer._compute_phase_coherence(coherent_envelope)
        assert (
            coherent_coherence > coherence
        )  # More coherent should have higher coherence
    ...
    def test_no_simplified_calculations_physics(self, test_envelope, test_parameters):
        ...
        modified_accuracy = optimizer._calculate_prediction_accuracy(
            test_parameters, modified_envelope
        )
        ...
        modified_accuracy2 = optimizer._calculate_prediction_accuracy(
            modified_params, test_envelope
        )
