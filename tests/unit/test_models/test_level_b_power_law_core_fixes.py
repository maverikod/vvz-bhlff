...
class TestPowerLawCoreFixes:
    ...
    def test_7d_correlation_function_implementation(
        ...
        assert (
            len(dimensional_correlations) == test_envelope_3d.ndim
        ), "Missing dimensional correlations"

        # Verify physical meaning: correlation should be positive and finite
        assert np.all(
            np.isfinite(correlation_7d)
        ), "Correlation function contains non-finite values"
        assert np.all(
            correlation_7d >= 0
        ), "Correlation function should be non-negative"
    ...
    def test_full_critical_exponents_implementation(
        ...
        for exponent in expected_exponents:
            assert (
                exponent in critical_exponents
            ), f"Missing critical exponent: {exponent}"
            assert isinstance(
                critical_exponents[exponent], (int, float)
            ), f"Invalid exponent type: {exponent}"
            assert np.isfinite(
                critical_exponents[exponent]
            ), f"Non-finite exponent: {exponent}"

        # Verify physical meaning of exponents
        assert (
            0.1 <= critical_exponents["nu"] <= 2.0
        ), "ν (correlation length exponent) out of range"
        assert (
            0.1 <= critical_exponents["beta"] <= 2.0
        ), "β (order parameter exponent) out of range"
        assert (
            0.5 <= critical_exponents["gamma"] <= 2.0
        ), "γ (susceptibility exponent) out of range"
        assert (
            1.0 <= critical_exponents["delta"] <= 10.0
        ), "δ (critical isotherm exponent) out of range"
        assert (
            -1.0 <= critical_exponents["eta"] <= 1.0
        ), "η (anomalous dimension) out of range"
        assert (
            -1.0 <= critical_exponents["alpha"] <= 1.0
        ), "α (specific heat exponent) out of range"
        assert (
            1.0 <= critical_exponents["z"] <= 4.0
        ), "z (dynamic exponent) out of range"

        # Verify scaling relations
        # δ = (γ + β) / β
        expected_delta = (
            critical_exponents["gamma"] + critical_exponents["beta"]
        ) / critical_exponents["beta"]
        assert (
            abs(critical_exponents["delta"] - expected_delta) < 0.1
        ), "Scaling relation δ = (γ + β) / β violated"

        # α = 2 - ν*d (for d=3)
        expected_alpha = 2 - critical_exponents["nu"] * 3
        assert (
            abs(critical_exponents["alpha"] - expected_alpha) < 0.1
        ), "Scaling relation α = 2 - ν*d violated"
        ...
        assert universality_class in [
            "mean_field",
            "ising_3d",
            "xy_3d",
            "custom_7d",
        ], "Invalid universality class"
    ...
    def test_mathematical_correctness_correlation_function(
        ...
        assert (
            abs(center_correlation - max_correlation) < 1e-10
        ), "C(0) should be maximum"

        # Mathematical property 2: C(r) should decay with distance
        correlation_decay = correlation_results["correlation_structure"][
            "correlation_decay"
        ]
        ...
        if len(radial_correlation) > 1:
            ...
            decay_trend = np.polyfit(
                np.arange(len(radial_correlation)), radial_correlation, 1
            )[0]
            assert (
                decay_trend <= 0.1
            ), "Correlation should generally decay with distance"

        # Mathematical property 3: Dimensional coupling should be symmetric
        dimensional_coupling = correlation_results["correlation_structure"][
    ...
    def test_mathematical_correctness_critical_exponents(
        ...
        rushbrooke = (
            critical_exponents["alpha"]
            + 2 * critical_exponents["beta"]
            + critical_exponents["gamma"]
        )
        assert (
            abs(rushbrooke - 2.0) < 0.2
        ), "Rushbrooke relation α + 2β + γ = 2 violated"

        # Widom relation: γ = β(δ - 1)
        widom = critical_exponents["beta"] * (critical_exponents["delta"] - 1)
        assert (
            abs(critical_exponents["gamma"] - widom) < 0.2
        ), "Widom relation γ = β(δ - 1) violated"

        # Mathematical property 2: Fisher relation: γ = ν(2 - η)
        fisher = critical_exponents["nu"] * (2 - critical_exponents["eta"])
        assert (
            abs(critical_exponents["gamma"] - fisher) < 0.3
        ), "Fisher relation γ = ν(2 - η) violated"

        # Mathematical property 3: Josephson relation: νd = 2 - α
        josephson = critical_exponents["nu"] * 3  # d = 3
        assert (
            abs(josephson - (2 - critical_exponents["alpha"])) < 0.2
        ), "Josephson relation νd = 2 - α violated"
    ...
    def test_physical_meaning_preservation(self, power_law_core, test_envelope_3d):
        ...
        for exponent, value in critical_exponents.items():
            ...
            assert value > 0 or exponent in [
                "eta",
                "alpha",
            ], f"Critical exponent {exponent} should be positive (except η, α)"

        # Physical meaning 3: Scaling regions should have consistent behavior
        for region in scaling_regions:
            assert (
                region["consistency"] >= 0
            ), "Scaling consistency should be non-negative"
    ...
    def test_performance_and_stability(self, power_law_core, test_envelope_3d):
        ...
        correlation_results_2 = power_law_core.compute_correlation_functions(
            test_envelope_3d
        )
    ...
    def test_backward_compatibility(self, power_law_core, test_envelope_3d):
        ...
        ), "Correlation results should be dict"
        ...
        assert (
            "spatial_correlation_7d" in correlation_results
        ), "Should have spatial correlation"
        assert (
            "critical_exponents" in critical_results
        ...
        assert (
            "amplitude_exponent" in power_law_results
        ), "Should have amplitude exponent"
