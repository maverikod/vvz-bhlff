...
class TestStep111ExponentialDecayFix:
    ...
    def test_no_exponential_decay_in_gravity_waves(self):
        ...
        # Check that exponential decay is not used in the main logic
        assert (
            'np.exp(-dt / self.params.get("damping_time"' not in content
        ), "Found exponential temporal damping in gravity_waves.py - violates 7D BVP theory"

        assert (
            'np.exp(-dx / self.params.get("spatial_damping"' not in content
    ...
    def test_step_resonator_implementation(self):
        ...
        temporal_damping = [
            wave_calc._step_resonator_boundary_condition(dt) for dt in dt_values
        ]
        ...
        spatial_damping = [
            wave_calc._step_resonator_spatial_boundary(dx) for dx in dx_values
        ]
    ...
    def test_no_exponential_in_other_models(self):
        ...
        for file_path in model_files:
            try:
                ...
                assert (
                    "np.exp(-" not in content or "classical comparison" in content
    ...
    def test_step_resonator_physics_correctness(self):
        ...
        assert np.all(
            transmission[below_cutoff] == 0.9
        ), "Step resonator should transmit below cutoff frequency"

        assert np.all(
            transmission[above_cutoff] == 0.0
        ...
        assert (
            transmission[cutoff_index - 1] == 0.9
        ), "Step resonator should be 0.9 just below cutoff"
        assert (
            transmission[cutoff_index] == 0.0
        ...
        for file_path in test_files:
            try:
                ...
                if "np.exp(" in content:
                    assert (
                        "classical comparison" in content or "for comparison" in content
